# Builds cmux-cua (Rust MCP server binary) for Linux.
#
# The cmux-cua binary speaks MCP JSON-RPC 2.0 over stdio and provides
# 40+ tools for screen capture, mouse/keyboard input, window management,
# and accessibility-based element interaction.
#
# Usage:
#   cuaDriver = import ./package.nix { inherit pkgs; src = ./../../libs/cmux-cua/rust; };
#
{
  pkgs,
  src,
...
pkgs.rustPlatform.buildRustPackage {
  ...
  # platform-macos, platform-windows, and cmux-cua-uia
  # which are gated behind cfg(target_os) and won't compile on Linux.
  # Using -p cmux-cua ensures Cargo only resolves Linux dependencies.
  #
  ...
  cargoBuildFlags = [ "-p" "cmux-cua" "--features" "portal-input,portal-capture" ];
  cargoTestFlags = [ "-p" "cmux-cua" "--features" "portal-input,portal-capture" ];
  ...
  meta = with pkgs.lib; {
    ...
    homepage = "https://github.com/manaflow-ai/cmux-cua";
    license = licenses.mit;
    mainProgram = "cmux-cua";
