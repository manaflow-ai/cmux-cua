...
Nix has two responsibilities in the cmux-cua test stack: reproducible Linux
builds and desktop-session dependencies for the canonical Rust harnesses.

| Attribute | Purpose | CI |
| --- | --- | --- |
| `cmux-cua-build` | Build the shipped Linux package from the locked Rust source | `ci-nix-linux.yml` |
| `cmux-cua-linux-rust-unit` | Compile and run the source-owned headless Rust tests | `ci-nix-linux.yml` |
| `cmux-cua-wayland-e2e` | Provide the Sway, GTK, WebKit, Electron, capture, and Rust toolchain used by native Wayland E2E | `e2e-rust-linux-wayland.yml` |
| `cmux-cua-inject-e2e` | Provide the same typed harness toolchain plus the nested `cua-compositor` package | Experimental nested-injection workflow |
...
```bash
nix develop .#cmux-cua-wayland-e2e -c \
...
JSONL schema and retain MP4 trajectories under `artifacts/cmux-cua/linux/`.
