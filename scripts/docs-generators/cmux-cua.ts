...
/**
 * cmux CUA Documentation Generator
 *
 * Generates MDX documentation files from the cmux-cua `dump-docs` command output.
 * This ensures documentation stays synchronized with the source code.
 *
 * Usage:
 *   npx tsx scripts/docs-generators/cmux-cua.ts          # Generate docs
 *   npx tsx scripts/docs-generators/cmux-cua.ts --check  # Check for drift (CI mode)
 */
...
const CMUX_CUA_DIR = path.join(ROOT_DIR, 'libs', 'cmux-cua', 'rust');
const CMUX_CUA_BIN = path.join(
  CMUX_CUA_DIR,
  'target',
  'release',
  process.platform === 'win32' ? 'cmux-cua.exe' : 'cmux-cua'
);
const DOCS_OUTPUT_DIR = path.join(ROOT_DIR, 'docs', 'content', 'docs', 'reference', 'cmux-cua');
const TAG_PREFIX = 'cmux-cua-v';
...
async function main() {
  ...
  console.log('cmux CUA Documentation Generator');
  console.log('===================================\n');

  // Step 1: Build cmux-cua
  console.log('Building cmux-cua...');
  try {
    const cargo = resolveCargoCommand();
    execFileSync(cargo, ['build', '-p', 'cmux-cua', '--release'], {
      cwd: CMUX_CUA_DIR,
      stdio: 'inherit',
    });
  } catch (error) {
    console.error('Failed to build cmux-cua');
    process.exit(1);
  }

  // Step 2: Extract all docs in a single invocation
  console.log('\nExtracting documentation...');
  const binary = process.env.CMUX_CUA_BINARY || CMUX_CUA_BIN;
  ...
  if (checkOnly) {
    ...
    if (hasDrift) {
      console.error(
        "\nRun 'npx tsx scripts/docs-generators/cmux-cua.ts' to update documentation"
      );
      process.exit(1);
    }

    console.log('\nAll cmux-cua documentation is up to date!');
  } else {
    ...
    console.log('\ncmux-cua documentation generated successfully!');
  }
}
...
export function generateCLIReferenceMDX(docs: CLIDocumentation, releasedVersion: string): string {
  ...
  lines.push('description: Command-line interface specification for cmux CUA');
  lines.push('---');
  lines.push('');
  lines.push(`{/*
  AUTO-GENERATED FILE - DO NOT EDIT DIRECTLY
  Generated by: npx tsx scripts/docs-generators/cmux-cua.ts
  Source: cmux-cua dump-docs
...
*/}`);
  ...
  lines.push('');
  lines.push(
    `Documented against cmux CUA **${releasedVersion}**. Run \`cmux-cua --version\` for your installed version.`
  );
  lines.push('');
  lines.push(
    'The macOS-only `cmux-cua permissions` command is documented separately in [macOS permissions](/reference/cmux-cua/macos-permissions).'
  );
  lines.push('');
  lines.push('---');
  lines.push('');
  ...
  const excludedFromReference = ['permissions']; // -> reference/cmux-cua/macos-permissions
...
export function generateCommandDoc(cmd: CommandDoc): string[] {
  ...
  if (cmd.subcommands.length > 0) {
    for (const sub of cmd.subcommands) {
      lines.push(`#### \`cmux-cua ${cmd.name} ${sub.name}\``);
      lines.push('');
      ...
      if (sub.subcommands.length > 0) {
        for (const nested of sub.subcommands) {
          lines.push(`##### \`cmux-cua ${cmd.name} ${sub.name} ${nested.name}\``);
          lines.push('');
...
export function generateMCPToolsMDX(docs: MCPDocumentation, releasedVersion: string): string {
  ...
  lines.push('description: Reference for every MCP tool cmux CUA exposes');
  lines.push('---');
  lines.push('');
  lines.push(`{/*
  AUTO-GENERATED FILE - DO NOT EDIT DIRECTLY
...
*/}`);
  ...
  lines.push(
    `\`cmux-cua\` exposes ${docs.tools.length} MCP tools through a single stdio server (\`cmux-cua mcp\`). Every tool is also callable from the shell as \`cmux-cua <name> '<JSON-args>'\`.`
  );
  lines.push('');
  lines.push(
    'Tool names are `snake_case`. Responses are MCP `CallTool.Result` envelopes: a text content block prefixed with a `✅` summary (or the error reason on failure), plus optional image or structured-content blocks on tools that produce them. See the [CLI reference](/reference/cmux-cua/cli-reference) for CLI-specific options like `--socket` and `--screenshot-out-file`.'
  );
  lines.push('');
  lines.push(
    'For the cross-cutting parameter contract (shared parameters, required-parameter rules, platform-specific parameters) and the action response shape, see [MCP tool notes](/reference/cmux-cua/mcp-tool-notes).'
  );
  lines.push('');
  lines.push('<Callout type="info">');
  lines.push(
    '  Tool names here match the CLI form exactly. `cmux-cua list_apps` and the MCP `list_apps` tool run the same code path.'
  );
  lines.push('</Callout>');
  lines.push('');
  lines.push('<Callout type="info">');
  lines.push(
    "  **TCC auto-delegation.** When an MCP client spawns `cmux-cua mcp` from an IDE terminal (Claude Code, Cursor, VS Code, Warp), macOS attributes the subprocess to the parent terminal — not `cmux Computer Use.app` — so AX probes fail against the wrong bundle id. `mcp` detects this and auto-launches a `cmux-cua serve` daemon via `open -n -g -a 'cmux Computer Use' --args serve`, then proxies every tool call through the daemon's Unix socket. Tool semantics are identical to the in-process path; no Python bridge is needed. Pass `--no-daemon-relaunch` (or set `CMUX_CUA_MCP_NO_RELAUNCH=1`) to force in-process execution. See the [process model](/reference/cmux-cua/process-model) for the full lifecycle, failure modes, and wrapper-author guidance."
  );
  lines.push('</Callout>');
  lines.push('');
