...
# macOS harness tests use the installed, TCC-authorized cmux-cua daemon path.
set -euo pipefail
...
The testkit proxies MCP calls through the installed CmuxCua daemon.
...
ARTIFACT_DIR="${REPO_ROOT}/artifacts/cmux-cua/macos"
...
if [[ "${SUITE}" == shared || "${SUITE}" == all ]]; then
  run_test shared-app-matrix cargo test -p cmux-cua --test cross_platform_behavior_test -- \
...
if [[ "${SUITE}" == native || "${SUITE}" == all ]]; then
  for appkit_test in \
    ...
    run_test "appkit-${appkit_test}" cargo test -p cmux-cua --test harness_appkit_test -- \
  ...
  for swiftui_test in \
    ...
    run_test "swiftui-${swiftui_test}" cargo test -p cmux-cua --test harness_swiftui_test -- \
      --ignored --exact "${swiftui_test}" --nocapture --test-threads=1
  done
  run_test installed-app-launch cargo test -p cmux-cua --test installed_app_launch_macos_test -- \
    --ignored --nocapture --test-threads=1
  run_test installed-app-textedit cargo test -p cmux-cua --test installed_app_textedit_macos_test -- \
    --ignored --exact background_type_on_native_cocoa_is_ax_verified \
    --nocapture --test-threads=1
fi
if [[ "${SUITE}" == capture || "${SUITE}" == all ]]; then
  run_test capture-contract cargo test -p cmux-cua --test capture_contract_test -- \
    --ignored --nocapture --test-threads=1
  run_test desktop-scope cargo test -p cmux-cua --test desktop_scope_macos_test -- \
    --ignored --nocapture --test-threads=1
fi
