...
def _package_json_member(archive: tarfile.TarFile) -> tarfile.TarInfo:
    ...
    for member in archive.getmembers():
        name = _validated_archive_member_name(member)
        _record_archive_member(name, member.name, seen)
        total_size = _bounded_archive_size(total_size, member.size)
...
def _validated_archive_member_name(member: tarfile.TarInfo) -> str:
...
def _record_archive_member(name: str, display_name: str, seen: set[str]) -> None:
    if name in seen:
        raise IdentityError(f"npm artifact contains a duplicate archive path: {display_name}")
    seen.add(name)
    if len(seen) > MAX_ARCHIVE_MEMBERS:
        raise IdentityError("npm artifact contains too many archive members")


def _bounded_archive_size(total_size: int, member_size: int) -> int:
    if member_size < 0:
        raise IdentityError("npm artifact contains a negative member size")
    result = total_size + member_size
    if result > MAX_ARCHIVE_BYTES:
...
def _expected_workflow_ref(
    event_name: str, ref_type: str, ref_name: str, identity: PublisherIdentity
) -> str:
    if event_name == "workflow_run":
        ...
        if source_tag != identity.tag:
            raise IdentityError(
                f"source tag must be exactly {identity.tag!r} for this package release"
            )
    ...
    if event_name == "push":
        ...
        if ref_name != identity.tag:
            raise IdentityError(
                f"GitHub tag must be exactly {identity.tag!r} for this package release"
            )
        return f"refs/tags/{identity.tag}"
...
def validate_context(identity: PublisherIdentity) -> None:
    if required_environment("GITHUB_REF_PROTECTED", "GitHub ref protection") != "true":
        raise IdentityError("npm publishing requires a protected release ref")
