...
`escalation` is the machine-readable climb-the-ladder hint. When present, it tells the caller which surface to try next: `"px"` for acting off the screenshot, `"foreground"` for explicitly fronting the target, or `"page"` for the browser-tab DOM path through the `page` tool. See [Agent action policy](/reference/cmux-cua/action-selection-policy) for the agent behavior and [MCP tool notes](/reference/cmux-cua/mcp-tool-notes#action-response-shape) for the field table.
...
The exact validity matrix and platform-support table live in [Interface contracts](/reference/cmux-cua/contracts). Keep this page as the mental model: observe both tree and pixels, start with the accessibility rung, use the screenshot-backed pixel rung when the tree is insufficient, and escalate to foreground only when the target app requires it.
