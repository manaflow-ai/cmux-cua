...
These notes are hand-maintained companions to the auto-generated [MCP Tools](/reference/cmux-cua/mcp-tools) reference. They document the cross-cutting parameter contract and response shape that span multiple tools and are not derivable from any single tool's schema.
...
`get_text`, `query_dom`, `click_element`, and `execute_javascript` work cross-platform (macOS, Windows, Linux). `insert_text` and `type_keystrokes` are implemented on macOS only for now; Windows and Linux return a clear "not implemented" error rather than a silent no-op. Tracked in [trycua/cua#2084](https://github.com/manaflow-ai/cmux-cua/issues/2084).
