...
Only the exact value `CUA_DRIVER_EMBEDDED=1` enables embedded mode. The host bundle id is an advisory label echoed in `check_permissions`; trust still comes from macOS's responsibility chain. `CUA_DRIVER_DEFAULT_SESSION` gives argument-less MCP actions a stable process-owned agent cursor. If it is omitted or empty, the driver uses `embedded-<pid>` instead. An explicit tool `session` or `cursor_id` still takes precedence.
...
| Anonymous action cursor | None | `CUA_DRIVER_DEFAULT_SESSION`, else `embedded-<pid>` |
