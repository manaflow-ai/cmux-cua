...
- **The branch also contains intentional work outside the cmux CUA convergence
...
- **Historical evidence** = CI runs on #2161 / the convergence branch, and the run
  records in `libs/cmux-cua/docs/e2e-convergence-journal.md`. It proves the _final
...
- `libs/cmux-cua/rust/**` (crates, tests, Skills, `Cargo.toml`, `Cargo.lock`, READMEs)
- `libs/cmux-cua/tests/**` (fixtures, runners - including the deletions)
- `libs/cmux-cua/docs/**`, `libs/cmux-cua/README.md`, `libs/cmux-cua/scripts/**`
- `libs/cmux-cua/wayland-helper/**`
- `scripts/ci/**` (`README.md`, `link-e2e-evidence.sh`, `linux/`, `macos/`, `windows/`)
- `nix/cmux-cua/**`, `flake.nix`
- `.github/workflows/`: `e2e-rust-windows.yml`, `e2e-rust-linux.yml`,
  `e2e-rust-linux-wayland.yml`, `ci-rust-linux.yml`, `ci-rust-windows.yml`,
  `ci-nix-linux.yml`, `cd-rust-cmux-cua.yml`, `ci-release-reminder.yml`,
  and the deletions `ci-cmux-cua-interactive-linux.yml`, `nix-build.yml`,
  `nix-screenshot.yml`, `nix-wayland.yml`
- `.github/scripts/tests/test_cmux_cua_release_wiring.py`
- `docs/content/docs/concepts/{how-cmux-cua-is-validated.mdx,index.mdx,meta.json}`
- `docs/content/docs/reference/cmux-cua/**`
- Root docs: `README.md`, `CONTRIBUTING.md`, `Development.md`, `TESTING.md`

### 2.2 Preservation queue (excluded from the seven cmux CUA PRs)
...
Separation enforcement: every cmux CUA stack branch must satisfy
...
cd libs/cmux-cua/rust && cargo check --workspace   # regenerates Cargo.lock honestly
```

Common fresh gates (every PR that touches `libs/cmux-cua/rust`):
...
### PR 1 - `codex/conv2-01-typed-evidence` - "test(cmux-cua): typed e2e evidence foundation (conv2 1/7)"
...
- `libs/cmux-cua/rust/crates/cmux-cua-testkit/**` - whole crate: `Cargo.toml`,
...
- `crates/cmux-cua-core/src/`: `cdp.rs`, `recording.rs`, `recording_tools.rs`, `tool.rs`
- `crates/cmux-cua/src/`: `main.rs`, `serve.rs`; `crates/cmux-cua/tests/README.md`
...
- Dev tooling: `libs/cmux-cua/scripts/_install-local-rust.sh`, `sync-vm-worktree.sh`
- `rust/Skills/cmux-cua/RECORDING.md` (documents recording behavior changed here)
- `Cargo.lock` (regenerated via `cargo check`, not copied)

**Excludes:** all platform crate changes, all workflows, all `libs/cmux-cua/docs/**`,
...
- [ ] `shellcheck scripts/ci/link-e2e-evidence.sh libs/cmux-cua/scripts/*.sh`
- [ ] `cargo run -p cmux-cua-testkit --bin cua-e2e-report -- --help` (or its
...
### PR 2 - `codex/conv2-02-windows` - "fix(cmux-cua): Windows e2e convergence (conv2 2/7)"
...
### PR 3 - `codex/conv2-03-macos` - "fix(cmux-cua): macOS e2e convergence (conv2 3/7)"
...
- [ ] `bash libs/cmux-cua/tests/fixtures/build/macos.sh` +
      `bash libs/cmux-cua/tests/fixtures/smoke/macos.sh` pass locally
...
### PR 4 - `codex/conv2-04-linux-x11` - "fix(cmux-cua): Linux core and X11 e2e convergence (conv2 4/7)"
...
- CI: `scripts/ci/linux/run-rust-e2e.sh`, `run-rust-e2e-desktop.sh` (A),
  ...
  **deletes** `.github/workflows/ci-cmux-cua-interactive-linux.yml`
- `rust/Skills/cmux-cua/LINUX.md`
- `Cargo.lock` regen
...
- [ ] Confirm nothing still references the deleted interactive workflow:
      `git grep -n ci-cmux-cua-interactive-linux` -> empty
...
### PR 5 - `codex/conv2-05-wayland` - "fix(cmux-cua): Wayland backend convergence (conv2 5/7)"
...
- Manifests (whole-file final state - the delta in each is entirely this PR's):
  `crates/platform-linux/Cargo.toml` (feature split), `crates/cmux-cua/Cargo.toml`
  (feature forwarding)
- CI: `scripts/ci/linux/run-rust-e2e-wayland.sh` (A),
  `.github/workflows/e2e-rust-linux-wayland.yml`,
  `.github/workflows/cd-rust-cmux-cua.yml` (release builds gain
  `--features portal-input` + `libxkbcommon-dev`)
- `libs/cmux-cua/wayland-helper/{README.md,winrects@cua/extension.js,winrects@cua/metadata.json}`
- `Cargo.lock` regen
...
- [ ] `cd-rust-cmux-cua.yml` build job dry-run (workflow_dispatch or `act`/container
...
### PR 6 - `codex/conv2-06-nix` - "ci(cmux-cua): nested compositor Nix consolidation (conv2 6/7)"

**Purpose:** consolidate the Nix story - rewritten `flake.nix` and
`nix/cmux-cua/package.nix` (new feature names), nested-compositor patch, delete the
21 superseded `.nix` test files and the three standalone nix workflows in favor of
`ci-nix-linux.yml`.

**Owns:**

- `flake.nix`, `nix/cmux-cua/{README.md,package.nix,compositor/cua_compositor_patch.py,tests/README.md}`
- `scripts/ci/linux/run-rust-e2e-inject.sh` (A), which activates the nested
  environment packaged by this PR
- **Deletes** `nix/cmux-cua/tests/*.nix` (integration, linux-background-gui,
  ...
  and `nix/cmux-cua/tests/wayland/**` (README.md + 10 `.nix` files)
...
- [ ] `nix flake check` and `nix build .#cmux-cua` (or the flake's package attr)
      succeed locally/CI
...
### PR 7 - `codex/conv2-07-docs-cleanup` - "docs(cmux-cua): convergence docs and replacement cleanup (conv2 7/7)"
...
- `docs/content/docs/concepts/`: `how-cmux-cua-is-validated.mdx` (A), `index.mdx`;
  `meta.json` - **hand-merge, never final-state** (see Section 4): insert the
  `how-cmux-cua-is-validated` entry into _current main's_ `meta.json`
- `docs/content/docs/reference/cmux-cua/`: `development.mdx`, `limits.mdx`,
  `mcp-tools.mdx`, `meta.json`, `platform-roadmap.mdx` (A), `platform-support.mdx` (A)
- `libs/cmux-cua/docs/`: adds `action-support.md`, `e2e-convergence-journal.md`,
...
- Root docs: `README.md`, `CONTRIBUTING.md`, `Development.md`, `TESTING.md`,
  `libs/cmux-cua/README.md`, `libs/cmux-cua/rust/README.md`
- Replacement cleanup - **deletes whole directories**:
  `libs/cmux-cua/tests/fixtures/linux-container/**` (5 files),
  `libs/cmux-cua/tests/fixtures/modality-recordings/**` (16 files),
  `libs/cmux-cua/tests/fixtures/vision-agent-test/**` (2 files);
...
- Release wiring: `.github/workflows/ci-release-reminder.yml` +
  `.github/scripts/tests/test_cmux_cua_release_wiring.py` (checklist links the
...
- [ ] `python -m unittest .github/scripts/tests/test_cmux_cua_release_wiring.py` passes
- [ ] `git grep -n 'modality-recordings\|linux-container\|vision-agent-test' -- ':!libs/cmux-cua/docs'`
      -> empty (nothing references deleted fixtures)
- [ ] Rendered-page spot check of `platform-support.mdx` / `platform-roadmap.mdx` /
      `how-cmux-cua-is-validated.mdx`
...
| `crates/cmux-cua-testkit/**`                                     | PR 1 only                                            | Entire crate (incl. cfg-gated `windows_setup.rs`) lands in PR 1 so `lib.rs` is final-state and compiles everywhere.                                                                                               |
...
| `crates/platform-linux/Cargo.toml`, `crates/cmux-cua/Cargo.toml` | PR 5 only                                            | Verified: the entire delta in both is the portal feature split -> PR 5 takes final state.                                                                                                                         |
...
| Workflows                                                          | one owner each (table in Section 2.1 / per-PR lists) | A workflow file never spans two PRs. `cd-rust-cmux-cua.yml` -> PR 5; `ci-release-reminder.yml` -> PR 7 (its wiring test rides with it).                                                                         |
...
land a follow-up to that layer first. Never "fix" a cmux CUA PR by pulling in a
sidecar-owned file.
...
git diff origin/main codex/link-e2e-videos -- \
  libs/cmux-cua nix/cmux-cua flake.nix scripts/ci \
  ...
  .github/workflows/cd-rust-cmux-cua.yml .github/workflows/ci-release-reminder.yml \
  .github/scripts/tests/test_cmux_cua_release_wiring.py \
  docs/content/docs/concepts docs/content/docs/reference/cmux-cua \
...
1. `libs/cmux-cua/rust/Cargo.lock` - regenerated, semantically compared via
...
(conv2 N/7, plan: libs/cmux-cua/docs/pr-2161-landing-plan.md)`. #2161's description
...
- [ ] The first post-landing cmux-cua release (per the `ci-release-reminder`
