# cmux CUA Test Harnesses Guide

This contributor guide explains where the cmux CUA tests live, how to run
...
|   |-- rust/
|   |   |-- crates/
|   |   |   |-- cmux-cua/          Rust driver and integration tests
|   |   |   |-- cmux-cua-core/     Shared driver logic and unit tests
|   |   |   |-- cmux-cua-testkit/  Shared Rust E2E helpers and evidence capture
...
| `cmux-cua-testkit` | Session helpers, fixture launching, screenshots, recordings, trajectories | The scenario list |
...
```text
libs/cmux-cua/rust/crates/cmux-cua/tests/
```
...
`cmux-cua-testkit::DesktopObserver` owns the shared interface. Native Windows,
...
```text
rust/crates/cmux-cua-testkit/src/
`-- observer.rs                     Cross-OS desktop-side-effect interface

rust/crates/cmux-cua/tests/
...
2. Add the Rust scenario under `rust/crates/cmux-cua/tests/`.
