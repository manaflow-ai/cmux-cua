...
[public platform roadmap](https://cua.ai/docs/reference/cmux-cua/platform-roadmap).
...
- Mark stale sections of `libs/cmux-cua/docs/test-harness-convergence-plan.md`
  as superseded by current evidence.
- Rewrite `nix/cmux-cua/tests/README.md`; it currently names deleted fixtures.
- Remove the empty `nix/cmux-cua/tests/wayland/` directory if it is still empty.
- Add the experimental nested-compositor row to
  `libs/cmux-cua/docs/action-support.md` and link it from the public platform
  support page.
...
- Add a `cmux-cua-inject-e2e` development shell containing the compositor and
