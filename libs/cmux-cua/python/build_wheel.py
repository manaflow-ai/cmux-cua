#!/usr/bin/env python3
"""Build script to download platform-specific cmux-cua binary and create wheel.

This script:
1. Detects the current platform
2. Downloads the appropriate cmux-cua binary from GitHub releases
3. Places it in src/cmux_cua/bin/
...
def get_platform_info(arch_override: str = None):
    ...
    # Map to cmux-cua release naming
    if system == "darwin":
...
def get_release_url(version: str, platform_name: str, arch: str) -> tuple[str, list[str]]:
    ...
    """
    base_url = f"https://github.com/manaflow-ai/cmux-cua/releases/download/cmux-cua-v{version}"

    if platform_name == "darwin":
        # Universal binary tarball
        filename = f"cmux-cua-{version}-darwin-universal-binary.tar.gz"
        binary_names = ["cmux-cua"]
    elif platform_name == "linux":
        filename = f"cmux-cua-{version}-linux-{arch}-binary.tar.gz"
        binary_names = ["cmux-cua"]
    elif platform_name == "windows":
        filename = f"cmux-cua-{version}-windows-{arch}-binary.zip"
        # Windows includes both main executable and UIAccess worker
        binary_names = ["cmux-cua.exe", "cmux-cua-uia.exe"]
    else:
...
def get_expected_sha256(version: str, archive_name: str) -> str:
    ...
    checksums_url = f"https://github.com/manaflow-ai/cmux-cua/releases/download/cmux-cua-v{version}/checksums.txt"
...
def build_wheel(package_dir: Path, wheel_tag: str, target_arch: str = None) -> None:
    ...
    env = os.environ.copy()
    env["CMUX_CUA_WHEEL_TAG"] = wheel_tag
...
def main():
    parser = argparse.ArgumentParser(description="Build cmux-cua Python wheel with bundled binary")
    parser.add_argument(
        "--version",
        help="cmux-cua version to download (default: pyproject.toml version)",
    )
    parser.add_argument(
        "--arch",
    ...
    bin_dir = script_dir / "src" / "cmux_cua" / "bin"
    ...
    expected_binary = "cmux-cua.exe" if sys.platform == "win32" else "cmux-cua"
