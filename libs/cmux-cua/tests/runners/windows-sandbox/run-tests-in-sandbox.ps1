...
try {
    ...
    cargo test -p cmux-cua --test protocol_handshake_test --no-run
    ...
    cargo test -p cmux-cua --test harness_wpf_test --no-run
    ...
    cargo test -p cmux-cua --test harness_winui3_test --no-run
    ...
    cargo test -p cmux-cua --test harness_web_test --no-run
    ...
    cargo test -p cmux-cua --test launch_windows_test --no-run
    ...
    cargo test -p cmux-cua --test agent_cursor_windows_test --no-run
...
<Configuration>
  ...
  <LogonCommand>
    <Command>powershell -ExecutionPolicy Bypass -NonInteractive -File C:\cmux-cua\tests\runners\windows-sandbox\sandbox-runner.ps1</Command>
  </LogonCommand>
</Configuration>
"@
$wsbPath = "$env:TEMP\cmux-cua-test.wsb"
