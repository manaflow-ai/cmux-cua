...
# -- set env so binary_path() resolves cmux-cua.exe correctly -------------
$repoRoot = "C:\cmux-cua"
$rustRoot = Join-Path $repoRoot "rust"
$targetDeps = Join-Path $rustRoot "target\debug\deps"

$env:CARGO_MANIFEST_DIR = Join-Path $rustRoot "crates\cmux-cua"
Log "CARGO_MANIFEST_DIR = $env:CARGO_MANIFEST_DIR"

$driverExe = Join-Path $rustRoot "target\debug\cmux-cua.exe"
