#!/usr/bin/env bash
# macOS cmux-cua per-tool smoke test.
#
# Spawns the built AppKit harness app from libs/cmux-cua/rust/test-apps/harness-appkit
# (source lives under libs/cmux-cua/tests/fixtures/apps/macos),
# runs every cmux-cua tool against it with sensible JSON args, and reports
...
# CLI (`cmux-cua call <tool> <json>`) rather than the MCP stdio loop,
...
#
# Prereqs:
#   - libs/cmux-cua/tests/fixtures/build/macos.sh must have been run
#   - cmux-cua binary built (this script uses target/release if present)
#   - TCC Accessibility permission granted to the cmux-cua binary
#     (otherwise AX-using tools return empty trees, which scores as PASS
#     because the call exits cleanly — flagged in output)
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)"
DRIVER="$ROOT/libs/cmux-cua/rust/target/release/cmux-cua"
[[ -x "$DRIVER" ]] || DRIVER="$ROOT/libs/cmux-cua/rust/target/debug/cmux-cua"
HARNESS_APP="$ROOT/libs/cmux-cua/rust/test-apps/harness-appkit/CuaTestHarness.AppKit.app"
...
echo "============================================================"
echo "macOS cmux-cua smoke test"
echo "Driver: $DRIVER"
...
if [[ ! -x "$DRIVER" ]]; then
    echo "ERROR: cmux-cua binary not found at $DRIVER" >&2
    echo "       Run: cd libs/cmux-cua/rust && cargo build --release -p cmux-cua" >&2
    exit 1
fi

if [[ ! -x "$HARNESS_EXE" ]]; then
    echo "ERROR: AppKit harness not built at $HARNESS_EXE" >&2
    echo "       Run: libs/cmux-cua/tests/fixtures/build/macos.sh" >&2
    exit 1
fi
