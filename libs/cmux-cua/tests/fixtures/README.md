# cmux-cua test fixtures

Deterministic host apps and shared fixtures used to verify cmux-cua across
Windows, macOS, and Linux. The fixtures are source-first: build scripts stage
local outputs into `libs/cmux-cua/rust/test-apps/harness-<name>/`; binaries are not
committed.
...
Build scripts stage outputs into `libs/cmux-cua/rust/test-apps/`.

```bash
# macOS: AppKit, SwiftUI, WKWebView, Electron, Tauri
libs/cmux-cua/tests/fixtures/build/macos.sh
libs/cmux-cua/tests/fixtures/build/macos.sh --skip electron
libs/cmux-cua/tests/fixtures/build/macos.sh --only wkwebview

# Linux: GTK3, Electron, Tauri
libs/cmux-cua/tests/fixtures/build/linux.sh
libs/cmux-cua/tests/fixtures/build/linux.sh --skip tauri
```

```powershell
# Windows: WPF, WinUI3, WebView2, Electron, Tauri
cd libs\cmux-cua\tests\fixtures
.\build\windows.ps1
...
Rust tests under `libs/cmux-cua/rust/crates/cmux-cua/tests/` consume the staged
...
Rust integration tests under `libs/cmux-cua/rust/crates/cmux-cua/tests/` drive
