# cmux-cua uninstaller (Windows) — removes everything install.ps1
...
#   $env:CMUX_CUA_UNINSTALL_FORCE = '1'
#   irm https://cua.ai/driver/uninstall.ps1 | iex
#
...
#
# What gets removed:
#   - Scheduled Task 'cmux-cua-serve' (autostart entry registered by
#     `cmux-cua autostart enable` or install.ps1 -AutoStart)
#   - Any running cmux-cua.exe processes (so file handles don't pin
#     the binary directory open during the delete pass)
#   - <visibleBinDir>     = %LOCALAPPDATA%\Programs\Cua\cmux-cua\bin  (directory junction)
#   - <currentDir>        = %USERPROFILE%\.cmux-cua\packages\current  (directory junction)
#   - <packageHome>       = %USERPROFILE%\.cmux-cua\                  (entire tree:
#                                                                              releases, lockfile,
#                                                                              telemetry id,
#                                                                              install marker,
#                                                                              version_check.json)
#   - Skill junctions under:
#       %USERPROFILE%\.claude\skills\cmux-cua
#       %USERPROFILE%\.agents\skills\cmux-cua
#       %USERPROFILE%\.openclaw\skills\cmux-cua
#       %APPDATA%\opencode\skills\cmux-cua
...
# `claude mcp remove cmux-cua` command for the user to run.
#
# Env overrides (mirror install.ps1's variable names):
#   $env:CMUX_CUA_INSTALL_DIR   visible bin dir to remove
#                                    (default %LOCALAPPDATA%\Programs\Cua\cmux-cua\bin;
#                                     v0.2.13 and earlier used Programs\trycua\cmux-cua\bin
#                                     — that legacy path is always cleaned up too)
#   $env:CMUX_CUA_HOME          package home to remove
#                                    (default %USERPROFILE%\.cmux-cua;
#                                     v0.2.13 and earlier used .cmux-cua —
#                                     that legacy path is always cleaned up too)
#
# Env (force mode):
#   $env:CMUX_CUA_UNINSTALL_FORCE
...
#
# Elevation:
#   `install.ps1 -AutoStart` (and `cmux-cua autostart enable`) register
#   the `cmux-cua-serve` Scheduled Task at RunLevel=Highest — the
...
#   under ~\.cmux-cua\... . If we detect either condition at startup
...
$Force = [bool]$env:CMUX_CUA_UNINSTALL_FORCE

# ---------- Elevation pre-check -------------------------------------------

$AutoStartTask = "cmux-cua-serve"
...
function Test-NeedsElevation {
    ...
    # needs admin) or a cmux-cua.exe is running (its parent was the
    ...
    $hasDaemon = @(Get-Process -Name "cmux-cua" -ErrorAction SilentlyContinue).Count -gt 0
    return ($hasTask -or $hasDaemon)
}

if (-not (Test-IsElevated) -and (Test-NeedsElevation)) {
    Write-Host "==> cmux-cua uninstaller: detected -AutoStart install state" -ForegroundColor Cyan
    Write-Host "    (the 'cmux-cua-serve' task is RunLevel=Highest and/or a daemon is"
    ...
    #
    # No -Force forwarding needed: $env:CMUX_CUA_UNINSTALL_FORCE
    ...
    if (-not $scriptPath) {
        $tmp = Join-Path $env:TEMP ("cmux-cua-uninstall-" + [Guid]::NewGuid().ToString('N') + ".ps1")
...
# libs/cmux-cua/rust/crates/cmux-cua/src/skills.rs (the verb that
# creates them) so we remove from the same paths. Both the current
# `cmux-cua` name and the pre-rename `cmux-cua` name are swept
# so a user who installed before the rename ends up clean.
$SkillJunctions = @(
    (Join-Path $env:USERPROFILE ".claude\skills\cmux-cua"),
    (Join-Path $env:USERPROFILE ".agents\skills\cmux-cua"),
    (Join-Path $env:USERPROFILE ".openclaw\skills\cmux-cua"),
    (Join-Path $env:APPDATA      "opencode\skills\cmux-cua"),
    (Join-Path $env:USERPROFILE ".gemini\skills\cmux-cua"),
    (Join-Path $env:USERPROFILE ".hermes\skills\cmux-cua"),
    (Join-Path $env:USERPROFILE ".claude\skills\cmux-cua"),
    (Join-Path $env:USERPROFILE ".agents\skills\cmux-cua"),
    (Join-Path $env:USERPROFILE ".openclaw\skills\cmux-cua"),
    (Join-Path $env:APPDATA      "opencode\skills\cmux-cua"),
...
Write-Step "cmux-cua uninstaller (Windows)"
Write-Step "  bin dir     : $VisibleBinDir"
Write-Step "  package home: $HomeDir"
Write-Host ""

# 1. Scheduled Task autostart (registered by `cmux-cua autostart enable`
...
# 2. Running cmux-cua.exe processes. The serve daemon and any active
#    `cmux-cua` invocation hold file handles to the binary, which
#    pin the directory junction's target open and make Remove-Item
#    fail with "in use". Stop them up front so subsequent deletes
#    aren't racy.
$running = @(Get-Process -Name "cmux-cua" -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) {
    Write-Step "stopping $($running.Count) running cmux-cua.exe process(es)"
    ...
    Write-Step "no running cmux-cua.exe processes"
}
...
if (Test-Path -LiteralPath $HomeDir) {
    if (Confirm-Remove "package home tree $HomeDir (releases, lockfile, telemetry id, install marker)") {
        ...
        if (Test-Path -LiteralPath $HomeDir) {
            ...
            Write-WarningStep "  Close any open cmux-cua processes / shells with cwd inside the tree and re-run."
        } else {
...
#    (`Programs\trycua\cmux-cua\` + `.cmux-cua\`). We always
...
# Empty cmux-cua parent (under trycua\)
...
#    in place (a user with a hand-managed cmux-cua skill dir
...
Write-Host ""
Write-Host "cmux-cua uninstalled." -ForegroundColor Green
Write-Host ""
Write-Host "Claude Code MCP registrations:" -ForegroundColor Yellow
Write-Host "  We don't auto-edit ~/.claude.json on Windows. If you registered cmux-cua"
Write-Host "  with Claude Code, remove it manually:"
Write-Host ""
Write-Host "    claude mcp remove cmux-cua"
Write-Host ""
Write-Host "  Or edit ~/.claude.json directly and delete entries whose 'command' points at"
Write-Host "  cmux-cua.exe under %LOCALAPPDATA%\Programs\Cua\cmux-cua\bin\"
Write-Host "  (or the legacy %LOCALAPPDATA%\Programs\trycua\cmux-cua\bin\ from v0.2.13 and earlier)."
Write-Host ""
Write-Host "PATH:"
