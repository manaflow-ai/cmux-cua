#!/bin/bash
# cmux-cua installer — download and install the Rust implementation.
...
#
# Flags:
#   --bin-dir <path>     install the cmux-cua wrapper to <path> instead of
...
#
# Env overrides:
#   CMUX_CUA_VERSION=0.6.8    pin a specific Rust release tag
#   CMUX_CUA_VERSION=0.6.8       legacy alias for CMUX_CUA_VERSION
#   CMUX_CUA_INSTALL_DIR=PATH same as --bin-dir
#   CMUX_CUA_BIN_DIR=PATH        legacy alias for --bin-dir
...
BIN_DIR="${CMUX_CUA_BIN_DIR:-$HOME/.local/bin}"
...
printf 'note: installing cmux-cua via the Rust implementation.\n' >&2

if [[ -n "${CMUX_CUA_BIN_DIR:-}" && -z "${CMUX_CUA_INSTALL_DIR:-}" ]]; then
    export CMUX_CUA_INSTALL_DIR="$BIN_DIR"
fi
if [[ -n "${CMUX_CUA_VERSION:-}" && -z "${CMUX_CUA_VERSION:-}" ]]; then
    export CMUX_CUA_VERSION="$CMUX_CUA_VERSION"
fi
if [[ -n "${CMUX_CUA_NO_MODIFY_PATH:-}" && -z "${CMUX_CUA_NO_MODIFY_PATH:-}" ]]; then
    export CMUX_CUA_NO_MODIFY_PATH="$CMUX_CUA_NO_MODIFY_PATH"
fi
...
if [[ -n "${BASH_SOURCE[0]:-}" && "${BASH_SOURCE[0]}" != "-" && -f "${BASH_SOURCE[0]}" ]]; then
    ...
    # Helper lives next to this script under libs/cmux-cua/scripts/.
...
if [[ -n "$LOCAL_RUST_INSTALLER" ]]; then
    ...
    exec /bin/bash -c "$RUST_INSTALLER_SCRIPT" cmux-cua-install ${FORWARDED_ARGS[@]+"${FORWARDED_ARGS[@]}"}
fi
