...
artifact_root="$repo_root/libs/cmux-cua/docs/vm-artifacts"
...
exclude_args=(
  ...
  --exclude='._*'
  --exclude=target/
  --exclude=libs/cmux-cua/rust/test-apps/
...
ensure_remote_runtime_dirs() {
  case "$remote_os" in
    windows)
      "$rsync_ssh" "$target" \
        "powershell -NoProfile -Command \"New-Item -ItemType Directory -Force '$remote_dir/libs/cmux-cua/rust/test-apps' | Out-Null\""
      ;;
    posix)
      remote_path=${remote_dir/#\~/'$HOME'}
      "$rsync_ssh" "$target" \
        "mkdir -p \"$remote_path/libs/cmux-cua/rust/test-apps\""
      ;;
  esac
}
