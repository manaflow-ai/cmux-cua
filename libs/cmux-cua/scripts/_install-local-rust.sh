#!/usr/bin/env bash
#
# cmux-cua local/debug installer (macOS + Linux). Builds from the
# current source tree and drops the resulting cmux-cua binary into the
...
#
# Rust local installer (dev-only helper for libs/cmux-cua/rust):
...
#
#   ${CMUX_CUA_HOME:-$HOME/.cmux-cua}/packages/
#       releases/<version>-local-<config>-<target>/cmux-cua
#       current/cmux-cua  -> ../releases/<active>/cmux-cua
#   ${CMUX_CUA_INSTALL_DIR:-$HOME/.local/bin}/cmux-cua
#       -> ../current/cmux-cua
#
# Legacy env vars `CMUX_CUA_HOME` / `CMUX_CUA_INSTALL_DIR` /
# `CMUX_CUA_BIN_DIR` are still accepted for backwards compat
# (rename from v0.2.16 per PR #1644; this helper was missed in the
# initial rename and ported in PR #1717).
#
# macOS layout produced:
#   /Applications/cmux Computer Use.app/Contents/MacOS/cmux-cua  (bundle replaced wholesale)
#   $HOME/.local/bin/cmux-cua -> .../cmux Computer Use.app/Contents/MacOS/cmux-cua
#
...
# libs/cmux-cua/scripts/; the Cargo workspace lives one level deeper
# under libs/cmux-cua/rust/.
REPO_ROOT="$(cd "$SCRIPT_DIR/../rust" && pwd)"

# --- Load shared daemon-cleanup helpers ---------------------------------
#
# Sibling _install-common.sh defines stop_cmux_cua_daemons +
# show_cmux_cua_daemon_survivors, mirroring CmuxCuaInstall.psm1.
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --autostart)
            ...
            ;;
        --help|-h)
            echo "${BOLD}${BLUE}cmux-cua local installer${NORMAL}"
            ...
            echo "                is attributed to com.cmuxterm.cua (not your terminal),"
            echo "                so you grant Accessibility + Screen Recording once and"
            echo "                every cmux-cua call/mcp routes through it correctly."
...
# Canonical home is `~/.cmux-cua/` (renamed from `~/.cmux-cua/`
# in v0.2.16 — PR #1644). Accept the legacy `CMUX_CUA_HOME` env var
# too so any dev scripts that still set it keep working.
HOME_DIR="${CMUX_CUA_HOME:-${CMUX_CUA_HOME:-$HOME/.cmux-cua}}"
BIN_DIR="${CMUX_CUA_INSTALL_DIR:-${CMUX_CUA_BIN_DIR:-${CMUX_CUA_INSTALL_DIR:-${CMUX_CUA_BIN_DIR:-$HOME/.local/bin}}}}"
RELEASES_DIR="$HOME_DIR/packages/releases"
CURRENT_LINK="$HOME_DIR/packages/current"

# Best-effort sweep: if a previous install left a stale `~/.cmux-cua/`
...
echo "  bin:     ${BOLD}$BIN_DIR/cmux-cua${NORMAL}"
...
echo "${BOLD}Building cmux-cua ($BUILD_CONFIG)...${NORMAL}"
cd "$REPO_ROOT"
if [ "$BUILD_CONFIG" = "release" ]; then
    cargo build --release -p cmux-cua
else
    cargo build -p cmux-cua
fi

BUILT_BINARY="$REPO_ROOT/target/$BUILD_CONFIG/cmux-cua"
...
cp "$BUILT_BINARY" "$VERSIONED_DIR/cmux-cua"
chmod +x "$VERSIONED_DIR/cmux-cua"
...
if [ "$OS" = "Darwin" ]; then
    if command -v codesign >/dev/null 2>&1; then
        codesign --force --sign - "$VERSIONED_DIR/cmux-cua" 2>/dev/null \
...
SOURCE_SKILLS="$REPO_ROOT/Skills/cmux-cua"
if [ -d "$SOURCE_SKILLS" ]; then
    STAGED_SKILLS="$VERSIONED_DIR/Skills/cmux-cua"
    rm -rf "$STAGED_SKILLS"
...
CUA_LOCAL_SIGN_CN="CmuxCua Local Signing (cmux-cua)"
...
# --- macOS: wrap the binary in cmux Computer Use.app for a stable TCC identity ---
#
# TCC keys Accessibility / Screen-Recording grants on the bundle
# identifier (com.cmuxterm.cua), not the bare executable path. A loose
...
# CmuxCuaBundle skeleton, install the bundle to /Applications, and point
# the visible bin at the binary INSIDE the bundle. Linux/Windows have no
# .app concept and keep the bare-binary symlink below.
APP_DEST="/Applications/cmux Computer Use.app"
if [ "$OS" = "Darwin" ]; then
    SKELETON="$REPO_ROOT/scripts/CmuxCuaBundle"
    if [ ! -d "$SKELETON/Contents" ]; then
        echo "${RED}Error: bundle skeleton missing at $SKELETON${NORMAL}" >&2
        exit 1
    fi
    APP_STAGE="$VERSIONED_DIR/cmux Computer Use.app"
    rm -rf "$APP_STAGE"
    mkdir -p "$APP_STAGE/Contents/MacOS"
    cp -R "$SKELETON/Contents/." "$APP_STAGE/Contents/"
    cp "$VERSIONED_DIR/cmux-cua" "$APP_STAGE/Contents/MacOS/cmux-cua"
    chmod +x "$APP_STAGE/Contents/MacOS/cmux-cua"
        ...
        if [ "$SIGN_ID" != "-" ] \
           && codesign_bounded 20 --force --deep --sign "$SIGN_ID" "$APP_STAGE" 2>/dev/null; then
            ...
            if codesign_bounded 20 --force --deep --sign - "$APP_STAGE" 2>/dev/null; then
                ...
                clean_partial_bundle_signature "$APP_STAGE"
                echo "${RED}Error: codesign of staged cmux Computer Use.app failed; live installation was not changed.${NORMAL}" >&2
                exit 1
            fi
        fi
        if ! codesign --verify --deep --strict "$APP_STAGE" 2>/dev/null; then
            echo "${RED}Error: staged cmux Computer Use.app failed signature verification; live installation was not changed.${NORMAL}" >&2
            exit 1
        fi
    fi
    ...
    if ditto "$APP_STAGE" "$APP_DEST"; then
        ...
        echo "${RED}Error: failed to install cmux Computer Use.app; restored the previous bundle.${NORMAL}" >&2
        exit 1
    fi
        ...
        case "$NEW_IDENTITY" in
            cert:*)
                if [ "$NEW_IDENTITY" != "$OLD_IDENTITY" ]; then
                    tccutil reset Accessibility com.cmuxterm.cua >/dev/null 2>&1 || true
                    tccutil reset ScreenCapture com.cmuxterm.cua >/dev/null 2>&1 || true
...
# that actually runs carries the com.cmuxterm.cua identity (TCC keys
# grants on it). On Linux/Windows point at the versioned-store binary.
mkdir -p "$BIN_DIR"
if [ "$OS" = "Darwin" ]; then
    BIN_TARGET="$APP_DEST/Contents/MacOS/cmux-cua"
else
    BIN_TARGET="$CURRENT_LINK/cmux-cua"
fi
ln -sf "$BIN_TARGET" "$BIN_DIR/cmux-cua"
echo "${GREEN}$BIN_DIR/cmux-cua -> $BIN_TARGET${NORMAL}"
echo ""

INSTALLED_BIN="$BIN_DIR/cmux-cua"
...
# `cmux-cua skills install --local` to symlink agent dirs to the
# staged copy at $VERSIONED_DIR/Skills/cmux-cua above.
echo ""
...
if [ "$INSTALL_AUTOSTART" = true ]; then
    if [ "$OS" = "Darwin" ]; then
        ...
        LEGACY_PLIST_PATH="$HOME/Library/LaunchAgents/com.cmuxterm.cmux-cua.plist"
        if [ -f "$LEGACY_PLIST_PATH" ]; then
            launchctl unload "$LEGACY_PLIST_PATH" 2>/dev/null || true
            rm -f "$LEGACY_PLIST_PATH"
        fi
        PLIST_PATH="$HOME/Library/LaunchAgents/com.cmuxterm.cmux-cua.plist"
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.cmuxterm.cmux-cua</string>
...
EOF
        ...
        # race the old one.
        LEGACY_UNIT_PATH="$HOME/.config/systemd/user/cmux-cua.service"
        if [ -f "$LEGACY_UNIT_PATH" ]; then
            systemctl --user disable --now cmux-cua.service 2>/dev/null || true
            rm -f "$LEGACY_UNIT_PATH"
        fi
        UNIT_PATH="$HOME/.config/systemd/user/cmux-cua.service"
        echo "${BOLD}Writing systemd user unit → $UNIT_PATH${NORMAL}"
        mkdir -p "$(dirname "$UNIT_PATH")"
        cat >"$UNIT_PATH" <<EOF
[Unit]
Description=cmux-cua serve daemon
...
EOF
        ...
        echo "${GREEN}Enabled.${NORMAL} Manage with systemctl --user {start|stop|status} cmux-cua."
    fi
    echo ""
fi
...
if [ "$INSTALL_AUTOSTART" != true ]; then
    ...
    if [ "$OS" = "Darwin" ]; then
        ...
        echo "  A launchd-started daemon is attributed to com.cmuxterm.cua (not your terminal),"
        echo "  so permission prompts say \"cmux CUA\" and grants stick — grant Accessibility +"
        echo "  Screen Recording once and every cmux-cua call/mcp routes through it correctly."
