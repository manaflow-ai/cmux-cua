...
#     during execution. It uses Import-CmuxCuaInstallModule (below)
...
# Best-effort kill of any running cmux-cua / cmux-cua-uia processes
# so the next `cmux-cua autostart kick` / `cmux-cua mcp` starts the
...
#      - High-IL daemons whose parent wasn't `cmux-cua-serve`).
function Stop-CmuxCuaDaemons {
    [CmdletBinding()]
    param()
    ...
    Start-Sleep -Milliseconds 200
    return @(Get-Process -Name "cmux-cua","cmux-cua-uia" -ErrorAction SilentlyContinue)
}

# Probe whether `\\.\pipe\cmux-cua` is currently accepting connections.
...
function Test-CmuxCuaPipeAlive {
    [CmdletBinding()]
    param()
    try {
...
# Stop-CmuxCuaDaemons + stale-daemon detection in one go. Returns a
...
function Stop-CmuxCuaDaemonsWithHealth {
    [CmdletBinding()]
    param()
    $survivors = Stop-CmuxCuaDaemons
    $pipeAlive = Test-CmuxCuaPipeAlive
...
function Show-CmuxCuaDaemonSurvivors {
    ...
    if ($Stale) {
        ...
        Write-Host "ERROR: cmux-cua daemon is STALE - process alive (pid: $pids) but \\.\pipe\cmux-cua" -ForegroundColor Red
        ...
        Write-Host "         schtasks /Run /TN 'cmux-cua-serve'" -ForegroundColor Yellow
        Write-Host "" -ForegroundColor Yellow
        Write-Host "       Reboot also clears it. After the kill+restart, MCP recovers automatically." -ForegroundColor Yellow
    } else {
        Write-Host "Note: $($Survivors.Count) cmux-cua process(es) still running after best-effort kill (pid: $pids)." -ForegroundColor Yellow
        ...
        Write-Host "        taskkill /IM cmux-cua.exe /F" -ForegroundColor Yellow
...
#
# Either way the caller ends up with Stop-CmuxCuaDaemons +
# Show-CmuxCuaDaemonSurvivors in scope.
#
# Pulled into the module itself (recursively used) so install.ps1's
# inline bootstrap is one short call. Callers must define
# $CmuxCuaInstallPsmUrl before invoking the fallback branch.
function Import-CmuxCuaInstallModule {
    [CmdletBinding()]
    param(
        [string]$LocalDir,
        [string]$Url
    ...
    if (-not $Url) {
        throw "Import-CmuxCuaInstallModule: no local file and no -Url to fetch from."
    }
    $body = Invoke-RestMethod -Uri $Url -UseBasicParsing
    $tmp = Join-Path $env:TEMP ("CmuxCuaInstall-" + [Guid]::NewGuid().ToString('N') + ".psm1")
...
# (High-IL) cmux-cua pids and restarts the scheduled task. Returns
...
# panel.
function Invoke-CmuxCuaStaleDaemonKill {
    [CmdletBinding()]
    param(
...
& schtasks.exe /Run /TN 'cmux-cua-serve' | Out-Null
Start-Sleep -Milliseconds 1500
try {
    `$fs = [System.IO.File]::Open('\\.\pipe\cmux-cua','Open','ReadWrite','ReadWrite')
    `$fs.Close()
    exit 0
} catch {
    exit 3
...
# `Stop-CmuxCuaDaemonsWithHealth + Show-CmuxCuaDaemonSurvivors`
# pair at the call site.
function Repair-CmuxCuaStaleDaemon {
    [CmdletBinding()]
    param(
    ...
    $result = Stop-CmuxCuaDaemonsWithHealth
    if (-not $result.Stale) {
        Show-CmuxCuaDaemonSurvivors -Survivors $result.Survivors
        return $result
    }
    # Stale: tell user what we see, offer to elevate.
    $survivorPids = @($result.Survivors | ForEach-Object { $_.Id })
    Write-Host ""
    Write-Host "Detected STALE cmux-cua daemon (pid: $($survivorPids -join ', '))." -ForegroundColor Yellow
    Write-Host "  Process is alive but \\.\pipe\cmux-cua is not accepting connections." -ForegroundColor Yellow
    ...
    if (-not $proceed) {
        Show-CmuxCuaDaemonSurvivors -Survivors $result.Survivors -Stale
        return $result
    }

    Write-Host "  Triggering UAC prompt (accept to kill the stale daemon)..." -ForegroundColor Cyan
    $ok = Invoke-CmuxCuaStaleDaemonKill -Pids $survivorPids
    if ($ok) {
        Write-Host "  Stale daemon killed; new cmux-cua-serve started; pipe is healthy." -ForegroundColor Green
        return [pscustomobject]@{
            Survivors = @()
            PipeAlive = $true
            Stale     = $false
        ...
        $rep = Stop-CmuxCuaDaemonsWithHealth
        Show-CmuxCuaDaemonSurvivors -Survivors $rep.Survivors -Stale:$rep.Stale
        return $rep
    }
}
...
Export-ModuleMember -Function `
    Stop-CmuxCuaDaemons, `
    Stop-CmuxCuaDaemonsWithHealth, `
    Test-CmuxCuaPipeAlive, `
    Show-CmuxCuaDaemonSurvivors, `
    Invoke-CmuxCuaStaleDaemonKill, `
    Repair-CmuxCuaStaleDaemon, `
    Import-CmuxCuaInstallModule
