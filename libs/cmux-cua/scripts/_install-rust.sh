#!/usr/bin/env bash
# _install-rust.sh — private helper invoked by libs/cmux-cua/scripts/install.sh
...
#
# Downloads the latest cmux-cua release tarball from GitHub Releases
# and drops the binary into ~/.local/bin (or a path given via --bin-dir /
# CMUX_CUA_INSTALL_DIR). Sudo-free.
#
# This is the cross-platform cmux-cua implementation for macOS / Linux /
# Windows via WSL or git-bash. The retired Swift implementation (macOS
# only) still ships separately under tag prefix `cmux-cua-v*`; this
# helper is hard-pinned to `cmux-cua-v*` and will never pick it up.
#
...
#
# Env overrides:
#   CMUX_CUA_VERSION=0.1.2          pin a specific release tag
#   CMUX_CUA_INSTALL_DIR=PATH       same as --bin-dir; sets the visible
#                                        binary location
#   CMUX_CUA_BIN_DIR=PATH           legacy alias for INSTALL_DIR
#   CMUX_CUA_HOME=PATH              package home for versioned installs
#                                        (default ~/.cmux-cua). Holds
#                                        packages/releases/<v>-<target>/ and
#                                        packages/current/ on Linux/Windows.
#                                        Renamed from ~/.cmux-cua in
#                                        v0.2.16 / PR #1644 — this release
#                                        installer was missed in that rename
#                                        and is reconciled here; a stale
#                                        ~/.cmux-cua is swept post-install.
#   CMUX_CUA_NO_MODIFY_PATH=1       same as --no-modify-path
#   CMUX_CUA_KEEP_VERSIONS=N        keep the N most recent per-version
...
#     packages/
#       releases/
#         0.1.3-x86_64-unknown-linux-gnu/cmux-cua   (per-version binary)
#         0.1.4-x86_64-unknown-linux-gnu/cmux-cua
#       current/cmux-cua -> ../releases/<active>/cmux-cua  (active version)
#   $CMUX_CUA_INSTALL_DIR/cmux-cua -> $HOME/packages/current/cmux-cua
#
...
# 5, override via CMUX_CUA_KEEP_VERSIONS); the dir that `current`
...
#
# Bash counterpart of install.ps1's `Import-CmuxCuaInstallModule`. On
...
TAG_PREFIX="cmux-cua-v"
# CMUX_CUA_INSTALL_DIR is the documented name; CMUX_CUA_BIN_DIR is
# the legacy alias kept for users with the old env in their shell rc.
BIN_DIR="${CMUX_CUA_INSTALL_DIR:-${CMUX_CUA_BIN_DIR:-$HOME/.local/bin}}"
# Canonical home is ~/.cmux-cua (renamed from ~/.cmux-cua in v0.2.16 /
...
# kept writing to the legacy ~/.cmux-cua, which is the root cause of the
# install collision (release wrote one home, install-local + runtime used the
# other). Reconcile the default here, keep accepting the CMUX_CUA_HOME
# override for back-compat, and sweep the stale legacy dir post-install below.
HOME_DIR="${CMUX_CUA_HOME:-$HOME/.cmux-cua}"
# Pre-v0.2.16 home this installer used to write to. Swept after the new
# install is staged so a single rooted home (~/.cmux-cua) is left behind.
LEGACY_HOME_DIR="$HOME/.cmux-cua"
NO_MODIFY_PATH="${CMUX_CUA_NO_MODIFY_PATH:-0}"
...
KEEP_VERSIONS="${CMUX_CUA_KEEP_VERSIONS:-$KEEP_VERSIONS_DEFAULT}"

# macOS-only: name and install location of the .app bundle that wraps
# the bare binary so the TCC auto-relaunch path in `cmux-cua mcp` has
# a stable bundle id (com.cmuxterm.cua) to attribute the daemon to.
# See libs/cmux-cua/rust/scripts/CmuxCuaBundle/Contents/Info.plist and
# the matching docs on `cmux-cua mcp`'s auto-relaunch behavior.
# Identical to the Swift driver's cmux Computer Use.app + com.cmuxterm.cua
...
APP_NAME="cmux Computer Use.app"
...
if ! [[ "$KEEP_VERSIONS" =~ ^[0-9]+$ ]]; then
    printf 'warning: CMUX_CUA_KEEP_VERSIONS=%s is not a non-negative integer; falling back to %d\n' \
...
acquire_install_lock() {
    ...
    while ! mkdir "$LOCK_DIR" 2>/dev/null; do
        if (( waited == 0 )); then
            log "another cmux-cua install is already in progress (lock at $LOCK_DIR); waiting..."
        fi
...
prune_old_releases() {
    ...
    if [[ "$keep" == "0" ]]; then
        log "version GC disabled (CMUX_CUA_KEEP_VERSIONS=0)"
        return 0
    fi
...
# SAME canonical home this release installer now writes to (~/.cmux-cua, see
...
# /Applications/cmux Computer Use.app with a self-signed identity recorded at
# `~/.cmux-cua/.tcc-signing-identity`.
#
...
# /Applications/cmux Computer Use.app is shared (bundle id com.cmuxterm.cua) and the
...
# LABEL  = the release-asset tarball label (matches what cd-rust-cmux-cua.yml
...
case "$OS-$ARCH_RAW" in
    ...
    *)
        ...
        err "  cmux-cua ships prebuilts for: darwin-arm64, darwin-x86_64, linux-x86_64, linux-arm64."
...
#   1. CMUX_CUA_VERSION env var (explicit pin)
#   2. CMUX_CUA_BAKED_VERSION below (set automatically by CD after
...
CMUX_CUA_BAKED_VERSION="0.7.1"
# ~~~ END_BAKED_VERSION ~~~

if [[ -n "${CMUX_CUA_VERSION:-}" ]]; then
    TAG="${TAG_PREFIX}${CMUX_CUA_VERSION#v}"
    log "using version from CMUX_CUA_VERSION: $TAG"
elif [[ -n "${CMUX_CUA_BAKED_VERSION:-}" ]]; then
    TAG="${TAG_PREFIX}${CMUX_CUA_BAKED_VERSION#v}"
    log "using baked release: $TAG"
else
    log "resolving latest $TAG_PREFIX* release via GitHub API"
    # Pinned to the exact `cmux-cua-v*` prefix so this script can never
    # accidentally pick up a Swift `cmux-cua-v*` release.
    ...
    if [[ -z "$TAG" ]]; then
        ...
        err "  (cmux-cua is a BETA-stage cross-platform port; releases may not be published yet.)"
        exit 1
    fi
...
# Tarball selection:
#
# macOS — fetch the directory tarball (cmux-cua-vN-darwin-universal.tar.gz).
#   The directory layout includes `cmux Computer Use.app/` alongside the bare
#   binary, which we need to install into /Applications so the TCC
#   auto-relaunch path in `cmux-cua mcp` can resolve
#   `com.cmuxterm.cua` via `open -n -g -a "cmux Computer Use"`. The
...
case "$LABEL" in
    darwin-*) TARBALL="cmux-cua-${VERSION}-darwin-universal.tar.gz" ;;
    *)        TARBALL="cmux-cua-${VERSION}-${LABEL}-binary.tar.gz" ;;
esac
URL="https://github.com/$REPO/releases/download/$TAG/$TARBALL"

log "downloading $URL"
if ! curl -fsSL -o "$TMP_DIR/$TARBALL" "$URL"; then
    err "download failed; try CMUX_CUA_VERSION=<version> to pin a specific release"
    exit 1
fi
...
#     cmux-cua-${VERSION}-darwin-universal/
#       ├── cmux-cua           (bare universal binary)
#       ├── cmux Computer Use.app/     (minimal bundle; copy of the same binary
#       │                         lives at Contents/MacOS/cmux-cua)
#       └── LICENSE
#   Linux bare-binary tarball expands to:
#     cmux-cua               (single file at the archive root)
case "$LABEL" in
    darwin-*)
        STAGE="cmux-cua-${VERSION}-darwin-universal"
...
# bin into the bundle so `~/.local/bin/cmux-cua` resolves into
# `/Applications/cmux Computer Use.app/Contents/MacOS/cmux-cua`. The
# `realpath` walk in `is_executable_inside_cmux_cua_app()` keys on
# that resolved path to know whether the auto-relaunch heuristic
# should fire. Same path and same bundle id as the Swift `cmux-cua`
# install (`/Applications/cmux Computer Use.app`, `com.cmuxterm.cua`), so an
...
# Linux. Reason: /Applications/cmux Computer Use.app placement is the
# anchor for both TCC attribution (cdhash + bundle id) and
# LaunchServices' `open -a CmuxCua` discovery — symlinking the
...
# `current` symlink atomically. The visible $BIN_DIR/cmux-cua
...
if [[ "$OS" == "Darwin" ]]; then
    if [[ -z "${SRC_APP:-}" || ! -d "$SRC_APP" ]]; then
        ...
        err "  This usually means the downloaded tarball is missing cmux Computer Use.app — re-run the installer or"
        err "  pin a known-good release via CMUX_CUA_VERSION=<version>."
        exit 1
    fi
fi
if [[ "$OS" == "Darwin" && -n "$SRC_APP" && -d "$SRC_APP" ]]; then
    if [[ ! -w "/Applications" ]]; then
        ...
        err "  Without the .app bundle, \`cmux-cua mcp\` from an IDE terminal will not auto-relaunch into a TCC-correct daemon."
        exit 1
    fi
    # The Rust port and the legacy Swift driver both live at
    # /Applications/cmux Computer Use.app with bundle id `com.cmuxterm.cua` —
    ...
    if [[ -e "$APP_DEST" ]]; then
        ...
        if [[ "$PREV_BUNDLE_ID" == "com.cmuxterm.cua" ]] && [[ -n "$PREV_BUNDLE_VERSION" ]]; then
            log "replacing existing cmux-cua at $APP_DEST (${PREV_BUNDLE_ID}, version ${PREV_BUNDLE_VERSION})"
            REPLACED_SWIFT=1
    ...
    #   releases/<version>-<target>/cmux-cua   (this install)
    #   releases/<older>-<target>/cmux-cua     (kept for rollback)
    #   current/cmux-cua -> ../releases/<active>-<target>/cmux-cua
    #
    ...
    # `current/cmux-cua` keeps using the open file handle — Unix
    ...
    # + atomic rename instead so a concurrent `cmux-cua` lookup
...
# --- Sweep the legacy ~/.cmux-cua home -----------------------------
#
# This release installer used to default HOME_DIR to ~/.cmux-cua (the
# pre-v0.2.16 name). Now that it writes to ~/.cmux-cua like install-local
# and the runtime, a prior RELEASE install can have left a stale
# ~/.cmux-cua behind — the source of the two-homes collision this PR
...
# skipped when the user pinned CMUX_CUA_HOME to the legacy path on
...
# --- Stop any pre-swap cmux-cua daemons -------------------------------
#
# Mirror of install.ps1's `Stop-CmuxCuaDaemons` call sequence. The
# new binary is now in place under packages/current/ (Linux) or
# /Applications/cmux Computer Use.app (macOS) — kill any in-memory daemon
# that was holding the OLD binary so the next `cmux-cua` invocation
...
# ~/.claude/skills/, ~/.agents/skills/, etc. Run `cmux-cua skills
...
#
# Anonymous adoption signal — sends `cmux_cua_install` to PostHog
# exactly once per install (guarded by ~/.cmux-cua/.installation_recorded
# on the binary side). The Rust port keeps its install signal independent
# of the Swift `cmux-cua` install (separate marker dir + separate env var)
# so users can opt out of one without affecting the other.
#
# Bypasses the CMUX_CUA_TELEMETRY_ENABLED check by design — see
...
if [[ "$NO_MODIFY_PATH" != "1" ]] && [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
    ...
    if [[ -n "$SHELL_RC" ]]; then
        {
            printf '\n# Added by cmux-cua installer — see https://github.com/manaflow-ai/cmux-cua\n'
...
echo ""
echo "cmux-cua $VERSION installed."
echo ""

if [[ "${REPLACED_SWIFT:-0}" == "1" ]]; then
    echo "Upgraded the cmux-cua bundle that was previously at $APP_DEST."
    echo "TCC grants (Accessibility, Screen Recording) are keyed on the bundle id"
    echo "(com.cmuxterm.cua) — which is preserved — so they transfer to the new"
...
if curl -fsSL "$HINTS_URL" -o "$HINTS_TXT" 2>/dev/null && [ -s "$HINTS_TXT" ]; then
    ...
    echo "Docs: https://github.com/manaflow-ai/cmux-cua/tree/main/libs/cmux-cua/rust"
fi
...
case "$(uname -s)" in
    Darwin)
        ...
        echo "  open -n -g -a 'cmux Computer Use' --args serve"
