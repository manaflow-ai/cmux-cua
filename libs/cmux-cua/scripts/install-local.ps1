# cmux-cua local installer (Windows). Builds release-mode from the
# current source tree and drops the resulting cmux-cua.exe into the same
...
#   -AutoStart      register the cmux-cua-serve Scheduled Task at logon
...
# Use `cargo build -p cmux-cua` directly + invoke target\debug\cmux-cua.exe
...
#     0.0.0-local-release-<target>\cmux-cua.exe
#
...
if ($env:CMUX_CUA_INSTALL_DIR) {
    ...
    # (trycua\cmux-cua → Cua\cmux-cua). See PR #1644.
    $VisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\Cua\cmux-cua\bin"
}
if ($env:CMUX_CUA_HOME) {
    $PackageHome = $env:CMUX_CUA_HOME
} else {
    $PackageHome = Join-Path $env:USERPROFILE ".cmux-cua"
}
...
# Stop-CmuxCuaDaemons + Show-CmuxCuaDaemonSurvivors are defined in
...
Write-Step "cmux-cua local installer (Windows)"
...
Write-Step "cargo build --release -p cmux-cua"
...
# being executed (typical: `cmux-cua autostart kick` spawned a
...
if (Test-Path -LiteralPath $DestBinary) {
    ...
    try {
        ...
        Write-Host "      Most likely a running cmux-cua daemon is holding it." -ForegroundColor Yellow
        Write-Host "      Stop it first (e.g. ``schtasks /End /TN cmux-cua-serve`` then re-run)." -ForegroundColor Yellow
    }
    ...
    Write-Step "killing previous cmux-cua processes (best-effort; High-IL needs admin)"
    # Repair- variant does Stop-CmuxCuaDaemonsWithHealth + stale
...
# install.ps1 does from a release zip — copies Skills/cmux-cua/ from
# the repo into the versioned dir so the `current` junction below
# transparently exposes it to agents.
$SourceSkills = Join-Path $RepoRoot "Skills\cmux-cua"
if (Test-Path -LiteralPath $SourceSkills) {
    $StagedSkills = Join-Path $VersionedDir "Skills\cmux-cua"
...
if (-not $NoPathUpdate) {
    ...
    if (-not $alreadyOnPath) {
        ...
        # commands in THIS shell see cmux-cua immediately. install.ps1
...
if ($AutoStart) {
    Write-Step "registering Scheduled Task 'cmux-cua-serve'"
    try {
        Register-CmuxCuaAutostart -InstalledBinary (Join-Path $VisibleBinDir $BinaryName)
        Write-Host "  Registered. cmux-cua serve auto-starts at every interactive logon." -ForegroundColor Green
    }
    catch {
        Write-Host "  Failed to register: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    # User didn't pass -AutoStart, but if a `cmux-cua-serve` task is
    # ALREADY registered (from a previous `install.ps1 -AutoStart` or
    # `cmux-cua autostart enable`), re-register it pointing at this
    # fresh binary. Otherwise the user ends up with a task whose
    # <Command> path is the OLD release-install dir, running the OLD
    # binary - even though `cmux-cua` on PATH now resolves to the
    ...
    if ($hasTask) {
        Write-Step "found existing 'cmux-cua-serve' task - re-registering against fresh binary"
        try {
            Register-CmuxCuaAutostart -InstalledBinary (Join-Path $VisibleBinDir $BinaryName)
...
if ($AutoStart) {
    ...
    Write-Host "  cmux-cua autostart enable    (register Scheduled Task at RunLevel=Highest)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart kick      (start now without re-logging)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart status    (inspect)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart disable   (remove)" -ForegroundColor Cyan
    Write-Host ""
