# cmux-cua installer (Windows) — download the latest cmux-cua
...
#
# Pin a version:
#   $env:CMUX_CUA_VERSION = "0.2.0"
#   irm https://cua.ai/driver/install.ps1 | iex
#
# Layout on disk (three tiers, two directory junctions):
#
#   <visibleBinDir>            [directory junction → currentDir]
#     = %LOCALAPPDATA%\Programs\Cua\cmux-cua\bin
#   <currentDir>               [directory junction → release dir]
#     = %USERPROFILE%\.cmux-cua\packages\current
#   <release dir>              [real directory, immutable per version]
#     = %USERPROFILE%\.cmux-cua\packages\releases\<version>-<target>
#         cmux-cua.exe
#
# Path layout renamed v0.2.14: `Programs\trycua\cmux-cua\` →
# `Programs\Cua\cmux-cua\` and `.cmux-cua\` → `.cmux-cua\`. The
...
#
# Env overrides:
#   $env:CMUX_CUA_VERSION       pin a specific release (e.g. "0.2.0")
#   $env:CMUX_CUA_INSTALL_DIR   override the visible PATH-entry dir
#                                    (default %LOCALAPPDATA%\Programs\Cua\cmux-cua\bin)
#   $env:CMUX_CUA_HOME          override the package home
#                                    (default %USERPROFILE%\.cmux-cua)
#   $env:CMUX_CUA_KEEP_VERSIONS keep the N most recent per-version
...
#               Overridden by $env:CMUX_CUA_VERSION when set.
#   -AutoStart  register a Scheduled Task that runs `cmux-cua serve` at
...
#               `cmux-cua --version` works in new shells without manual
...
#
# Windows installer for the cross-platform cmux-cua Rust implementation.

[CmdletBinding()]
param(
    [string]$Release = "latest",
    # Default-on: cmux-cua-serve is what makes the agent flow work
    # across logon / reboot. Without the scheduled task the user has
    # to remember to run `cmux-cua autostart kick` every time, and
...
$Repo       = "trycua/cua"
$TagPrefix  = "cmux-cua-v"
$BinaryName = "cmux-cua.exe"

# Baked-version constant — kept in lock-step with the latest published
# cmux-cua-v* release tag by the CD workflow's bake-version step
# (see .github/workflows/cd-rust-cmux-cua.yml). The sentinel-block
...
#
# Precedence at resolve time: $env:CMUX_CUA_VERSION > -Release arg >
...
$Script:CmuxCuaBakedVersion = "0.7.1"
# ~~~ END_BAKED_VERSION ~~~

# ---------- Path resolution ------------------------------------------------

if ($env:CMUX_CUA_INSTALL_DIR) {
    $VisibleBinDir = $env:CMUX_CUA_INSTALL_DIR
} else {
    # Path layout renamed v0.2.14: `Programs\trycua\cmux-cua\` →
    # `Programs\Cua\cmux-cua\`. The Rust port IS the canonical Windows
    ...
    $VisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\Cua\cmux-cua\bin"
}
...
$LegacyVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\trycua\cmux-cua\bin"
$LegacyVendorDir     = Join-Path $env:LOCALAPPDATA "Programs\trycua"

if ($env:CMUX_CUA_HOME) {
    $HomeDir = $env:CMUX_CUA_HOME
} else {
    # Same rename: `.cmux-cua/` → `.cmux-cua/`. The `-rs` suffix
    # was the Rust-port-vs-Swift-driver disambiguator while the Swift one
    # still existed for Windows; it doesn't anymore.
    $HomeDir = Join-Path $env:USERPROFILE ".cmux-cua"
}

$LegacyHomeDir = Join-Path $env:USERPROFILE ".cmux-cua"
...
function Resolve-KeepVersions {
    $raw = $env:CMUX_CUA_KEEP_VERSIONS
    if (-not $raw) { return $Script:KeepVersionsDefault }
    $n = 0
    if ([int]::TryParse($raw, [ref]$n) -and $n -ge 0) { return $n }
    Write-WarningStep "CMUX_CUA_KEEP_VERSIONS=$raw is not a non-negative integer; falling back to $($Script:KeepVersionsDefault)"
...
function Get-TargetTriple {
    ...
    switch ($arch) {
        ...
        default {
            ...
            Write-ErrorStep "  cmux-cua ships prebuilts for: x86_64 (AMD64) and arm64 (ARM64)."
            Write-ErrorStep "  Please file an issue at https://github.com/manaflow-ai/cmux-cua/issues with the output of"
...
function Add-JunctionSupportType {
    if ("CmuxCuaInstaller.Junction" -as [type]) { return }
...
function Set-JunctionTarget([string]$linkPath, [string]$targetPath) {
    ...
    [CmuxCuaInstaller.Junction]::SetTarget($linkPath, $targetPath)
}

function Get-JunctionTarget([string]$linkPath) {
    Add-JunctionSupportType
    return [CmuxCuaInstaller.Junction]::GetTarget($linkPath)
}
...
# Thin wrapper that delegates to `cmux-cua autostart enable`. The binary
...
# Stop-CmuxCuaDaemons + Show-CmuxCuaDaemonSurvivors live in the
...
#     file. See Import-CmuxCuaInstallModule below (defined inline so
...
function Import-CmuxCuaInstallModuleBootstrap {
    [CmdletBinding()]
    param(
        [string]$LocalDir,
    ...
    $tmp = Join-Path (Get-CmuxCuaTempDir) ("CmuxCuaInstall-" + [Guid]::NewGuid().ToString('N') + ".psm1")
...
Import-CmuxCuaInstallModuleBootstrap `
    -LocalDir $PSScriptRoot `
    -Url "https://cua.ai/driver/_install-common.psm1"

function Register-CmuxCuaAutostart {
    ...
    # at the user's elevated/admin token. This is what lets cmux-cua drive
    ...
    if (Test-IsElevated) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "cmux-cua autostart enable failed (exit $LASTEXITCODE)"
        }
        return
    }
    ...
    $elevCmd = "& `"$InstalledBinary`" autostart enable; `$ec = `$LASTEXITCODE; if (`$ec -ne 0) { Read-Host 'cmux-cua autostart enable failed; press Enter to close' }; exit `$ec"
    try {
        $proc = Start-Process -FilePath "powershell.exe" `
            -ArgumentList "-NoProfile","-ExecutionPolicy","Bypass","-Command",$elevCmd `
            -Verb RunAs -Wait -PassThru -ErrorAction Stop
        if ($proc.ExitCode -ne 0) {
            throw "cmux-cua autostart enable failed in elevated session (exit $($proc.ExitCode))"
        }
    } catch {
...
function Acquire-InstallLock {
    ...
    while ($true) {
        try {
            ...
            if (-not $announced) {
                Write-Step "another cmux-cua install is already in progress (lock at $($Script:LockFilePath)); waiting..."
                $announced = $true
            }
    ...
    # $env:USERPROFILE\.cmux-cua\install.lock).
    try {
...
function Invoke-OldReleasesGc {
    ...
    if ($keep -eq 0) {
        Write-Step "version GC disabled (`$env:CMUX_CUA_KEEP_VERSIONS=0)"
        return
    }
...
function Resolve-Version {
    if ($env:CMUX_CUA_VERSION) {
        $v = $env:CMUX_CUA_VERSION -replace '^v', ''
        Write-Step "using version from `$env:CMUX_CUA_VERSION: $v"
        return $v
    }
    ...
    if ($Script:CmuxCuaBakedVersion) {
        $v = $Script:CmuxCuaBakedVersion -replace '^v', ''
    ...
    # include any cmux-cua-v* tag — the repo also ships Swift
    # cmux-cua-v* releases plus other unrelated tags, which can push
...
function Get-ReleaseAsset([string]$version, [string]$archLabel, [string]$destDir) {
    $zipName = "cmux-cua-$version-$archLabel.zip"
    ...
    try {
        ...
        Write-ErrorStep "  Try pinning a known-good version via `$env:CMUX_CUA_VERSION = '<x.y.z>'`."
        exit 1
    }
    ...
    # cmux-cua-<v>-<arch>\cmux-cua.exe (+ LICENSE).
    $stage = "cmux-cua-$version-$archLabel"
...
Write-Step "cmux-cua installer (Windows)"
...
function Remove-LegacyInstall {
    ...
    # CMUX_CUA_INSTALL_DIR / CMUX_CUA_HOME aren't surprised by us
    ...
    if ($env:CMUX_CUA_INSTALL_DIR -or $env:CMUX_CUA_HOME) {
        return
    }
    ...
    Write-Step "detected legacy install layout (v0.2.13 or earlier); migrating to Cua\cmux-cua"
    ...
    #       with "Access to the path 'cmux-cua.exe' is denied" because
    #       the legacy binary is held open by an unkillable elevated
    #       process. Discovered during the cuademo v0.2.13 → v0.2.14
    #       migration dogfood.
    #    b. taskkill /F /IM as a backstop for any cmux-cua process that
    #       wasn't task-attached (manual `cmux-cua serve`, legacy uia
    ...
    try {
        ...
        & schtasks.exe /End /TN "cmux-cua-serve" 2>$null | Out-Null
        Start-Sleep -Milliseconds 250
        # Force-kill via taskkill — handles High-IL processes that
        # Stop-Process can't touch from a Medium-IL caller.
        & taskkill.exe /F /IM "cmux-cua.exe" /T 2>$null | Out-Null
        & taskkill.exe /F /IM "cmux-cua-uia.exe" /T 2>$null | Out-Null
    } finally {
        $ErrorActionPreference = $prevEAP
    }
    $procs = Get-Process -Name "cmux-cua","cmux-cua-uia" -ErrorAction SilentlyContinue
    if ($procs) {
        foreach ($p in $procs) {
    ...
    $ErrorActionPreference = 'Continue'
    try {
        & schtasks.exe /Delete /TN "cmux-cua-serve" /F 2>$null | Out-Null
    } finally {
        $ErrorActionPreference = $prevEAP
    }
    ...
    # Remove the parent `cmux-cua` dir (now empty) and the vendor
...
if (-not $skipDownload) {
    $tmpRoot = Join-Path (Get-CmuxCuaTempDir) ("cmux-cua-install-" + [Guid]::NewGuid().ToString("N"))
    ...
    try {
        ...
        # Optional sibling: the uiAccess'd worker (cmux-cua-uia.exe). Started
        # shipping with cmux-cua-v0.2.8; absent in earlier releases. Copy
        # it when present so `cmux-cua autostart enable` can register the
        # second ShellExecute-based scheduled task. See #1602.
        $uiaStage = Join-Path $stageDir 'cmux-cua-uia.exe'
        if (Test-Path -LiteralPath $uiaStage) {
            Copy-Item -LiteralPath $uiaStage -Destination (Join-Path $versionedDir 'cmux-cua-uia.exe') -Force
            Write-Step "installed $versionedDir\cmux-cua-uia.exe (uiAccess worker)"
        }
    }
    finally {
...
#
# Same shape as the Linux install.sh path: invoke `cmux-cua telemetry
# install-event` once per install. The binary itself guards against
# double-counting via ~\.cmux-cua\.installation_recorded.
...
#
# We append $VisibleBinDir to the User-scope PATH so `cmux-cua` resolves
...
function Add-UserPathEntry([string]$dir) {
    ...
    # Also update the CURRENT process's $env:Path so `cmux-cua` resolves
...
Write-Host ""
Write-Host "cmux-cua $version installed."
Write-Host ""
$onPath = Test-OnUserPath $VisibleBinDir
if ($onPath) {
    Write-Host "$VisibleBinDir is on your user PATH -- cmux-cua should resolve in any new shell."
    Write-Host ""
}
elseif ($NoPathUpdate) {
    ...
    try {
        ...
        Write-Host '  cmux-cua resolves immediately in THIS shell and in any new shell.'
...
# Kill any cmux-cua / cmux-cua-uia process still running off the
# OLD binary, so the next time the daemon is invoked (autostart kick,
# manual `cmux-cua mcp`, MCP client startup) it picks up the freshly-
...
# Medium-IL kill and get reported via Show-CmuxCuaDaemonSurvivors.
Write-Host ""
Write-Host "Stopping any previous cmux-cua processes (best-effort; High-IL needs admin)..." -ForegroundColor Cyan
...
$null = Repair-CmuxCuaStaleDaemon

if ($AutoStart) {
    Write-Host ""
    Write-Host "Registering auto-start (cmux-cua autostart enable)..." -ForegroundColor Cyan
    try {
        Register-CmuxCuaAutostart -InstalledBinary $installedBinary
        Write-Host "  cmux-cua serve will auto-start at every interactive logon (RunLevel=Highest)." -ForegroundColor Green
    }
    catch {
        Write-Host "  Failed: $($_.Exception.Message)" -ForegroundColor Red
        Write-Host '  Install otherwise succeeded; from an elevated shell run: cmux-cua autostart enable'
        Write-Host "  In THIS shell (if already elevated), use: $installedBinary autostart enable"
        Write-Host ""
    }
} else {
    # No -AutoStart, but if a `cmux-cua-serve` task is already
    ...
    $ErrorActionPreference = 'Continue'
    try {
        & schtasks.exe /Query /TN "cmux-cua-serve" 2>$null | Out-Null
    ...
    if ($hasTask) {
        Write-Host ""
        Write-Host "Existing 'cmux-cua-serve' autostart task detected - re-registering against the fresh binary..." -ForegroundColor Cyan
        try {
            Register-CmuxCuaAutostart -InstalledBinary $installedBinary
            ...
            Write-Host "  The existing task still points at the previous binary. Run 'cmux-cua autostart enable' from an elevated shell to update."
        }
    }
}
...
try {
    ...
    Write-Host "Docs: https://github.com/manaflow-ai/cmux-cua/tree/main/libs/cmux-cua/rust"
}

# Windows-specific autostart hint (kept inline; OS-natural location).
Write-Host ""
if ($AutoStart) {
    Write-Host "Auto-start: 'cmux-cua-serve' is registered at RunLevel=Highest." -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart status    (inspect)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart disable   (remove)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart kick      (start now without re-logging)" -ForegroundColor Cyan
} else {
    Write-Host "Auto-start at logon (NOT enabled - re-run without -NoAutoStart to register, or:):" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart enable    (Scheduled Task at RunLevel=Highest)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart kick      (start now without re-logging)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart status    (inspect)" -ForegroundColor Cyan
    Write-Host "  cmux-cua autostart disable   (remove)" -ForegroundColor Cyan
}
