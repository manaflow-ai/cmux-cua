#!/usr/bin/env bash
# cmux-cua uninstaller (Rust implementation only). Mirrors uninstall.ps1 on
...
#   - ~/.local/bin/cmux-cua symlink (+ legacy /usr/local/bin/cmux-cua)
#   - /Applications/cmux Computer Use.app bundle
#   - ~/.cmux-cua/ (telemetry id + install marker)
#   - ~/Library/Application Support/cmux CUA/ (config.json)
#   - ~/Library/Caches/cmux-cua/ (daemon/cache state)
#   - Skill symlinks under ~/.claude/skills/cmux-cua, ~/.agents/skills/
#     cmux-cua, ~/.openclaw/skills/cmux-cua, ~/.config/opencode/
#     skills/cmux-cua (only when they point at our app bundle)
#   - Claude MCP registrations in ~/.claude.json (cmux-cua / cua-computer-use)
#
# Rust uninstall removes:
#   Linux:
#     - ~/.local/bin/cmux-cua symlink (only when it resolves to a
#       cmux-cua path — a Swift-driver symlink is left in place)
#     - ~/.cmux-cua/ (current package home) + legacy ~/.cmux-cua/
#       (telemetry id, config.json, versioned releases, current symlink)
#     - ~/.config/systemd/user/cmux-cua.service (if --autostart
#       was used via install-local.sh — stop + disable + remove)
#     - Skill symlinks under ~/.claude/skills/cmux-cua(-rs), ~/.agents/
#       skills/…, ~/.openclaw/skills/…, ~/.config/opencode/skills/…
#   macOS:
#     - /Applications/cmux Computer Use.app bundle (+ legacy CmuxCua.app)
#     - ~/.local/bin/cmux-cua symlink (only when it resolves into
#       /Applications/cmux Computer Use.app)
#     - ~/.cmux-cua/ (current package home) + legacy ~/.cmux-cua/
#     - ~/Library/LaunchAgents/com.cmuxterm.cmux-cua.plist (if
#       --autostart was used via install-local.sh — unload + remove)
#     - Skill symlinks under ~/.claude/skills/cmux-cua(-rs), etc.
#
# Shared-path safety: /Applications/cmux Computer Use.app + its ~/.local/bin
# symlink use the same bundle id (com.cmuxterm.cua) as the Swift driver,
# so they're only removed when an unambiguous Rust marker is on disk
# (~/.cmux-cua/packages/, legacy ~/.cmux-cua/, CmuxCua.app,
...
#   CMUX_CUA_HOME    Rust package home to remove (default ~/.cmux-cua)
set -euo pipefail
...
if [[ "$USE_RUST_BACKEND" == "1" ]]; then
    if [[ "$OS" != "Darwin" ]]; then
        printf 'note: detected non-macOS host (%s); uninstalling cmux-cua via the Rust implementation.\n' "$OS" >&2
    else
        printf 'note: uninstalling cmux-cua via the Rust implementation.\n' >&2
    fi
fi
...
# in a clean promptable state. The bundle id com.cmuxterm.cua is shared with
...
# com.cmuxterm.cua. macOS-only; no-op elsewhere.
maybe_reset_tcc() {
    ...
    log "revoking TCC grants for com.cmuxterm.cua"
    log "  note: com.cmuxterm.cua is shared with the retired Swift driver;"
    log "  this clears grants for both. Pass --keep-tcc to preserve them."
    for SVC in Accessibility ScreenCapture AppleEvents; do
        if tccutil reset "$SVC" com.cmuxterm.cua >/dev/null 2>&1; then
            log "  reset $SVC"
        else
...
# ----------------------------------------------------------------------
if [[ "$USE_RUST_BACKEND" == "1" ]]; then
    USER_BIN_LINK="$HOME/.local/bin/cmux-cua"
    # Canonical bundle path (post-rename — shares bundle id
    # `com.cmuxterm.cua` with the Swift driver). The Rust install
    # replaces Swift here; both uninstallers target this path.
    APP_BUNDLE="/Applications/cmux Computer Use.app"
    # Legacy bundle path from earlier Rust releases that coexisted with
    # Swift under a separate name. Cleaned up if found.
    LEGACY_APP_BUNDLE="/Applications/CmuxCua.app"
    # Canonical package home is ~/.cmux-cua (renamed from ~/.cmux-cua
    ...
    HOME_DIR="${CMUX_CUA_HOME:-${CMUX_CUA_HOME:-$HOME/.cmux-cua}}"
    ...
    LAUNCHAGENT_PLIST="$HOME/Library/LaunchAgents/com.cmuxterm.cmux-cua.plist"
    SYSTEMD_USER_UNIT="$HOME/.config/systemd/user/cmux-cua.service"
    SKILL_PACK_NAME="cmux-cua"
    ...
    LEGACY_SKILL_PACK_NAME="cmux-cua"

    # Rust-install marker. The Rust bundle path `/Applications/cmux Computer Use.app`
    # is shared with the Swift driver (same bundle id `com.cmuxterm.cua`),
    ...
    #   - ~/.cmux-cua/packages/ exists (Rust install-local / updater store)
    #   - ~/.cmux-cua/ exists (legacy Rust state dir, pre-rename)
    #   - /Applications/CmuxCua.app exists (legacy bundle, pre-rename)
    ...
    # --- CLI symlink ---
    # Only remove ~/.local/bin/cmux-cua when it resolves into a
    # cmux-cua install. Pre-rename installs at
    # /Applications/CmuxCua.app are unambiguously Rust and always
    # removed. Post-rename, the Rust install lives at
    # /Applications/cmux Computer Use.app — the SAME path the Swift driver
    # uses — so we only remove that link when $RUST_INSTALL_PRESENT.
    if [[ -L "$USER_BIN_LINK" ]]; then
        RESOLVED="$(resolve_link "$USER_BIN_LINK")"
        case "$RESOLVED" in
            *"CmuxCua.app"*|*"$HOME_DIR"*|*".cmux-cua"*)
            ...
            *"/Applications/cmux Computer Use.app"*)
                ...
                ;;
            *)
                log "$USER_BIN_LINK resolves to $RESOLVED (not a cmux-cua path; skipping)"
                ;;
        esac
    ...
    # ~/.config/systemd/user/cmux-cua.service. Stop + disable +
    ...
    if [[ "$OS" == "Linux" && -f "$SYSTEMD_USER_UNIT" ]]; then
        if command -v systemctl >/dev/null 2>&1; then
            systemctl --user stop    cmux-cua.service 2>/dev/null || true
            systemctl --user disable cmux-cua.service 2>/dev/null || true
            log "stopped + disabled systemd --user unit cmux-cua.service"
        fi
    ...
    # ~/Library/LaunchAgents/com.cmuxterm.cmux-cua.plist. Unload (so
    ...
    # Legacy /Applications/CmuxCua.app is unambiguously Rust and
    # always removed when present. /Applications/cmux Computer Use.app is the
    # current canonical Rust path BUT also where the Swift driver
    # lives (same bundle id `com.cmuxterm.cua`), so we only remove
    ...
    if [[ "$OS" == "Darwin" ]]; then
        ...
        if [[ -d "$APP_BUNDLE" ]]; then
            if [[ "$RUST_INSTALL_PRESENT" == "1" ]]; then
                ...
                log "$APP_BUNDLE exists but no Rust marker on disk (~/.cmux-cua/packages/, ~/.cmux-cua/, CmuxCua.app, LaunchAgent, systemd unit); leaving it (looks like a Swift-only install)"
            fi
        else
    ...
    # --- Package home ---
    # Everything under $HOME_DIR (default ~/.cmux-cua): telemetry id,
    ...
    # a Swift-only Mac's ~/.cmux-cua/config.json on a mistaken run.
    if [[ -d "$HOME_DIR" ]]; then
        if [[ "$RUST_INSTALL_PRESENT" == "1" ]]; then
            rm -rf "$HOME_DIR"
    ...
    if [[ "$OS" == "Darwin" && "$RUST_INSTALL_PRESENT" == "1" ]]; then
        for SWIFT_DATA_DIR in \
            "$HOME/Library/Application Support/cmux CUA" \
            "$HOME/Library/Caches/cmux-cua"; do
    ...
    # We don't check the target here because `cmux-cua skills install`
    # writes platform-dependent targets (the local copy under $HOME_DIR/
    # skills/cmux-cua/). The [[ -L ]] check is the load-bearing
    # safety bar.
    if [[ "$RUST_INSTALL_PRESENT" == "1" ]]; then
        for SKILL_LINK in \
    ...
    # Same scrub shape as the Swift branch, keyed on the cmux-cua
...
def invokes_cmux_cua(server):
    ...
    # explicit ".cmux-cua" segment. Plain "cmux-cua" alone is
    # ambiguous (the Swift binary uses the same filename). The shared
    # /Applications/cmux Computer Use.app path is ALSO ambiguous (Rust took
    ...
    if home_dir and home_dir in joined:
        return True
    if "CmuxCua.app" in joined or ".cmux-cua" in joined or "cmux-cua" in joined:
        return True
    if rust_install_present and "/Applications/cmux Computer Use.app" in joined:
        return True
    return False

def should_remove(name, server):
    return name in {"cmux-cua"} or invokes_cmux_cua(server)
...
backup = f"{path}.bak-cmux-cua-uninstall-{int(time.time())}"
...
PY
        ...
        if [[ -n "$PY_OUTPUT" ]]; then
            ...
            log "no Claude MCP registrations for cmux-cua found in $CLAUDE_JSON"
        fi
    else
...
Reset them explicitly if you want a clean re-install flow:
  ...
  tccutil reset Accessibility com.cmuxterm.cua
  tccutil reset ScreenCapture com.cmuxterm.cua
FINALUNMSG
        fi
    else
        cat << 'FINALUNMSG'

cmux-cua uninstalled.
FINALUNMSG
    fi
    exit 0
fi
...
USER_BIN_LINK="$HOME/.local/bin/cmux-cua"
SYSTEM_BIN_LINK="/usr/local/bin/cmux-cua"
APP_BUNDLE="/Applications/cmux Computer Use.app"
USER_DATA="$HOME/.cmux-cua"
CONFIG_DIR="$HOME/Library/Application Support/cmux CUA"
CACHE_DIR="$HOME/Library/Caches/cmux-cua"
# Legacy — remove if present from older installs.
LEGACY_UPDATE_SCRIPT="/usr/local/bin/cmux-cua-update"
LEGACY_UPDATER_PLIST="$HOME/Library/LaunchAgents/com.trycua.cmux_cua_updater.plist"
...
SKILL_TARGET_EXPECTED="$APP_BUNDLE/Contents/Resources/Skills/cmux-cua"
for SKILL_LINK in \
    "$HOME/.claude/skills/cmux-cua" \
    "$HOME/.agents/skills/cmux-cua" \
    "$HOME/.openclaw/skills/cmux-cua" \
    "$HOME/.config/opencode/skills/cmux-cua" \
    "$HOME/.gemini/skills/cmux-cua" \
    "$HOME/.hermes/skills/cmux-cua"; do
...
# registrations explicitly named cmux-cua or whose command points at
# a cmux-cua binary, so unrelated servers named "computer-use" are
# left alone.
CLAUDE_JSON="$HOME/.claude.json"
if [[ -f "$CLAUDE_JSON" ]] && command -v python3 >/dev/null 2>&1; then
    PY_OUTPUT="$(
...
def invokes_cmux_cua(server):
    ...
    parts.extend(text_parts(server.get("args")))
    joined = " ".join(parts)
    return "cmux-cua" in joined or "cmux Computer Use.app" in joined

def should_remove(name, server):
    return name in {"cmux-cua", "cua-computer-use"} or invokes_cmux_cua(server)
...
if command -v claude >/dev/null 2>&1; then
    for SERVER in cmux-cua cua-computer-use; do
        for SCOPE in local project user; do
            if claude mcp remove "$SERVER" -s "$SCOPE" >/dev/null 2>&1; then
                log "removed Claude MCP server $SERVER from $SCOPE scope"
            fi
