...
# be granted from CI. See ../../Skills/cmux-cua/EMBEDDING.md for the pass
# criteria.
#
...
echo "     process; it then spawns cmux-cua directly as its child.)"
# `open` does not forward shell env; route a custom driver path through
# launchd so the app can see it.
if [ -n "${CMUX_CUA_PATH:-}" ]; then
    launchctl setenv CMUX_CUA_PATH "$CMUX_CUA_PATH"
    trap 'launchctl unsetenv CMUX_CUA_PATH' EXIT
fi
rm -f "$LOG"
open "$APP"
...
echo "    Security lists ONLY ExampleAgentHarness (no CmuxCua)."
