...
fn default_store_path() -> PathBuf {
    ...
    home.join("Library/Application Support/CmuxCua/app-approvals.json")
}

fn persistent_approval_allowed_by_policy() -> bool {
    !matches!(
        std::env::var("CMUX_CUA_CODEX_ALLOW_PERSISTENT_APPROVAL")
            .ok()
            .as_deref()
            .map(str::trim)
...
pub fn register_session_cleanup() {
    ...
    REGISTER.call_once(|| {
        cmux_cua_core::session::register_session_end_hook(|session_id| {
