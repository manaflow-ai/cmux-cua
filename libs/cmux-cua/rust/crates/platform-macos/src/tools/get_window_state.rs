...
impl Tool for GetWindowStateTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let mut structured = serde_json::json!({
            ...
            "snapshot_id": cmux_cua_core::element_token::token_for(snapshot_id, 0)
                .trim_end_matches(":0")
                .to_string(),
    ...
    nodes
        ...
        .filter_map(|node| {
            ...
            if let Some(idx) = node.element_index {
                ...
                // one in the per-pid LRU). See cmux-cua-core's
                // `element_token` module.
                entry["element_token"] = serde_json::Value::String(
                    cmux_cua_core::element_token::token_for(snapshot_id, idx),
                );
            }
...
mod tests {
    ...
    fn build_elements_array_with_token_emits_element_token_per_row() {
        let reg = cmux_cua_core::element_token::global();
