...
//! `cmux_cua_core::health_report`. This file owns the platform
//! probes only.

use async_trait::async_trait;
use cmux_cua_core::health_report::{
...
/// reports this id.
pub const CANONICAL_BUNDLE_ID: &str = "com.cmuxterm.cua";
...
pub(crate) fn check_binary_version() -> CheckEntry {
    ...
    CheckEntry::pass(
        NAME_BINARY_VERSION,
        format!("cmux-cua {}", env!("CARGO_PKG_VERSION")),
    )
}
...
pub(crate) fn external_permission_flow_enabled() -> bool {
    let from_environment = std::env::var("CMUX_CUA_EXTERNAL_PERMISSION_FLOW")
        .map(|value| {
            matches!(
    ...
    let (message, hint) = if _external_permission_flow {
        match bid.as_deref() {
            None | Some("") => (
                ...
                "Restart the embedding host so it can launch its bundled Computer Use helper. Do not run the standalone cmux-cua permission flow."
                    .to_owned(),
            ),
            Some(other) => (
                format!("Bundle is {other}, but the executable is not inside its helper app."),
                "Restart the embedding host's Computer Use runtime so permission status comes from its own LaunchServices helper. Do not run `cmux-cua permissions grant`."
                    .to_owned(),
            ),
        }
    } else {
        match bid.as_deref() {
            None | Some("") => (
                "Process has no CFBundleIdentifier.".to_owned(),
                format!(
                    "Run the binary inside cmux Computer Use.app so TCC grants attribute correctly. \
                 Start the daemon with `open -n -g -a 'cmux Computer Use' --args serve` and \
                 connect via `cmux-cua mcp`."
                ),
            ),
            Some(other) => (
                format!("Bundle is {other}, not {CANONICAL_BUNDLE_ID}."),
                format!(
                    "TCC grants will be attributed to {other}, not the cmux-cua daemon. \
                 Run via `cmux-cua mcp` (auto-relaunches inside cmux Computer Use.app) or \
                start the daemon manually: `open -n -g -a 'cmux Computer Use' --args serve`."
                ),
            ),
        }
    };
...
fn check_tcc_accessibility() -> CheckEntry {
    ...
    let hint = if external_permission_flow_enabled() {
        ...
        "Grant Accessibility to cmux Computer Use.app in System Settings → Privacy & Security → Accessibility. If the process bundle is not com.cmuxterm.cua (see bundle_identity), the grant must target the responsible app — restart via `cmux-cua mcp` to relaunch inside cmux Computer Use.app."
    };
    CheckEntry::fail(
        NAME_TCC_ACCESSIBILITY,
...
fn check_tcc_screen_recording() -> CheckEntry {
    ...
    let hint = if external_permission_flow_enabled() {
        "Grant Screen Recording to the Computer Use helper reported by bundle_identity using the embedding host's onboarding. Do not run the standalone cmux-cua permission flow."
    } else {
        "Grant Screen Recording to cmux Computer Use.app in System Settings → Privacy & Security → Screen Recording. The grant is attributed to the responsible process — see bundle_identity to confirm the right binary is being prompted."
    };
    CheckEntry::fail(
        NAME_TCC_SCREEN_RECORDING,
...
/// running the bare binary outside `cmux Computer Use.app`).
...
mod tests {
    use super::*;
    use cmux_cua_core::health_report::{CheckStatus, HealthReportTool};
    ...
    fn binary_version_always_passes() {
        ...
        assert!(
            entry.message.contains("cmux-cua "),
    ...
    fn bundle_identity_in_test_host_fails_with_full_shape() {
        ...
        // bundle id is either absent or not com.cmuxterm.cua. Either
    ...
    fn external_cmux_helper_bundle_identity_passes_without_standalone_hint() {
        let entry = check_bundle_identity_for_context(
            ...
            "/Library/Application Support/cmux/computer-use/helper/tag/cmux Computer Use.app/Contents/MacOS/cmux-cua".to_owned(),
            true,
        );
