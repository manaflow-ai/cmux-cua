...
impl Tool for LaunchAppTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let watchable_front = cmux_cua_core::watchable_front_mode();
        ...
        if let Ok(Ok((pid, _, _))) = &launch_result {
            if let Some(prior) = prior_frontmost {
                if *pid != prior {
                    ...
                    // long-running `cmux-cua mcp` / `cmux-cua serve`
                    // daemon modes. The one-shot `cmux-cua call` mode
                    ...
                    // the agent is driving" — `cmux-cua call` doesn't
...
mod tests {
    ...
    fn preflight_file_urls_returns_structured_file_not_found() {
        let missing = "/tmp/cmux-cua-definitely-missing-file-for-test.md".to_string();
        ...
        assert_eq!(structured["path"], "/tmp/cmux-cua-definitely-missing-file-for-test.md");
