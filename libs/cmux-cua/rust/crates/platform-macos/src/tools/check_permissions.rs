...
///   - the **CmuxCua daemon** (`com.cmuxterm.cua`) when this process is
...
///     standalone check can read `true` while `tccutil … com.cmuxterm.cua`
///     reports no record.
fn permission_source() -> serde_json::Value {
    ...
    let exe = std::env::current_exe()
        ...
        .unwrap_or_default();
    let disclaimed = std::env::var_os(cmux_cua_core::RESPONSIBILITY_DISCLAIMED_ENV).is_some();
    let embedded = cmux_cua_core::embedded_mode();
    let host_bundle_id = std::env::var(cmux_cua_core::HOST_BUNDLE_ID_ENV).unwrap_or_default();
...
fn permission_source_for_context(
    ...
    // can't run from inside the code-signed `cmux Computer Use.app` bundle without
        ...
        && bundle_identifier == Some(super::health_report::CANONICAL_BUNDLE_ID)
    {
        (
            ...
            "These booleans reflect the CmuxCua daemon's own TCC identity \
             (com.cmuxterm.cua) because this process is its own responsible \
             process.",
        )
    } else if is_responsible_app {
        (
            "helper-daemon",
            "These booleans reflect this helper application's own TCC identity. Permission setup belongs to the embedding host; do not run the standalone cmux-cua permission flow.",
        )
    } else {
        (
            "caller",
            "These booleans reflect the TCC identity of the app that launched \
             ...
             (com.cmuxterm.cua). A standalone check can read `true` here while \
             `tccutil … com.cmuxterm.cua` reports no record. To grant for the \
    ...
    if is_caller {
        summary.push_str(
            "\nℹ️  Status reflects the launching app's TCC identity, not the CmuxCua \
             daemon (com.cmuxterm.cua). See `source` for details.",
        );
    }
...
mod tests {
    ...
    fn disclaim_env_var_alone_does_not_grant_daemon_attribution() {
        ...
        // inside the code-signed `cmux Computer Use.app` bundle — the test runner does
        // not, so even with the env var present we must fail closed to "caller".
        let _guard = env_lock();
        let name = cmux_cua_core::RESPONSIBILITY_DISCLAIMED_ENV;
        let original = swap_env(name, Some("1"));
        let embedded = swap_env(cmux_cua_core::EMBEDDED_ENV, None);
        ...
        restore_env(cmux_cua_core::EMBEDDED_ENV, embedded);
    ...
    fn embedded_mode_reports_host_attribution() {
        let _guard = env_lock();
        let embedded = swap_env(cmux_cua_core::EMBEDDED_ENV, Some("1"));
        let host = swap_env(
            cmux_cua_core::HOST_BUNDLE_ID_ENV,
        ...
        restore_env(cmux_cua_core::HOST_BUNDLE_ID_ENV, host);
        restore_env(cmux_cua_core::EMBEDDED_ENV, embedded);
    }
    ...
    fn embedded_plus_disclaim_env_never_yields_daemon_attribution() {
        ...
        let _guard = env_lock();
        let embedded = swap_env(cmux_cua_core::EMBEDDED_ENV, Some("1"));
        let disclaim = swap_env(cmux_cua_core::RESPONSIBILITY_DISCLAIMED_ENV, Some("1"));
        ...
        restore_env(cmux_cua_core::RESPONSIBILITY_DISCLAIMED_ENV, disclaim);
        restore_env(cmux_cua_core::EMBEDDED_ENV, embedded);
    }
    ...
    fn embedded_env_requires_exact_value_one() {
        let _guard = env_lock();
        let embedded = swap_env(cmux_cua_core::EMBEDDED_ENV, Some("true"));
        let source = permission_source();
        assert_ne!(
            source.get("attribution").and_then(|v| v.as_str()),
            Some("host"),
            "only CMUX_CUA_EMBEDDED=1 may enable embedded mode"
        );
        restore_env(cmux_cua_core::EMBEDDED_ENV, embedded);
    }
