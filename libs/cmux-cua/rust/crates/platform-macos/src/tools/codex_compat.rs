...
//!
//! The native cmux-cua API is pid/window oriented and intentionally broad.
...
//! contract, a per-MCP-session app snapshot, and action-only dispatch results,
...
impl CompatState {
    ...
    fn revive_cursor_after_fresh_state(&self, session: &str) -> bool {
        ...
        should_revive
            && cmux_cua_core::session::with_live_session(session, || {
    ...
    async fn run_action(&self, kind: CompatKind, args: Value) -> ToolResult {
        ...
        // Sky's action methods return only an acknowledgement. Perception is
        // an explicit, caller-controlled operation (`get_app_state`) and may
        // be performed once after a group of actions. Keeping this boundary
        // action-only avoids a full AX walk, screenshot process, image encode,
        // and model payload after every click/key/scroll.
        action_ack(&action_name, &action_result)
    }
    ...
    async fn perform_secondary_action(
        ...
        .await;
        let changes = window_snapshot.detect_async_fast().await;
        match result {
...
fn native_action_arguments(
    ...
    // The Codex compatibility contract separates input dispatch from the
    // subsequent perception call. Native action tools use these private
    // markers to skip their legacy one-second window poll and to use the
    // cursor's close-enough interaction milestone without changing the broad
    // native/Claude profile.
    object.insert(
        "_codex_compat_fast_action".to_owned(),
        Value::Bool(true),
    );
    object.insert(
        "_codex_compat_close_enough_cursor".to_owned(),
        Value::Bool(true),
    );
    ...
    {
        object.insert(
            cmux_cua_core::HOST_SESSION_ARG.to_owned(),
...
fn action_ack(action_name: &str, native_result: &ToolResult) -> ToolResult {
    let mut structured = json!({
        "action": {
            "name": action_name,
            "result": first_text(native_result),
        },
        "state_refresh_required": true,
    });
    if let Some(native) = native_result.structured_content.as_ref() {
        // Keep useful dispatch diagnostics (path/effect/verified) without
        // returning the native tree or screenshot payload.
        if let Some(object) = native.as_object() {
            let compact = object
                .iter()
                .filter(|(key, _)| matches!(key.as_str(), "path" | "effect" | "verified" | "escalation"))
                .map(|(key, value)| (key.clone(), value.clone()))
                .collect::<serde_json::Map<_, _>>();
            if !compact.is_empty() {
                structured["dispatch"] = Value::Object(compact);
            }
        }
    }
    ToolResult {
...
fn action_ack_with_warning(action_name: &str, warning: ToolResult) -> ToolResult {
    let mut ack = action_ack(action_name, &warning);
    if let Some(structured) = ack.structured_content.as_mut() {
        structured["warning"] = Value::String(first_text(&warning));
    }
    ack
}
...
fn enforce_target_policy(app: &AppIdentity) -> Result<(), CompatError> {
    ...
    let host_bundle = std::env::var(cmux_cua_core::HOST_BUNDLE_ID_ENV)
        .unwrap_or_default()
        .to_ascii_lowercase();
...
mod tests {
    ...
    fn native_actions_keep_lifecycle_and_host_cursor_sessions_distinct() {
        ...
        assert_eq!(native["_codex_compat_fast_action"], true);
        assert_eq!(native["_codex_compat_close_enough_cursor"], true);
    }
    ...
    fn compat_session_end_clears_state_and_cursor_before_explicit_revival() {
        ...
        cmux_cua_core::session::fire_session_end(session);
        assert!(cmux_cua_core::session::is_session_ended(session));
        ...
        assert!(cmux_cua_core::session::revive_session(session));
    ...
    fn compat_indices_do_not_depend_on_global_native_tokens() {
        ...
        let registry = cmux_cua_core::element_token::global();
        let first = registry.register_snapshot(app_snapshot.app.pid, 7, 4);
        let stale_token = cmux_cua_core::element_token::token_for(first, 3);
        for window_id in 8..(8 + cmux_cua_core::element_token::LRU_CAP_PER_PID as u32) {
    ...
    #[test]
    fn action_ack_is_perception_free_and_preserves_dispatch_diagnostics() {
        let native = ToolResult::text("clicked").with_structured(json!({
            "path": "ax",
            "effect": "unverifiable",
            "verified": false,
            "elements": [{"element_index": 1}],
        }));
        let ack = action_ack("click", &native);
        assert!(ack.content.is_empty());
        assert_eq!(ack.structured_content.as_ref().unwrap()["state_refresh_required"], true);
        assert_eq!(ack.structured_content.as_ref().unwrap()["dispatch"]["path"], "ax");
        assert!(ack.structured_content.as_ref().unwrap().get("elements").is_none());
    }
