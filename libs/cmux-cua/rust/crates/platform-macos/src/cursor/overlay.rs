...
//! `cmux-cua/src/main.rs`).  AppKit MUST run on the **main thread**.
...
use cursor_overlay::{
    CursorConfig, CursorKey, FocusRect, KeyedOverlayCommand, MotionConfig, MotionTickResult,
    OverlayCommand, OverlayMsg, Palette, RenderStateCore, ZOrderEnforcer,
};
...
static CLOSE_ENOUGH_TX: Mutex<Option<HashMap<CursorKey, ArrivalWaiter>>> = Mutex::new(None);
static NEXT_ARRIVAL_GENERATION: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
static NEXT_CLOSE_ENOUGH_GENERATION: std::sync::atomic::AtomicU64 =
...
fn close_enough_register(key: CursorKey, tx: tokio::sync::oneshot::Sender<()>) -> u64 {
    let generation = NEXT_CLOSE_ENOUGH_GENERATION.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    let mut guard = CLOSE_ENOUGH_TX.lock().unwrap();
    let map = guard.get_or_insert_with(HashMap::new);
    if let Some(old) = map.insert(key, ArrivalWaiter { generation, tx }) {
        let _ = old.tx.send(());
    }
...
fn close_enough_fire(key: &CursorKey) {
    if let Ok(mut guard) = CLOSE_ENOUGH_TX.lock() {
        if let Some(map) = guard.as_mut() {
            if let Some(waiter) = map.remove(key) {
                let _ = waiter.tx.send(());
            }
...
fn close_enough_cancel(key: &CursorKey, generation: u64) {
    if let Ok(mut guard) = CLOSE_ENOUGH_TX.lock() {
        if let Some(map) = guard.as_mut() {
            if map
                .get(key)
...
async fn wait_for_close_enough(
    key: &CursorKey,
    generation: u64,
    rx: tokio::sync::oneshot::Receiver<()>,
    timeout: Duration,
    ...
    match tokio::time::timeout(timeout, rx).await {
        Ok(Ok(())) => true,
        Ok(Err(_)) | Err(_) => {
            close_enough_cancel(key, generation);
            false
        }
    }
}
...
pub fn init(cfg: CursorConfig) {
    ...
    REVIVE_HOOK_REGISTERED.get_or_init(|| {
        cmux_cua_core::session::register_session_revive_hook(|session_id| {
...
fn send_registered_close_enough_move(
    sender: Option<&std::sync::mpsc::Sender<OverlayMsg>>,
    key: CursorKey,
    cmd: OverlayCommand,
    generation: u64,
...
pub async fn animate_cursor_to(key: CursorKey, x: f64, y: f64) {
    animate_cursor_to_with_timing(key, x, y, false).await;
}

/// Animate the cursor and unblock once it is close enough for the next input.
///
/// The render thread continues the final glide/spring after this future
/// resolves. This is the timing mode used by the Codex compatibility profile;
/// native callers retain [`animate_cursor_to`] when they need full arrival.
pub async fn animate_cursor_to_close_enough(key: CursorKey, x: f64, y: f64) {
    animate_cursor_to_with_timing(key, x, y, true).await;
}

/// Select the interaction milestone encoded by a native tool request.
pub async fn animate_cursor_for_action(
    key: CursorKey,
    x: f64,
    y: f64,
    ...
    if args
        ...
        .unwrap_or(false)
    {
        animate_cursor_to_close_enough(key, x, y).await;
    } else {
...
async fn animate_cursor_to_with_timing(
    ...
    // can fire it at the selected interaction milestone.
    let (tx, rx) = tokio::sync::oneshot::channel::<()>();
    let generation = if close_enough {
        close_enough_register(key.clone(), tx)
    } else {
        arrival_register(key.clone(), tx)
    };

    // Send the MoveTo command (click offset applied inside apply_command).
    let move_command = OverlayCommand::MoveTo {
        x,
        y,
    ...
    let sent = if close_enough {
        send_registered_close_enough_move(CMD_TX.get(), key.clone(), move_command, generation)
    } else {
        send_registered_move(CMD_TX.get(), key.clone(), move_command, generation)
    };
    ...
    // Await the selected signal, but never let a stopped/stalled renderer hang
    // a tool call indefinitely.
    if close_enough {
        let _ = wait_for_close_enough(&key, generation, rx, ARRIVAL_TIMEOUT).await;
    } else {
...
impl RenderState {
    ...
    fn tick(&mut self, dt: f64) -> bool {
        self.tick_events(dt).arrived
    }

    fn tick_events(&mut self, dt: f64) -> MotionTickResult {
        let result = self.core.tick_swift_constants_events(dt);
...
fn render_loop(
    ...
    loop {
        ...
        // `arrived` collects keys whose path just ended; `close_enough` collects
        // keys that reached the earlier interaction milestone. Position-only motion
        ...
        let (
            ...
            match guard.as_mut() {
                Some(map) => {
                    ...
                    let mut close_enough: Vec<CursorKey> = Vec::new();
                    if frame_tick_needed || had_msg || woke_for_idle_maintenance {
                        for (k, rs) in map.cursors.iter_mut() {
                            let tick = rs.tick_events(dt);
                            if tick.close_enough {
                                close_enough.push(k.clone());
                            }
                            if tick.arrived {
