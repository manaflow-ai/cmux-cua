...
impl CursorRegistry {
    ...
    pub fn get_or_create(&self, cursor_id: &str) -> CursorState {
        ...
        if cmux_cua_core::session::is_session_ended(cursor_id) {
    ...
    pub fn update_config(&self, config: CursorConfig) {
        ...
        if config.cursor_id.is_empty()
            || cmux_cua_core::session::is_session_ended(&config.cursor_id)
        {
            return;
        }
        let mut inner = self.inner.lock().unwrap();
    ...
    pub fn update_position(&self, cursor_id: &str, x: f64, y: f64) {
        ...
        if cursor_id.is_empty() || cmux_cua_core::session::is_session_ended(cursor_id) {
            return;
        }
        let config = self.config_for(cursor_id);
        let mut inner = self.inner.lock().unwrap();
...
mod tests {
    ...
    fn default_cursor_unaffected_by_guard() {
        ...
        let reg = CursorRegistry::new();
        assert!(!cmux_cua_core::session::is_session_ended("default"));
    ...
    fn explicit_non_session_cursor_id_unaffected_by_guard() {
        ...
        assert!(!cmux_cua_core::session::is_session_ended(cid));
