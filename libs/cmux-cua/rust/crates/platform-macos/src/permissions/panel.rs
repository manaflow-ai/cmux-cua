...
//! bundled `/Applications/cmux Computer Use.app`. Mirrors the Swift gate at
//! `libs/cmux-cua/Sources/CmuxCuaCore/Permissions/PermissionsGate.swift`.
//!
//! ## UX
//!
...
//! `cmux-cua/src/main.rs` starts its socket worker, then invokes the gate on
...
///
///   * `CMUX_CUA_PERMISSIONS_PANEL` is not set to an explicit off
...
pub fn panel_enabled() -> bool {
    ...
    if env_off("CMUX_CUA_PERMISSIONS_PANEL") {
        return false;
    }
...
const REQUESTED_PERMISSIONS_ENV: &str = "CMUX_CUA_PERMISSION_REQUESTED";
...
extern "C" fn on_poll_tick(
    ...
    if let Err(e) = result {
        eprintln!("[cmux-cua] permissions panel poll tick panicked: {e:?}");
...
/// Returns the runtime-registered `CmuxCuaPermissionsPanelTarget`
...
fn panel_target_class() -> &'static objc2::runtime::AnyClass {
    ...
    CLASS.get_or_init(|| {
        ...
        let mut builder = ClassBuilder::new("CmuxCuaPermissionsPanelTarget", superclass)
            .expect("CmuxCuaPermissionsPanelTarget already registered");
        unsafe {
            builder.add_method(
...
fn handle_permission_action(permission: MissingPermission) {
        ...
        match action {
            ...
            Some(RowAction::CompleteInSystemSettings) => {
                ...
                if let Err(error) = crate::permissions::gate::open_system_settings_for(permission) {
                    eprintln!(
                        "[cmux-cua] could not open System Settings for {}: {error}",
                        permission.label()
                    );
                    HANDLES.with(|cell| unsafe {
                        let mut guard = cell.borrow_mut();
    ...
    if let Err(error) = result {
        eprintln!("[cmux-cua] permissions panel action panicked: {error:?}");
    }
}
...
mod tests {
    ...
    fn explicit_off_values_disable_panel() {
        ...
        for v in &["0", "false", "FALSE", "False", "no", "NO", "off", "OFF"] {
            std::env::set_var("CMUX_CUA_PERMISSIONS_PANEL", v);
            assert!(
                env_off("CMUX_CUA_PERMISSIONS_PANEL"),
                "value {v:?} must disable the panel",
            );
        }
        std::env::remove_var("CMUX_CUA_PERMISSIONS_PANEL");
    }
    ...
    fn unset_env_uses_bundled_panel_by_default() {
        let _guard = env_lock();
        std::env::remove_var("CMUX_CUA_PERMISSIONS_PANEL");
        assert!(!env_off("CMUX_CUA_PERMISSIONS_PANEL"));
    }
    ...
    fn copy_tracks_each_grant_state() {
        ...
        assert_eq!(
            heading_for(st(false, false), "cmux CUA"),
            "Enable cmux CUA Computer Use"
        );
        assert_eq!(
            heading_for(st(true, false), "cmux CUA"),
            "Finish enabling cmux CUA Computer Use"
        );
        assert_eq!(
        ...
        assert_eq!(
            heading_for(st(true, true), "cmux CUA"),
            "cmux CUA Computer Use is ready"
        );
        assert_eq!(
    ...
    fn app_bundle_ships_complete_english_and_japanese_permission_copy() {
        let english = include_str!(
            "../../../../scripts/CmuxCuaBundle/Contents/Resources/en.lproj/Localizable.strings"
        );
        let japanese = include_str!(
            "../../../../scripts/CmuxCuaBundle/Contents/Resources/ja.lproj/Localizable.strings"
        );
    ...
    fn localized_templates_replace_every_dynamic_product_token() {
        let value = localized_template(
            ...
            &[("{feature}", "Computer Use"), ("{product}", "cmux CUA")],
        );
        assert_eq!(value, "Enable Computer Use for cmux CUA");
