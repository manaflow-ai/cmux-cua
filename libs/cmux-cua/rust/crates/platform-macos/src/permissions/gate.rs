...
//!      (which grant, why cmux-cua needs it, what to do next).
...
//! `CMUX_CUA_PERMISSIONS_GATE` set to `0` / `false` / `no` / `off`
...
unsafe extern "C" fn permission_main_callback(context: *mut c_void) {
    ...
    if result.is_err() {
        ...
        eprintln!("[cmux-cua] permission control panicked on the AppKit main thread");
    }
}
...
impl GateOpts {
    ...
    /// (`CMUX_CUA_PERMISSIONS_GATE` set to `0` / `false` / `no` /
    /// `off`, case-insensitive, disables the gate), an explicit
    /// `--no-permissions-gate` flag, and embedded mode
    /// (`CMUX_CUA_EMBEDDED=1`).  Any signal is sufficient to opt out.
    ...
    pub fn from_env_and_flag(no_gate_flag: bool) -> Self {
        ...
        let env_disabled = std::env::var("CMUX_CUA_PERMISSIONS_GATE")
            .ok()
            .map(|v| {
            ...
            .unwrap_or(false);
        Self {
            opt_out: no_gate_flag || env_disabled || cmux_cua_core::embedded_mode(),
            ..Self::default()
        }
    }
}
...
/// re-exec (which otherwise spams the user with "cmux CUA" dialogs).
const GATE_REEXEC_ENV: &str = "CMUX_CUA_GATE_REEXEC";
...
const GATE_START_ENV: &str = "CMUX_CUA_GATE_START_UNIX";
...
pub fn run_if_needed(opts: GateOpts) -> Result<()> {
    ...
    // re-doing that on every re-exec spams a fresh "cmux CUA" dialog at the
...
pub fn wait_for_grants(opts: &GateOpts) -> Result<()> {
    ...
    loop {
        if last_missing.is_empty() {
            println!("[cmux-cua] permissions granted — continuing startup");
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if polls_without_change >= EXEC_AFTER_POLLS {
                println!(
                    "[cmux-cua] rechecking permissions — restarting daemon \
                     (still missing: {})",
                    fmt_missing(&last_missing)
                );
                let _ = std::io::stdout().flush();
        ...
        if new_missing != last_missing {
            ...
            if !new_missing.is_empty() {
                println!(
                    "[cmux-cua] progress — still waiting on: {}",
...
fn reexec_self(mode: ReexecMode) {
    ...
    // binary path (e.g. /Applications/cmux Computer Use.app/Contents/MacOS/
    // cmux-cua) which is what we want — execvp searches PATH for
    // bare names, and we don't want a stale `~/.local/bin/cmux-cua`
    // symlink to win.
    let Ok(exe) = std::env::current_exe() else {
        eprintln!("[cmux-cua] re-exec failed: cannot resolve current_exe");
        return;
    };
    let argv: Vec<CString> = std::env::args_os()
        ...
        .collect();
    if argv.is_empty() {
        eprintln!("[cmux-cua] re-exec failed: empty argv");
        return;
    }
    ...
    let exe_c = match CString::new(exe.into_os_string().into_vec()) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("[cmux-cua] re-exec failed: exe path contains NUL ({e})");
            return;
        }
    };
    ...
    eprintln!("[cmux-cua] re-exec failed: {err}");
}
...
fn print_banner(missing: &[MissingPermission], open_settings: bool) {
    println!();
    println!("──────────────────────────────────────────────────────────────");
    println!(" cmux-cua needs your permission before `serve` can start");
    println!("──────────────────────────────────────────────────────────────");
    println!();
    ...
    println!("   cmux-cua serve --no-permissions-gate");
    println!(" or set CMUX_CUA_PERMISSIONS_GATE to 0/false/no/off");
...
mod tests {
    ...
    /// `CMUX_CUA_EMBEDDED`, which the `check_permissions` tests mutate.
    ...
    fn env_var_disables_gate() {
        let _guard = env_lock();
        std::env::set_var("CMUX_CUA_PERMISSIONS_GATE", "0");
        let opts = GateOpts::from_env_and_flag(false);
        assert!(opts.opt_out, "env=0 must opt out");
        std::env::remove_var("CMUX_CUA_PERMISSIONS_GATE");
    }
    ...
    fn neither_flag_nor_env_does_not_opt_out() {
        let _guard = env_lock();
        std::env::remove_var("CMUX_CUA_PERMISSIONS_GATE");
        std::env::remove_var(cmux_cua_core::EMBEDDED_ENV);
        let opts = GateOpts::from_env_and_flag(false);
        assert!(!opts.opt_out);
    }
    ...
    fn embedded_mode_opts_out_of_gate() {
        ...
        std::env::set_var(cmux_cua_core::EMBEDDED_ENV, "true");
        assert!(!GateOpts::from_env_and_flag(false).opt_out);
        std::env::remove_var(cmux_cua_core::EMBEDDED_ENV);
    }
    ...
    fn env_var_truthy_values_do_not_opt_out() {
        ...
        for v in &["1", "true", "yes", "on", "garbage", ""] {
            std::env::set_var("CMUX_CUA_PERMISSIONS_GATE", v);
            let opts = GateOpts::from_env_and_flag(false);
            assert!(
                !opts.opt_out,
