//! cmux-cua — cross-platform background computer-use automation daemon.
//!
...
/// Fire the per-entry-point telemetry event (e.g. `cmux_cua_mcp`,
/// `cmux_cua_api_click`). Respects the opt-out env var — no-op when
...
/// top of `CmuxCuaCommand.main()`. The install ping is *not* emitted
...
fn maybe_init_pip() {
    ...
    match pip_preview::start_pip(&cfg) {
        Ok(backend) => {
            // Bridge: when the tool dispatcher in cmux-cua-core wants
            ...
            cmux_cua_core::pip_hook::set_pip_push_fn(|frame| {
            ...
            eprintln!(
                "⚗️  PiP preview enabled (experimental — macOS only today; \
                 see https://github.com/manaflow-ai/cmux-cua/issues for follow-up)"
            );
        }
        Err(e) => {
...
/// `cmux-cua-core` graph (shared with every `platform-*` crate) stays
...
fn build_macos_registry_with_compat(
    claude_compat: bool,
    codex_compat: bool,
) -> cmux_cua_core::tool::ToolRegistry {
    if codex_compat {
...
fn run_guarded_appkit_worker(name: &str, work: impl FnOnce()) -> i32 {
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(work)) {
        Ok(()) => 0,
        Err(_) => {
            eprintln!("[cmux-cua] {name} worker panicked");
            101
        }
    }
}
...
fn main() {
    ...
    // probes) it sets CMUX_CUA_RESPONSIBILITY_DISCLAIMED, asking this process to disclaim
    ...
    // already-disclaimed re-exec, or a binary inside cmux Computer Use.app.
    if crate::bundle::is_env_truthy("CMUX_CUA_RESPONSIBILITY_DISCLAIMED") {
    ...
    {
        eprintln!(
            "cmux-cua: TCC-protected commands belong to cmux Computer Use. {}",
    ...
    match command {
        ...
        cli::Command::Call { tool, json_args, screenshot_out_file, socket } => {
            ...
            cmux_cua_core::recording::set_screenshot_fn(|window_id, pid| {
            ...
            cmux_cua_core::recording::set_click_marker_fn(|png_bytes, cx, cy| {
                platform_macos::capture::crosshair_png_bytes(png_bytes, cx, cy).ok()
            });
            cmux_cua_core::recording::set_ax_snapshot_fn(|window_id, pid| {
                platform_macos::recording_hooks::app_state_json_for(window_id, pid)
            });
            cmux_cua_core::recording::set_element_bounds_fn(|wid, pid, idx| {
        ...
        cli::Command::Serve {
            ...
            //
            // The gate's `wait_for_grants` blocks while `com.cmuxterm.cua`
            ...
            // `open -n -g -a 'cmux Computer Use' --args serve` (the correct-TCC-
            ...
            // CMUX_CUA_PERMISSIONS_GATE=0 for CI / headless.
            //
            ...
            if let Err(e) = platform_macos::permissions::run_if_needed(gate_opts) {
                eprintln!("[cmux-cua] permissions gate: {e}");
                eprintln!("[cmux-cua] continuing — tool calls touching AX or \
        ...
        cli::Command::Mcp {
            ...
            // resolves into /Applications/cmux Computer Use.app, run the
            ...
            let exit_code = run_guarded_appkit_worker("mcp", || {
                ...
                rt.block_on(async move {
                    ...
                    let result = if codex_compat {
                        cmux_cua_core::server::run_codex_computer_use_compat(registry.clone())
                            .await
                    } else {
                        cmux_cua_core::server::run(registry.clone()).await
                    };
                    if let Err(e) = result {
...
fn main() -> anyhow::Result<()> {
    ...
    match command {
        ...
        cli::Command::Mcp {
            ...
            // without this, an `cmux-cua mcp` spawned by Claude
...
async fn async_main() -> anyhow::Result<()> {
    ...
    registry.init_self_weak();
    maybe_init_pip();
    let result = cmux_cua_core::server::run(registry.clone()).await;
    if let Err(e) = &result {
        tracing::error!("MCP server error: {e}");
    }
    registry.remove_state_file();
...
#[cfg(not(target_os = "macos"))]
fn build_registry(cursor_cfg: cursor_overlay::CursorConfig) -> cmux_cua_core::tool::ToolRegistry {
    let compat = CLAUDE_CODE_COMPAT.load(Ordering::SeqCst);
    #[cfg(target_os = "windows")]
    {
        cmux_cua_core::recording::set_classified_screenshot_fn(|window_id, pid| {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if platform_linux::wayland::is_wayland() {
            cmux_cua_core::video::set_video_backend_factory(Box::new(
                platform_linux::video_wayland::WfRecorderVideoBackendFactory,
            ));
        } else {
            cmux_cua_core::video::set_video_backend_factory(Box::new(
                cmux_cua_core::video_ffmpeg::FfmpegVideoBackendFactory,
            ));
        }
...
#[cfg(not(target_os = "macos"))]
fn build_registry_no_cursor() -> cmux_cua_core::tool::ToolRegistry {
    let compat = CLAUDE_CODE_COMPAT.load(Ordering::SeqCst);
    #[cfg(target_os = "windows")]
    {
        cmux_cua_core::recording::set_classified_screenshot_fn(|window_id, pid| {
...
mod stub {
    ...
    use cmux_cua_core::tool::{Tool, ToolDef, ToolResult};
