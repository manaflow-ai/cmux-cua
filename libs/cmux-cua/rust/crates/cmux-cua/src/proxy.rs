...
//! the running `cmux-cua serve` daemon over its Unix socket.
//!
//! This is the runtime half of the TCC auto-relaunch path (issue #1525,
//! mirror of Swift PR #1479). When `cmux-cua mcp` is invoked from
//! an IDE terminal — Claude Code, Cursor, VS Code, Warp — macOS TCC
//! attributes the process to the calling terminal, not to
//! `cmux Computer Use.app`. The MCP client side sees a normal stdio server,
...
//!
//! Why this lives in `cmux-cua` and not `mcp-server`:
//!   `cmux_cua_core::server::run` already speaks JSON-RPC over stdio
...
//!   Putting the proxy here avoids `mcp-server → cmux-cua` reverse
//!   coupling.

use std::sync::Arc;

use cmux_cua_core::protocol::{
    codex_computer_use_initialize_result, initialize_result, Request, Response,
};
#[cfg(target_os = "macos")]
use cmux_cua_core::protocol::{ToolCall, ToolResult};
...
/// JSON-RPC envelope.
///
/// Mirrors `cmux_cua_core::server::run`'s control flow exactly — same
...
pub async fn run_proxy(
    ...
    #[cfg(not(target_os = "macos"))]
    {
        if !is_daemon_listening(&socket_path) {
            anyhow::bail!(
                "cmux-cua daemon not reachable on {socket_path}. Start it \
                 with `cmux-cua serve` and retry."
            );
        }
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "daemon on {socket_path} did not report a tool profile. Stop that daemon and \
                 restart it with the current cmux-cua binary before retrying."
            )
        })?;
    if reported_profile != expected_profile.as_str() {
        anyhow::bail!(
...
fn fetch_tools_list_from_daemon(
        ...
        .map(|t| {
            ...
            let capabilities = t.get("capabilities")
                ...
                .unwrap_or_else(|| {
                    ...
                    name.as_str()
                        .map(cmux_cua_core::tool::default_capabilities_for)
                        .unwrap_or_default()
                        .into_iter()
    ...
    let capability_version = result
        ...
        .unwrap_or_else(|| serde_json::Value::String(
            cmux_cua_core::tool::CAPABILITY_VERSION.to_owned()
        ));
...
fn admit_bootstrap_tool_call(
    ...
    let def = cmux_cua_core::tool::ToolDef {
    ...
    if let Err(result) = cmux_cua_core::tool::validate_dispatch_args(&def, &call.args) {
...
async fn ensure_daemon_started(
    ...
    let external_permission_flow =
        crate::bundle::is_env_truthy("CMUX_CUA_EXTERNAL_PERMISSION_FLOW");
...
/// `cmux_cua_core::server::handle_request`:
...
///     true`. Mirrors the in-process `cmux_cua_core::server` path so
...
fn daemon_response_to_mcp(
    ...
    if !response.ok {
        ...
        // level, so the in-process `cmux_cua_core::server` would surface
...
fn configured_default_session() -> Option<String> {
    std::env::var(cmux_cua_core::DEFAULT_SESSION_ENV)
        .ok()
...
fn configured_state_owner_pid() -> Option<u32> {
    std::env::var("CMUX_CUA_STATE_OWNER_PID")
        .ok()
    ...
    if managed_session {
        if let Some(object) = args.as_object_mut() {
            ...
            if let Some(owner_pid) = configured_state_owner_pid() {
                object.insert(
                    cmux_cua_core::session_state::STATE_OWNER_PID_ARG.to_owned(),
...
// (the CMUX_CUA_MCP_FORCE_PROXY harness); these tests just lock
...
mod tests {
    ...
    async fn external_proxy_waits_for_cmux_daemon_without_launching_standalone() {
        ...
        assert_eq!(
            ...
            0,
            "an externally-owned proxy must never launch cmux Computer Use.app"
        );
    }
