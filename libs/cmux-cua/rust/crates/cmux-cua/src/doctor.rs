//! `cmux-cua doctor` — environment + install diagnostic probes.
//!
...
//!   script), plus a hint to run `cmux-cua diagnose` for a full
...
/// Probe: version + target triple — the same string `cmux-cua --version`
...
fn probe_version() -> Probe {
    ...
    Probe::ok("binary", format!("cmux-cua {version} ({target})"))
}
...
/// `~/.local/bin/cmux-cua -> packages/current/cmux-cua` chain) so the
...
/// Probe: package-home directory (`~/.cmux-cua`) — counts cached
/// release dirs so the user can sanity-check
/// `CMUX_CUA_KEEP_VERSIONS` is doing the right thing. Renamed from
/// `.cmux-cua/` in v0.2.16 to match the install-path rename (PR #1644).
fn probe_home_dir() -> Probe {
    let home = match home_dir() {
        Some(h) => h,
        None => return Probe::warn("home dir", "neither HOME nor USERPROFILE set"),
    };
    let cua_home = home.join(".cmux-cua");
    if !cua_home.exists() {
        return Probe::warn(
            "home dir",
...
fn probe_telemetry() -> Probe {
    let env_disabled = std::env::var("CMUX_CUA_TELEMETRY_ENABLED")
        .ok()
        .map(|v| matches!(v.to_ascii_lowercase().as_str(), "0" | "false" | "no" | "off"))
        .unwrap_or(false);
    let install_id_present = home_dir()
        .map(|h| h.join(".cmux-cua").join(".telemetry_id").exists())
        .unwrap_or(false);
    if env_disabled {
        Probe::ok(
            "telemetry",
            "disabled via CMUX_CUA_TELEMETRY_ENABLED",
        )
    } else {
    ...
    let in_session_0 = match desktop.session_id {
        Some(0) => {
            report.push(
                ...
                .with_detail(
                    "re-run cmux-cua from an interactive logon (RDP, console, or a scheduled task in the user's session) for the GUI tools to function.",
                ),
            );
            true
        }
...
fn append_platform_probes(report: &mut Report) {
    ...
    let legacy_plist = format!("{home}/Library/LaunchAgents/com.trycua.cmux_cua_updater.plist");
    let legacy_script = "/usr/local/bin/cmux-cua-update";
    ...
    report.push(Probe::ok(
        "TCC + cdhash report",
        "for a full bundle / signature / TCC dump, run `cmux-cua diagnose`",
    ));
}
