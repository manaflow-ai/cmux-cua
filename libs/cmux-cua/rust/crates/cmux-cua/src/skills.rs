...
//!   delete the local copy under `<HomeDir>/skills/cmux-cua/` (and the
//!   pre-rename `cmux-cua/` location if present).
//! - `status` — print local install state + per-agent link state.
//! - `path` — print `<HomeDir>/skills/cmux-cua` (the local copy).
//!
...
//! binary's own version: `cmux-cua-v<v>-skills.tar.gz` from
//! `https://github.com/manaflow-ai/cmux-cua/releases/...`. This pins the skill
...
//! `Skills/cmux-cua/` directory (one HTTP call per file — used
//! for bleeding-edge dev validation; not the default).
//!
//! ## Agent detection
//!
//! Same agent dirs as the Swift cmux-cua installer detects:
//!
...
const SKILL_PACK_NAME: &str = "cmux-cua";
/// Pre-rename name. The skill pack used to install as `cmux-cua`
/// (when the Rust port lived at `libs/cmux-cua/`). On install /
...
const LEGACY_SKILL_PACK_NAME: &str = "cmux-cua";
...
///
/// Override with `cmux-cua skills install --all-platforms` to keep
...
fn home_dir() -> Result<PathBuf> {
    if let Ok(h) = std::env::var("CMUX_CUA_HOME") {
...
/// The pre-rename home (`~/.cmux-cua/`) if it exists on disk.
/// `None` when CMUX_CUA_HOME is set (the env var overrides both
/// the new and legacy defaults — caller is on their own).
fn legacy_home_dir() -> Option<PathBuf> {
    if std::env::var("CMUX_CUA_HOME").is_ok() {
        return None;
    }
    #[cfg(windows)]
...
const AGENTS: &[Agent] = &[
    ...
    // cmux-cua pack symlinked here adds the platform-specific deep
...
pub fn run(subcommand: &str, flags: &[String]) {
    let result = match subcommand {
        ...
        other => {
            ...
            eprintln!("Usage: cmux-cua skills {{install|update|uninstall|status|path}}");
            std::process::exit(64);
        }
    };
    match result {
        Ok(()) => {}
        Err(e) => {
            eprintln!("cmux-cua skills {subcommand}: {e}");
            std::process::exit(1);
        }
    }
}
...
fn install(flags: &[String], force: bool) -> Result<()> {
    ...
    // Sweep the legacy `cmux-cua`-named pack out FIRST so the
    ...
    if !already_present || force {
        ...
        println!("✅ Skill pack already at {} (use `cmux-cua skills update` to refresh)", local.display());
    }
...
/// locations are swept:
///   1. `<HomeDir>/skills/cmux-cua/`       — old pack NAME under new home dir
///   2. `<LegacyHomeDir>/skills/cmux-cua/`    — new pack NAME under old home dir
///   3. `<LegacyHomeDir>/skills/cmux-cua/` — old pack NAME under old home dir
/// Plus every `<agent_skills>/cmux-cua` symlink/junction.
///
...
fn sweep_legacy_skill_pack() {
    ...
    // Agent links named `<parent>/cmux-cua`.
    for agent in AGENTS {
...
fn fetch_into(dest: &Path, from_main: bool, all_platforms: bool) -> Result<()> {
    ...
    if from_main {
        ...
        let base = "https://raw.githubusercontent.com/trycua/cua/main/libs/cmux-cua/rust/Skills/cmux-cua";
    ...
    let url = format!(
        "https://github.com/manaflow-ai/cmux-cua/releases/download/cmux-cua-v{version}/cmux-cua-v{version}-skills.tar.gz"
    );
...
fn extract_tar_gz(bytes: &[u8], dest: &Path, all_platforms: bool) -> Result<()> {
    ...
    //   v0.2.18 and earlier: cmux-cua-v<v>-skills/cmux-cua/<file>
    //   v0.2.19 (briefly):   cmux-cua-v<v>-skills/cmux-cua/<file>
    //   v0.2.20+:            cmux-cua-v<v>-skills/<file>     (CD workflow now flattens)
    //
    // Strip the outer staging dir always; additionally strip a SECOND
    // wrapping dir IF it's named `cmux-cua` or `cmux-cua` — that
    ...
    for entry in archive.entries()? {
        ...
        if let Some(next) = peek.next() {
            ...
            if name == "cmux-cua" || name == "cmux-cua" {
                components.next();
            }
        }
...
fn uninstall(flags: &[String]) -> Result<()> {
    ...
    // Try BOTH the current name and the legacy `cmux-cua` name so a
    ...
    if remove_local {
        ...
        // Also clean up any pre-rename home (`~/.cmux-cua/`) that
        // might still hold a skill pack from before the
        // `.cmux-cua/` → `.cmux-cua/` migration. Remove the empty
...
fn status() -> Result<()> {
    ...
    if local.exists() && local.join("SKILL.md").exists() {
        ...
        println!("Local skill pack: not installed (`cmux-cua skills install` to fetch)");
    }
    println!();
    println!("Agent links:");
    for agent in AGENTS {
...
mod tests {
    ...
    fn extract_flat_tarball_v_0_2_20_plus() {
        ...
        //   cmux-cua-v0.2.20-skills/SKILL.md
        //   cmux-cua-v0.2.20-skills/WINDOWS.md
        let bytes = build_tarball(&[
            ("cmux-cua-v0.2.20-skills/SKILL.md",   b"flat-skill"),
            ("cmux-cua-v0.2.20-skills/WINDOWS.md", b"flat-win"),
        ]);
        ...
        assert!(!dest.path().join("cmux-cua").exists());
        assert!(!dest.path().join("cmux-cua").exists());
    }
    ...
    fn extract_legacy_tarball_v_0_2_18_double_wrap_old_name() {
        ...
        //   cmux-cua-v0.2.18-skills/cmux-cua/SKILL.md
        // Both wrappers must be stripped or the user ends up with a
        // nested cmux-cua/ dir (the bug this fixes).
        let bytes = build_tarball(&[
            ("cmux-cua-v0.2.18-skills/cmux-cua/SKILL.md",   b"legacy-skill"),
            ("cmux-cua-v0.2.18-skills/cmux-cua/WINDOWS.md", b"legacy-win"),
        ]);
        let dest = tempdir().unwrap();
        extract_tar_gz(&bytes, dest.path(), true).unwrap();
        ...
        assert!(!dest.path().join("cmux-cua").exists(),
            "nested cmux-cua/ dir should have been stripped");
    }

    #[test]
    fn extract_double_wrap_new_name_also_strips() {
        // Interim shape (v0.2.19, briefly): inner dir is `cmux-cua/`.
        let bytes = build_tarball(&[
            ("cmux-cua-v0.2.19-skills/cmux-cua/SKILL.md", b"interim-skill"),
        ]);
        let dest = tempdir().unwrap();
        extract_tar_gz(&bytes, dest.path(), true).unwrap();
        let s = std::fs::read_to_string(dest.path().join("SKILL.md")).unwrap();
    ...
    fn extract_preserves_subdirs_inside_pack() {
        ...
        // wrappers are.
        let bytes = build_tarball(&[
            ("cmux-cua-v0.2.20-skills/examples/click.md", b"sample"),
        ]);
        let dest = tempdir().unwrap();
        extract_tar_gz(&bytes, dest.path(), true).unwrap();
    ...
    fn extract_per_host_filter_drops_other_platform_docs() {
        ...
        let bytes = build_tarball(&[
            ("cmux-cua-v0.2.20-skills/README.md",   b"r"),
            ("cmux-cua-v0.2.20-skills/SKILL.md",    b"s"),
            ("cmux-cua-v0.2.20-skills/WINDOWS.md",  b"w"),
            ("cmux-cua-v0.2.20-skills/MACOS.md",    b"m"),
            ("cmux-cua-v0.2.20-skills/LINUX.md",    b"l"),
            ("cmux-cua-v0.2.20-skills/RECORDING.md",b"R"),
            ("cmux-cua-v0.2.20-skills/WEB_APPS.md", b"W"),
            ("cmux-cua-v0.2.20-skills/TESTS.md",    b"T"),
        ]);
        let dest = tempdir().unwrap();
