...
//!
//! Subcommand dispatch (mirrors the Swift cmux-cua CLI):
//!
//!   cmux-cua                              → mcp server (default)
//!   cmux-cua mcp                          → mcp server (explicit)
//!   cmux-cua list-tools                   → print all tool names + descriptions
//!   cmux-cua describe <tool>              → print tool schema
//!   cmux-cua call <tool> [json-args]      → invoke tool, print result
//!   cmux-cua <tool> [json-args]           → shorthand for call (snake_case names)
//!
//! Cursor-overlay flags (--cursor-id, --no-overlay, etc.) are consumed by
//! `CursorConfig::from_args()` and are ignored here.

use std::process;
use cmux_cua_core::{protocol::Content, tool::ToolRegistry};
...
pub enum Command {
    ...
    Call {
        ...
        /// when None — i.e. `cmux-cua call X` looks for the user's
    ...
    Serve {
        ...
        /// `CMUX_CUA_PERMISSIONS_GATE=0` short-circuits the gate too
        ...
        /// flag was a no-op for `cmux-cua mcp --claude-code-computer-use-compat`,
    ...
    Doctor { json: bool },
    Diagnose,
    /// `cmux-cua permissions status|grant [--json]`: report profile-bound
    /// daemon TCC status or open correctly-attributed CmuxCua onboarding.
    ...
    /// one-shot `cmux_cua_install` PostHog event. Bypasses the
    ...
    /// `cmux-cua autostart {enable|disable|status|kick}` —
    /// platform-native auto-start so `cmux-cua serve` comes up on
    ...
    /// message. See `crates/cmux-cua/src/autostart.rs`.
    Autostart { subcommand: String },
    /// `cmux-cua manifest` — emit a stable JSON description of the CLI
    ...
    /// `_CMUX_CUA_ARGS = ["mcp"]` and read the canonical invocation
    ...
    /// (`cmux-cua-v<v>-skills.tar.gz` — the asset filename keeps
    /// the legacy `-rs` for backward-compat with pinned URLs) from
    /// GitHub, places it under `<HomeDir>/skills/cmux-cua/`, and
    /// symlinks into each detected agent's `skills/` dir. See
    /// `crates/cmux-cua/src/skills.rs`.
...
pub fn parse_command() -> Command {
    ...
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("cmux-cua {}", env!("CARGO_PKG_VERSION"));
        std::process::exit(0);
    }
    if args.iter().any(|a| a == "--help" || a == "-h") {
        println!("cmux-cua {} — cross-platform computer-use automation driver", env!("CARGO_PKG_VERSION"));
        println!("Usage: cmux-cua [SUBCOMMAND] [OPTIONS]");
        ...
        println!("                                  identity (com.cmuxterm.cua). If no daemon is running it reports");
        ...
        println!("                                  status reflects com.cmuxterm.cua instead of your terminal.");
        println!();
        println!("approvals options (macOS):");
        println!("  cmux-cua approvals list       List permanent Computer Use app approvals.");
        println!("  cmux-cua approvals revoke ID  Revoke one bundle id or stable identity.");
        println!("  cmux-cua approvals clear      Revoke every permanent Computer Use app approval.");
        println!("    --json                        Emit a machine-readable result.");
        println!();
        println!("Updating cmux-cua:");
        println!("  cmux-cua check-update         Ask GitHub whether a newer release is available. Read-only.");
        ...
        println!("  cmux-cua update               Same check as above, then suggest --apply if outdated.");
        ...
        println!("  cmux-cua autostart enable     Register a logon Scheduled Task so serve starts at every interactive logon.");
        println!("  cmux-cua autostart disable    Remove the autostart entry. No-op if not registered.");
        println!("  cmux-cua autostart status     Print whether the entry is registered + whether the daemon is running.");
        println!("  cmux-cua autostart kick       Start the entry now without re-logging.");
        println!();
        println!("skills options (agent skill-pack management, opt-in):");
        println!("  cmux-cua skills install       Fetch the versioned skill pack from GitHub Releases and symlink it");
        ...
        println!("  cmux-cua skills update        Re-fetch the skill pack from GitHub, refreshing the local copy + links.");
        println!("  cmux-cua skills uninstall     Remove the agent symlinks. Add --all to also delete the local copy.");
        ...
        println!("                          Also: CMUX_CUA_MCP_NO_RELAUNCH=1");
        println!("  --embedded              Run embedded inside a host app (also: CMUX_CUA_EMBEDDED=1).");
        println!("                          Inherits the host's TCC grants; never prompts or relaunches.");
        println!("                          See Skills/cmux-cua/EMBEDDING.md.");
        ...
        println!("  Embedded stdio processes (CMUX_CUA_EMBEDDED=1) get an automatic default");
        println!("  cursor keyed by CMUX_CUA_DEFAULT_SESSION (else embedded-<pid>), so anonymous");
        ...
        println!("  (These are no-ops for one-shot CLI calls like `cmux-cua call` — the overlay");
        println!("   needs the long-lived AppKit runloop that only `serve` / `mcp` keep alive.)");
        println!();
        println!("manifest options:");
        println!("  cmux-cua manifest             Emit a stable JSON description of this CLI's surface");
    ...
    // `cmux_cua_core::embedded_mode()` and children inherit the mode.
    if args.iter().any(|a| a == "--embedded") {
        std::env::set_var(cmux_cua_core::EMBEDDED_ENV, "1");
    }
    if let Some(id) = flag_value(&args, "--host-bundle-id") {
        std::env::set_var(cmux_cua_core::HOST_BUNDLE_ID_ENV, id);
    }
    ...
    match pos.next() {
        None => {
            // Bare `cmux-cua` defaults to MCP, which reads JSON-RPC from
            ...
            // Explicit `cmux-cua mcp` bypasses the check entirely.
            use std::io::IsTerminal as _;
            if std::io::stdin().is_terminal() {
                eprintln!("cmux-cua: bare invocation defaults to the MCP server, which reads");
                eprintln!("JSON-RPC from stdin. From a terminal that looks like a hang.");
                eprintln!();
                eprintln!("You probably meant one of:");
                eprintln!("  cmux-cua list-tools                           # available tools");
                eprintln!("  cmux-cua status                               # check the daemon");
                eprintln!("  cmux-cua mcp-config --client claude-code      # wire into a client");
                eprintln!("  cmux-cua --help                               # everything else");
                eprintln!();
                eprintln!("To run the MCP server explicitly (and pipe JSON-RPC by hand):");
                eprintln!("  cmux-cua mcp");
                std::process::exit(0);
            }
            Command::Mcp {
                no_daemon_relaunch,
        ...
        Some("approvals") => {
            ...
            let subcommand = match parse_approvals_args(&approval_args) {
                ...
                Err(error) => {
                    eprintln!("{error}");
                    eprintln!("Usage: cmux-cua approvals {{list|revoke <bundle-id-or-stable-id>|clear}} [--json]");
                    process::exit(64);
                }
            };
            let json = args.iter().any(|a| a == "--json");
        ...
        Some("call") => {
            ...
            let json_args = match pos.next() {
                Some(s) => match serde_json::from_str(s) {
                    Ok(v) => Some(v),
                    Err(e) => {
                        eprintln!("error: positional JSON arg to 'cmux-cua call' did not parse: {e}");
                        eprintln!("       received: {s}");
                        eprintln!();
                        ...
                        eprintln!("        '{{\"pid\":1234,\"window_id\":5678}}' | cmux-cua call {}", tool);
                        eprintln!();
        ...
        Some("telemetry") => {
            ...
            match pos.next() {
                ...
                _ => {
                    eprintln!("Usage: cmux-cua telemetry install-event");
                    process::exit(64);
                }
            }
        }
        Some("autostart") => {
            ...
            if subcommand.is_empty() {
                eprintln!("Usage: cmux-cua autostart {{enable|disable|status|kick}}");
                process::exit(64);
            }
            Command::Autostart { subcommand }
        }
        Some("skills") => {
            // Skills subcommand. Default is `status` so plain `cmux-cua
        ...
        Some(first) => {
            ...
            let json_args = match pos.next() {
                Some(s) => match serde_json::from_str(s) {
                    Ok(v) => Some(v),
                    Err(e) => {
                        eprintln!("error: positional JSON arg to 'cmux-cua {tool}' did not parse: {e}");
                        eprintln!("       received: {s}");
                        eprintln!();
                        eprintln!("hint: PowerShell 5.1 strips quotes around JSON field names in");
                        eprintln!("      multi-field args. Pipe the JSON via stdin instead:");
                        eprintln!("        '{{\"pid\":1234,\"window_id\":5678}}' | cmux-cua {}", tool);
                        eprintln!();
                        eprintln!("      Or use PowerShell 7+ (pwsh) which preserves the quotes.");
                        process::exit(2);
                    }
...
pub fn run_list_tools(registry: &ToolRegistry) {
    ...
    let mut entries: Vec<(&str, &cmux_cua_core::tool::ToolDef)> = registry.iter_defs().collect();
...
/// installed `cmux Computer Use.app` bundle, with a non-launchd parent".
...
pub fn should_use_daemon_proxy(no_daemon_relaunch: bool) -> bool {
    use crate::bundle::{
        is_env_truthy, is_executable_inside_cmux_cua_app, parent_is_not_launchd,
    ...
    if is_env_truthy("CMUX_CUA_MCP_NO_RELAUNCH") {
        return false;
    }
    // Hidden test/escape hook: force proxy mode without requiring the
    // executable to live inside cmux Computer Use.app. Used by the
    ...
    if std::env::var_os("CMUX_CUA_DAEMON_APP").is_some() {
        return true;
    }
    if !is_executable_inside_cmux_cua_app() {
...
/// interactive session (via `cmux-cua autostart enable && kick`,
/// or any other Session-1+ launch) can answer tool calls correctly —
/// so when one is up, we proxy through it.
///
/// Behaviour:
///   * `--no-daemon-relaunch` or `CMUX_CUA_MCP_NO_RELAUNCH=1`
///     forces in-process (matches macOS opt-out).
///   * `CMUX_CUA_MCP_FORCE_PROXY=1` always proxies, even with
...
///     Linux/Windows — there's no `open -a CmuxCua` analog).
#[cfg(not(target_os = "macos"))]
pub fn should_use_daemon_proxy(no_daemon_relaunch: bool) -> bool {
    ...
    if is_env_truthy("CMUX_CUA_MCP_FORCE_PROXY") {
        return true;
    }
    // Either the regular daemon (`\\.\pipe\cmux-cua`) OR the uiAccess'd
    // worker (`\\.\pipe\cmux-cua-uia`) is a valid proxy target on Windows:
    ...
    // that bypasses UIPI for UWP apps. See #1602 / the cmux-cua-uia crate.
    #[cfg(target_os = "windows")]
    {
...
/// Spawn `/usr/bin/open -n -g -a 'cmux Computer Use' --args serve` to launch
...
pub fn launch_daemon_and_wait(
    ...
    // passed a non-default socket via `cmux-cua mcp --socket /path`.
    ...
    // cmux sets CMUX_CUA_DAEMON_APP to its bundled "cmux Computer Use.app" so
    ...
    let daemon_app =
        std::env::var("CMUX_CUA_DAEMON_APP").unwrap_or_else(|_| "cmux Computer Use".to_string());
    ...
    // hardcoded false), so `cmux-cua mcp --claude-code-computer-use-compat`
    ...
    let status = Cmd::new("/usr/bin/open")
        // `-n` forces a new instance: cmux Computer Use.app might already be
    ...
    if !status.success() {
        anyhow::bail!(
            "`open -n -g -a 'cmux Computer Use' --args serve{}` exited {:?}. \
             Check that `/Applications/cmux Computer Use.app` is installed, or \
    ...
    anyhow::bail!(
        "daemon did not appear on {socket_path} within {timeout_secs}s. If this \
         ...
         cmux Computer Use.app in System Settings and retry. Pass --no-daemon-relaunch \
         to stay in-process."
    );
}
...
pub fn run_mcp_via_daemon_proxy(
    ...
    if !crate::serve::is_daemon_listening(&socket_path) {
        match missing_daemon_action(
            ...
            // (or lazily launches an ordinary CmuxCua daemon) on the first
            // `tools/call`. In particular, the external branch never launches
            // a standalone daemon.
            MissingDaemonAction::EnterLazyProxy => {}
            MissingDaemonAction::FailExternalDaemon => {
                if crate::bundle::is_env_truthy("CMUX_CUA_EXTERNAL_PERMISSION_FLOW") {
                    anyhow::bail!(
                        "the cmux Computer Use runtime is not listening on {socket_path}. \
                         ...
                         cmux; do not launch cmux-cua directly"
                    );
                }
                anyhow::bail!(
                    "CMUX_CUA_MCP_FORCE_PROXY=1 but no daemon listening on \
                     {socket_path}. Start one with `cmux-cua serve --socket {socket_path}` \
                     and retry."
                );
            }
            MissingDaemonAction::FailPlatformDaemon => {
                ...
                anyhow::bail!(
                    "no cmux CUA daemon listening on {socket_path}. Start one in \
                     your interactive session — on Windows run \
                     `cmux-cua autostart enable && cmux-cua autostart kick`; \
                     on Linux run `cmux-cua serve &` in the user's session. \
                     Then re-run `cmux-cua mcp`. To skip the proxy and run \
...
/// Emit a stable, machine-readable JSON description of the cmux-cua CLI
...
/// `_CMUX_CUA_ARGS = ["mcp"]` and read the canonical invocation from the
/// binary itself.
///
...
pub fn build_manifest() -> serde_json::Value {
    ...
    // a bare "cmux-cua" the caller has to resolve.
    let binary = std::env::current_exe()
        .ok()
        .and_then(|p| p.to_str().map(str::to_owned))
        .unwrap_or_else(|| "cmux-cua".to_owned());
    ...
    serde_json::json!({
        ...
        "subcommands": [
            ...
            { "name": "autostart",
              "description": "Platform-native auto-start so `cmux-cua serve` comes up on every logon.",
              "args": [ { "name": "subcommand", "type": "positional-string", "description": "enable | disable | status | kick" } ] },
            { "name": "skills",
              "description": "Manage the cmux-cua agent skill pack (install / update / uninstall / status / path).",
...
fn codex_mcp_add_command(binary: &str, computer_use_compat: bool) -> String {
    if computer_use_compat {
        ...
        format!("codex mcp add cmux-cua -- {binary} mcp")
    }
}
    ...
    "cmux-cua": {{
      ...
      "args": ["mcp"]
        ...
        Some("claude") | Some("claude-code") => {
            ...
            // which is the source of the "registered cmux-cua but
  ...
  "mcpServers": {{
    "cmux-cua": {{
      ...
      "type": "stdio"
        ...
        Some("openclaw") => {
            println!("openclaw mcp set cmux-cua '{{\"command\":\"{binary}\",\"args\":[\"mcp\"]}}'");
        }
        Some("opencode") => {
    ...
    "cmux-cua": {{
      ...
      "enabled": true
        ...
        Some("pi") => {
            println!(
                "Pi (badlogic/pi-mono) does not support MCP natively — the author\n\
                 ...
                 Use cmux-cua as a plain CLI from inside Pi instead:\n\n\
                     {binary} list_apps\n\
        ...
        Some("qwen") | Some("qwen-code") => {
            ...
            println!("qwen mcp add cmux-cua {binary} mcp");
        }
        Some("droid") | Some("factory") => {
            ...
            println!("droid mcp add cmux-cua \"{binary} mcp\"");
        }
        Some("zcode") => {
            ...
            println!(
                "# ZCode (Z.ai) is a GUI app — add via Settings -> MCP Servers ->\n\
                 ...
                 #   zai mcp add cmux-cua --transport stdio --command \"{binary}\" --args mcp\n\
                 {pretty}",
            );
        }
        Some(other) => {
...
pub fn run_call(
    ...
    //
    // On Windows, prefer the uiAccess-elevated worker (cmux-cua-uia.exe) when
    ...
    // `\\.\pipe\cmux-cua` is Medium integrity and gets ERROR_ACCESS_DENIED on
    ...
    // (LaunchServices' "responsible" process), not to com.cmuxterm.cua —
    ...
    // grants correctly (`open … CmuxCua --args serve`, which raises the
    // dialog as CmuxCua and waits for the grant). We deliberately do NOT
    ...
    let json_args = {
        ...
        {
            eprintln!(
                "cmux-cua: reporting permission status only. A prompt raised from \
                 this terminal would attribute to the terminal, not CmuxCua, so the \
                 ...
                 then approve the CmuxCua dialog in System Settings."
            );
    ...
    if crate::serve::is_daemon_listening(&socket_path) {
        ...
        match crate::serve::send_request(&socket_path, &req) {
                ...
                if resp.ok {
                    if let Some(result) = resp.result {
                        ...
                        if let Some(sc) = result.get("structuredContent") {
                            ...
                            // this fn) so `cmux-cua call screenshot` over
            ...
            Err(e) => {
                ...
                eprintln!(
                    "[cmux-cua] WARNING: daemon proxy to {socket_path} failed ({e}); \
    ...
    if registry.get_def(tool).is_none() {
        ...
        eprintln!("Run `cmux-cua list-tools` to see available tools.");
        process::exit(64);
    }
...
/// `cmux-cua recording <start|stop|status>` — wrapper around
/// `start_recording` / `stop_recording` / `get_recording_state` tools
/// on the running daemon.
///
/// Requires a running daemon (`cmux-cua serve`) because recording
...
pub fn run_recording_cmd(subcommand: &str, args: &[String], socket: Option<&str>) {
    ...
    if !crate::serve::is_daemon_listening(&socket_path) {
        eprintln!(
            "cmux CUA daemon is not running.\n\
             Start it first with: cmux-cua serve"
        );
        process::exit(1);
    }
    ...
    match subcommand {
        "start" => {
            let output_dir = match args.first() {
                ...
                None => {
                    eprintln!("Usage: cmux-cua recording start <output-dir>");
                    process::exit(64);
                }
            };
...
/// `cmux-cua recording render <input-dir> <out.mp4> [--no-zoom] [--scale N]`
/// Pure file-to-file work — does NOT go through the daemon.
///
/// Note on flag parsing: the global cmux-cua CLI parser strips
...
fn run_recording_render(args: &[String]) {
    ...
    let input_dir = match positionals.get(0) {
        ...
        _ => {
            eprintln!("Usage: cmux-cua recording render <input-dir> <out.mp4> [--no-zoom] [--scale N]");
            process::exit(64);
        }
    };
    ...
    let opts = cmux_cua_core::recording_render::RenderOptions {
        no_zoom,
        default_scale: scale,
    };
    println!("Rendering {} -> {}{}",
    ...
    match cmux_cua_core::recording_render::render(&input_dir, &output_path, &opts) {
        Ok(res) => {
...
/// `cmux-cua update [--apply]` — check for a newer release and optionally apply it.
///
...
pub fn run_update_cmd(apply: bool, json: bool) {
    ...
    match latest {
        Err(e) => {
            ...
            tracing::debug!(target: "cmux_cua::update", "fetch failed: {e}");
            if !json {
            ...
            match crate::updater::run_install_script(&v) {
                Ok(s) if s.success() => {
                    ...
                    if daemon_was_running {
                        ...
                        println!("  cmux-cua stop && cmux-cua serve");
                    }
                }
                Ok(s) => {
                    eprintln!(
...
pub fn run_permissions_cmd(
    ...
    if crate::bundle::requires_external_daemon() {
        eprintln!(
            "cmux Computer Use owns this binary's permission flow. Use the tag-scoped cmux Computer Use helper and cmux Settings; standalone `cmux-cua permissions` is disabled."
        );
        process::exit(1);
    }
    match subcommand {
        ...
        match execute_approvals(platform_macos::app_approval::global(), subcommand) {
            ...
            Err(error) => {
                if json {
                    ...
                    eprintln!("cmux-cua approvals failed: {error}");
                }
                process::exit(1);
            }
        }
...
fn approvals_platform_error(is_macos: bool) -> Option<&'static str> {
    (!is_macos).then_some("cmux-cua approvals is supported only on macOS.")
}
...
fn run_permissions_status(json: bool) {
    ...
    if states.is_empty() {
        if json {
            let payload = serde_json::json!({
                ...
                "status": "unknown",
                "profiles": {},
                "reason": "no CmuxCua daemon profile is available under the app bundle's own TCC identity",
            });
            println!("{}", serde_json::to_string_pretty(&payload).unwrap());
        } else {
            ...
            println!("  Run `cmux-cua permissions grant` to launch onboarding.");
        }
        return;
    }
    ...
    if states.iter().any(|state| !permission_all_granted(state)) {
        println!("\n  Run `cmux-cua permissions grant` to continue onboarding.");
    }
}
...
fn run_permissions_grant() {
    ...
    #[cfg(not(target_os = "macos"))]
    {
        eprintln!("`cmux-cua permissions grant` is macOS-only.");
        process::exit(1);
    }
}
...
fn run_permissions_grant_macos() -> anyhow::Result<()> {
    ...
    if live.is_empty() {
        ...
        println!("Launching CmuxCua permission onboarding.");
        ...
        println!("Using the running CmuxCua daemon to open permission onboarding.");
    }
    ...
    if live.iter().any(permission_all_granted) {
        ...
        println!("CmuxCua has Accessibility and Screen Recording for every live profile.");
        return Ok(());
    }
    ...
    println!("Complete Accessibility and Screen Recording in the CmuxCua panel.");
...
/// `cmux-cua check-update [--json] [--no-cache]` — pure check, never installs.
///
        ...
        match (&state.latest_version, &state.error) {
            (Some(latest), _) => {
                ...
                if state.update_available {
                    println!();
                    println!("Update available. Run `cmux-cua update --apply` to install.");
        ...
        "commands": [
            {
                ...
                "discussion": "On macOS, shell-spawned MCP processes can auto-launch and proxy through a cmux Computer Use.app daemon so TCC grants attach to the bundle. On Windows and Linux, MCP proxies through an already-running daemon when one is listening.",
                "arguments": no_args,
                "options": [
                ...
                "flags": [
                    ...
                    {"name":"embedded","short_name":null,"help":"Run embedded inside a host app: inherit the host's TCC grants, never prompt or relaunch. Also CMUX_CUA_EMBEDDED=1.","default_value":false}
                ],
            ...
            {
                "name": "serve",
                "abstract": "Run cmux CUA as a long-running daemon.",
            ...
            {
                "name": "status",
                "abstract": "Report whether a cmux CUA daemon is running.",
                "discussion": "",
                "arguments": no_args,
                "options": [
                    {"name":"socket","short_name":null,"help":"Override the daemon socket or named-pipe path.","type":"String","default_value":null,"is_optional":true},
            ...
            {
                "name": "check-update",
                "abstract": "Check whether a newer cmux-cua release is available.",
...
/// `cmux-cua diagnose` — print a paste-able bundle-path / install-layout / TCC report.
///
...
///   - TCC DB rows for com.cmuxterm.cua (sqlite3, best-effort)
...
fn diagnose_install_layout_section() -> String {
    ...
    let app_path = "/Applications/cmux Computer Use.app";
    ...
    let cli_paths = [
        ("symlink", format!("{home}/.local/bin/cmux-cua")),
        ("legacy symlink", "/usr/local/bin/cmux-cua".to_owned()),
    ];
    ...
    let stale = format!("{home}/Applications/cmux Computer Use.app");
...
fn diagnose_tcc_db_section() -> String {
    ...
    let sql = "SELECT service, client, client_type, auth_value, auth_reason, \
               hex(csreq) AS csreq_hex FROM access WHERE client='com.cmuxterm.cua';";

    let mut lines = vec!["## tcc database rows for com.cmuxterm.cua".to_owned()];
...
fn diagnose_config_paths_section() -> String {
    ...
    let paths: &[(&str, String)] = &[
        ("user data dir",   format!("{home}/.cmux-cua")),
        ("config cache",    format!("{home}/Library/Caches/cmux-cua")),
        ("telemetry id",    format!("{home}/.cmux-cua/.telemetry_id")),
        ("updater plist",   format!("{home}/Library/LaunchAgents/com.trycua.cmux_cua_updater.plist")),
        ("daemon plist",    format!("{home}/Library/LaunchAgents/com.trycua.cmux_cua_daemon.plist")),
    ];
...
/// Path to the persistent JSON config file (`~/.cmux-cua/config.json`).
...
/// `cmux-cua config [show|get|set|reset] [key] [value]`
///
    ...
    match subcommand.unwrap_or("show") {
        ...
        "set" => {
            let key = match key {
                Some(k) => k,
                None => {
                    eprintln!("Usage: cmux-cua config set <key> <value>");
                    process::exit(64);
                }
            };
            ...
            if result.is_error.unwrap_or(false) {
                for item in &result.content {
                    if let cmux_cua_core::protocol::Content::Text { text, .. } = item {
                        eprintln!("{text}");
                    }
                }
                process::exit(1);
...
/// `cmux-cua doctor` — run platform-aware diagnostic probes and emit a
/// structured report.
///
...
///
/// Mirrors Swift's `CmuxCuaCommand.telemetryEntryEvent(for:)`. Per-tool
/// `call <tool>` invocations report as `cmux_cua_api_<tool>` so per-tool
...
pub fn telemetry_entry_event(cmd: &Command) -> Option<String> {
    ...
    let name = match cmd {
        ...
        Command::Update { .. } => "cmux_cua_update".to_owned(),
        Command::CheckUpdate { .. } => "cmux_cua_check_update".to_owned(),
        Command::Doctor { .. } => "cmux_cua_doctor".to_owned(),
        Command::Permissions { .. } => "cmux_cua_permissions".to_owned(),
        Command::Approvals { subcommand, .. } => match subcommand {
            ApprovalsSubcommand::List => "cmux_cua_approvals_list".to_owned(),
            ApprovalsSubcommand::Revoke { .. } => "cmux_cua_approvals_revoke".to_owned(),
            ApprovalsSubcommand::Clear => "cmux_cua_approvals_clear".to_owned(),
        },
        Command::Diagnose => "cmux_cua_diagnose".to_owned(),
        ...
        Command::Autostart { subcommand } => {
            format!("cmux_cua_autostart_{}", sanitize_tool_name(subcommand))
        }
        Command::Skills { subcommand, .. } => {
            format!("cmux_cua_skills_{}", sanitize_tool_name(subcommand))
        }
...
///
/// Tool names are concatenated onto `cmux_cua_api_` to build per-tool
...
mod tests {
    ...
    fn approvals_parser_accepts_list_revoke_clear_and_rejects_bad_shapes() {
        ...
        assert_eq!(
            ...
            Some("cmux-cua approvals is supported only on macOS.")
        );
    }
    ...
    fn codex_compat_uses_dedicated_default_daemon_paths() {
        ...
        assert!(compat_socket.ends_with("cmux-cua-codex-computer-use.sock")
            || compat_socket.ends_with("cmux-cua-codex-computer-use"));
        ...
        assert!(compat_pid.ends_with("cmux-cua-codex-computer-use.pid"));
        #[cfg(not(target_os = "windows"))]
        assert_eq!(
    ...
    fn codex_computer_use_config_emits_dedicated_profile_command() {
        assert_eq!(
            codex_mcp_add_command("/opt/cmux-cua", false),
            "codex mcp add cmux-cua -- /opt/cmux-cua mcp"
        );
        assert_eq!(
            codex_mcp_add_command("/opt/cmux-cua", true),
            "codex mcp add cua-computer-use -- /opt/cmux-cua mcp --codex-computer-use-compat"
        );
    }
    ...
    fn sanitize_tool_name_falls_back_when_non_ascii() {
        ...
        // we'd emit `cmux_cua_api_` (empty suffix), which collides with
        // the bare `cmux_cua_call` event.
