//! Unix-socket daemon server and client for `cmux-cua serve`/`stop`/`status`.
//!
...
//!   macOS  — ~/Library/Caches/cmux-cua/cmux-cua.sock
//!   Linux  — ~/.cache/cmux-cua/cmux-cua.sock
//!   Windows — \\.\pipe\cmux-cua  (TODO: use named pipe; stubs only for now)
...
fn maybe_start_http_transport(
    ...
    if !http_transport_allowed(profile) {
        eprintln!(
            "cmux-cua: HTTP MCP transport on port {port} is disabled for the \
...
/// proxy processes.
pub const SOCKET_AUTH_TOKEN_ENV: &str = "CMUX_CUA_SOCKET_AUTH_TOKEN";
...
pub const SOCKET_HOST_AUTH_TOKEN_ENV: &str = "CMUX_CUA_SOCKET_HOST_AUTH_TOKEN";
...
pub const SOCKET_AUTHORIZED_ROOT_PID_ENV: &str = "CMUX_CUA_SOCKET_AUTHORIZED_ROOT_PID";
...
/// `CMUX_CUA_RECORDING_IDLE_TTL_SECS` env var (used by the #1764 verify
...
fn apply_session_identity(
    ...
    if let Some(sess) = &explicit {
        cmux_cua_core::session::touch_session(sess);
    }
    args.as_object()
...
fn recording_idle_ttl_secs() -> u64 {
    std::env::var("CMUX_CUA_RECORDING_IDLE_TTL_SECS")
        .ok()
...
/// config cleared. Overridable via `CMUX_CUA_SESSION_IDLE_TTL_SECS`.
const SESSION_IDLE_TTL_SECS_DEFAULT: u64 = 300;

fn session_idle_ttl_secs() -> u64 {
    std::env::var("CMUX_CUA_SESSION_IDLE_TTL_SECS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|v| *v > 0)
...
fn spawn_session_idle_sweep() {
    ...
    tokio::spawn(async move {
        ...
        loop {
            tick.tick().await;
            let ended = cmux_cua_core::session::evict_idle(ttl);
            if !ended.is_empty() {
...
fn register_recording_session_end_hook(
    recording: std::sync::Arc<cmux_cua_core::recording::RecordingSession>,
) {
    cmux_cua_core::session::register_session_end_hook(move |sid| {
...
pub fn default_socket_path() -> String {
    #[cfg(target_os = "macos")]
    {
        ...
        format!("{home}/Library/Caches/cmux-cua/cmux-cua.sock")
    }
    #[cfg(target_os = "linux")]
    {
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".into());
        format!("{home}/.cache/cmux-cua/cmux-cua.sock")
    }
    #[cfg(target_os = "windows")]
    {
        r"\\.\pipe\cmux-cua".to_owned()
    }
    #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
    {
        "/tmp/cmux-cua.sock".to_owned()
    }
}
...
pub fn codex_compat_default_socket_path() -> String {
    #[cfg(target_os = "windows")]
    {
        r"\\.\pipe\cmux-cua-codex-computer-use".to_owned()
    }
    #[cfg(not(target_os = "windows"))]
    {
        let mut path = std::path::PathBuf::from(default_socket_path());
        path.set_file_name("cmux-cua-codex-computer-use.sock");
        path.to_string_lossy().into_owned()
    }
}

/// On Windows, returns the named-pipe path of the uiAccess-elevated worker
/// (`cmux-cua-uia.exe`). The main CLI/MCP binary can prefer this pipe over the
...
/// Medium-IL daemon. See #1602 / the `cmux-cua-uia` crate for the worker side.
#[cfg(target_os = "windows")]
pub fn default_uia_pipe_path() -> String {
    r"\\.\pipe\cmux-cua-uia".to_owned()
}
...
pub fn default_pid_file_path() -> String {
    ...
    {
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".into());
        format!("{home}/Library/Caches/cmux-cua/cmux-cua.pid")
    }
    #[cfg(target_os = "linux")]
    {
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".into());
        format!("{home}/.cache/cmux-cua/cmux-cua.pid")
    }
    #[cfg(target_os = "windows")]
    {
        let local = std::env::var("LOCALAPPDATA").unwrap_or_else(|_| "C:/Temp".into());
        format!("{local}/cmux-cua/cmux-cua.pid")
    }
    #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
    {
        "/tmp/cmux-cua.pid".to_owned()
    }
}

pub fn codex_compat_default_pid_file_path() -> String {
    let mut path = std::path::PathBuf::from(default_pid_file_path());
    path.set_file_name("cmux-cua-codex-computer-use.pid");
    path.to_string_lossy().into_owned()
}
...
pub struct DaemonRequest {
    ...
    /// MCP-session identity minted once per `cmux-cua mcp` proxy process and
    ...
    /// means an anonymous/"global" session — a one-shot `cmux-cua call` or a
...
fn attach_state_writer_identity(
    ...
    let requested_owner_pid = args
        .remove(cmux_cua_core::session_state::STATE_OWNER_PID_ARG)
        .and_then(|value| value.as_u64())
        .and_then(|value| u32::try_from(value).ok())
        .filter(|value| *value > 1);
    args.remove(cmux_cua_core::session_state::STATE_WRITER_PID_ARG);
    args.remove(cmux_cua_core::session_state::STATE_WRITER_START_SECONDS_ARG);
    args.remove(cmux_cua_core::session_state::STATE_WRITER_START_MICROSECONDS_ARG);
    #[cfg(target_os = "macos")]
    ...
    if let Some(identity) = writer_identity.filter(|identity| identity.process_id > 1) {
        args.insert(
            cmux_cua_core::session_state::STATE_WRITER_PID_ARG.to_owned(),
            serde_json::json!(identity.process_id),
        );
        args.insert(
            cmux_cua_core::session_state::STATE_WRITER_START_SECONDS_ARG.to_owned(),
            serde_json::json!(identity.start_seconds),
        );
        args.insert(
            cmux_cua_core::session_state::STATE_WRITER_START_MICROSECONDS_ARG.to_owned(),
    ...
    let inside_bundle = std::env::current_exe()
        ...
        .map(|path| {
            path.to_string_lossy()
                .contains("/cmux Computer Use.app/Contents/MacOS/")
        })
        .unwrap_or(false);
    let launched_by_launchd = unsafe { libc::getppid() } == 1;
    let disclaimed =
        std::env::var_os(cmux_cua_core::RESPONSIBILITY_DISCLAIMED_ENV).is_some();
...
pub fn send_request(socket_path: &str, req: &DaemonRequest) -> anyhow::Result<DaemonResponse> {
    ...
    cmux_cua_core::socket_io::write_all_with_retry(&mut w, line.as_bytes(), write_deadline)?;
                    ...
                    while let Ok(Some(line)) = lines.next_line().await {
                        ...
                        match req.method.as_str() {
                            ...
                            "list" => {
                                ...
                                // `cmux_cua_core::tool::default_capabilities_for`
                                ...
                                let tools: Vec<serde_json::Value> = reg.iter_defs()
                                    .map(|(name, def)| {
                                        let caps = cmux_cua_core::tool::default_capabilities_for(name);
                                        serde_json::json!({
                                            "name": name,
                                ...
                                let resp = DaemonResponse::ok(serde_json::json!({
                                    "tools": tools,
                                    "capability_version": cmux_cua_core::tool::CAPABILITY_VERSION,
                                    "schema_version": "1",
                                    "profile": profile,
                                }));
                                let _ = writer.write_all(
                            ...
                            "call" => {
                                ...
                                let tool_name = if raw_name == "type_text_chars" {
                                    eprintln!("[cmux-cua] deprecated tool name 'type_text_chars' — use 'type_text' instead.");
                                    "type_text".to_owned()
                                } else { raw_name.clone() };
                                ...
                                clamp_external_permission_prompt(
                                    crate::bundle::is_env_truthy(
                                        "CMUX_CUA_EXTERNAL_PERMISSION_FLOW"
                                    ),
                                    &tool_name,
                                    &mut args,
                                );
                                ...
                                let content: Vec<serde_json::Value> = result.content.iter().map(|c| {
                                    match c {
                                        ...
                                        cmux_cua_core::protocol::Content::Image { data, mime_type, .. } =>
...
/// (`cmux-cua-uia.exe`) via ShellExecute if it lives next to the main binary
...
///    Highest task), AND `CMUX_CUA_SPAWN_UIA_WORKER=1` is set (opt-in),
...
fn maybe_spawn_uia_worker() {
    ...
    if !crate::bundle::is_env_truthy("CMUX_CUA_SPAWN_UIA_WORKER") {
        tracing::debug!(
            "uia spawn skipped: CMUX_CUA_SPAWN_UIA_WORKER not set (opt-in only \
    ...
    let uia = match current.parent() {
        Some(dir) => dir.join("cmux-cua-uia.exe"),
        None => return,
    };
    if !uia.exists() {
    ...
    {
        Ok(_child) => {
            eprintln!("cmux-cua: spawned uiAccess worker via {}", uia.display());
        }
        Err(e) => {
...
///
/// Done via PowerShell instead of the windows-crate Win32 API because cmux-cua
...
/// Medium-IL user shell — every `cmux-cua <tool>` call from the CLI
...
pub async fn run_serve(
    ...
    if security_attrs.is_none() {
        eprintln!(
            "cmux-cua: failed to build cross-IL SECURITY_ATTRIBUTES; pipe will be \
...
pub async fn run_serve(
    _registry: std::sync::Arc<cmux_cua_core::tool::ToolRegistry>,
    _socket_path: &str,
    _pid_file_path: Option<&str>,
    _profile: DaemonProfile,
) -> anyhow::Result<()> {
    anyhow::bail!("cmux-cua serve is not supported on this platform");
}
...
pub fn run_serve_cmd(
    ...
    if is_daemon_listening(&socket_path) {
        let pid_hint = pid_file_path.as_deref()
            ...
            .unwrap_or_default();
        eprintln!(
            "cmux CUA daemon is already running on {socket_path}{pid_hint}. \
             Run `cmux-cua stop` first."
        );
        std::process::exit(1);
    }
    ...
    #[cfg(target_os = "windows")]
    {
        if matches!(
            ...
            eprintln!(
                "WARNING: cmux-cua serve is starting in Session 0 (services). \
...
/// `cmux-cua stop` implementation.
pub fn run_stop_cmd(socket_path: &str) {
    if !is_daemon_listening(socket_path) {
        eprintln!("cmux CUA daemon is not running");
        std::process::exit(1);
    }
        ...
        Ok(_) => {
            ...
            loop {
                ...
                if std::time::Instant::now() >= deadline {
                    eprintln!("cmux CUA daemon did not release socket within 2s");
                    std::process::exit(1);
                }
...
/// `cmux-cua status` implementation.
pub fn run_status_cmd(socket_path: &str, pid_file_path: &str) {
    if is_daemon_listening(socket_path) {
        println!("cmux CUA daemon is running");
...
mod gate_tests {
    ...
    use cmux_cua_core::tool::{Tool, ToolDef, ToolRegistry};
    ...
    async fn ended_session_call_is_gated_live_and_anon_pass() {
        ...
        reg.register(Box::new(cmux_cua_core::session_tools::StartSessionTool));
