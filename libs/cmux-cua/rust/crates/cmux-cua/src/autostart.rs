//! `cmux-cua autostart {enable|disable|status|kick}` — register / inspect /
//! trigger the platform-native auto-start mechanism so `cmux-cua serve`
//! comes up on every interactive logon without the user pasting a
//! startup one-liner.
//!
//! ## Platform mapping
//!
//! - **Windows**: Scheduled Task `cmux-cua-serve` registered with
...
pub const TASK_NAME: &str = "cmux-cua-serve";
...
pub enum Status {
    ...
    RegisteredIdle,
    /// Entry registered AND a `cmux-cua serve` process is live.
    RegisteredRunning,
}
...
/// Register the platform-native autostart entry for `cmux-cua serve`.
...
/// Find the cmux-cua executable to bake into the autostart entry.
...
mod platform {
    ...
    /// Inline PowerShell that mirrors `install.ps1::Register-CmuxCuaAutostart`
    ...
    /// `cmux-cua.exe serve` in `powershell -WindowStyle Hidden` +
    /// `Start-Process -WindowStyle Hidden`. Without this, Task Scheduler
    /// allocates a new CUI console window (because `cmux-cua.exe` is a
    ...
    /// immediately after spawning the child, leaving only `cmux-cua.exe`
    /// in the process tree.
    ///
    ...
    /// flag or `cmux-cua autostart enable`, both of which prompt for
...
# logon. cmux-cua.exe is a CUI (console-subsystem) binary: without
...
# cmux-cua.exe daemon in the process tree.
$action = New-ScheduledTaskAction `
    -Execute 'powershell.exe' `
    -Argument "-NoProfile -WindowStyle Hidden -NonInteractive -Command `"Start-Process -FilePath '$env:CMUX_CUA_AS_EXE' -ArgumentList 'serve' -WindowStyle Hidden -WorkingDirectory '$env:USERPROFILE'`"" `
...
Unregister-ScheduledTask -TaskName 'cmux-cua-serve' -Confirm:$false -ErrorAction SilentlyContinue
Register-ScheduledTask -TaskName 'cmux-cua-serve' -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Description 'cmux-cua: serve daemon, auto-start at interactive logon, RunLevel=Highest for UWP/AppContainer support' | Out-Null

# Note: the uiAccess'd worker (`cmux-cua-uia.exe`) does NOT get its own
...
# (0x800710E0). Instead, `cmux-cua serve` itself spawns the sibling worker
...
"#;
    ...
    pub fn enable(exe: &str) -> Result<()> {
        ...
        // or a user running cmux-cua autostart enable from an admin
        ...
        eprintln!("cmux-cua: registering autostart at RunLevel=Highest needs admin.");
        eprintln!("cmux-cua: triggering UAC prompt — accept it to register the task.");
        ...
        if elevated_status.success() {
            ...
            Err(anyhow!(
                "self-elevation for autostart registration failed (exit {}). The UAC \
                 prompt was probably dismissed. Re-run `cmux-cua autostart enable` \
                 and accept the prompt, or run install.ps1 -AutoStart which has the \
                 same self-elevation flow. See https://github.com/manaflow-ai/cmux-cua/issues/1602.",
    ...
    pub fn disable() -> Result<()> {
        // Tear down any legacy `cmux-cua-uia` task from earlier autostart
    ...
    pub fn status() -> Result<Status> {
        ...
        // Registered — now check whether `cmux-cua serve` is running.
...
mod platform {
    ...
    const NOT_YET: &str =
        "cmux-cua autostart is currently Windows-only. macOS users: see \
         libs/cmux-cua/scripts/install-local.sh --autostart for the \
...
/// `cmux-cua autostart <subcommand>` entry point. Prints user-facing
...
pub fn run_autostart_cmd(subcommand: &str) {
    let (verb_result, success_text): (Result<()>, String) = match subcommand {
        "enable" => (enable(), format!(
            "Registered autostart entry '{TASK_NAME}'.\n  \
             cmux-cua serve will start at every interactive logon."
        )),
        ...
        "status" => match status() {
            ...
            Err(e) => {
                eprintln!("cmux-cua autostart status: {e}");
                std::process::exit(1);
            }
        },
        "kick" => (kick(), format!(
    ...
    match verb_result {
        ...
        Err(e) => {
            eprintln!("cmux-cua autostart {subcommand}: {e}");
            std::process::exit(1);
        }
    }
}
