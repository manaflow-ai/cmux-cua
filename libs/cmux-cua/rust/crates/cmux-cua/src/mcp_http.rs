...
//!
//! Why: over **stdio**, one `cmux-cua mcp` process is a single pipe, so all of
...
//! shared MCP dispatch (`cmux_cua_core::server::handle_request`) → an
...
use cmux_cua_core::protocol::{Request, Response};
use cmux_cua_core::server::handle_request;
use cmux_cua_core::tool::ToolRegistry;
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tokio::net::{TcpListener, TcpStream};
use tracing::{debug, info, warn};

/// Resolve the configured HTTP MCP port: `CMUX_CUA_MCP_HTTP_PORT` (> 0), or
/// `None` (disabled — the daemon spawns the listener only when this is set).
pub fn configured_port() -> Option<u16> {
    std::env::var("CMUX_CUA_MCP_HTTP_PORT")
        .ok()
