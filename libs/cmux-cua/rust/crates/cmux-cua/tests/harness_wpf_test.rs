...
//!
//! Pairs of test apps live under `libs/cmux-cua/tests/fixtures/`. They
//! publish into `libs/cmux-cua/rust/test-apps/harness-{wpf,winui3}/` and
...
use cmux_cua_testkit::sentinel::{run_with_background_oracles, ForegroundSentinel};
use cmux_cua_testkit::{ax, harness_app, spawn_in_job, Driver, McpDriver, ToolResponse};
...
/// Spin up a fresh cmux-cua + harness pair, run the closure, then tear
...
fn harness_wpf_checkbox_toggle() {
    run_foreground_case(
        ...
        |pid, wid, driver| {
            ...
            // cmux-cua's click tool tries UIA Invoke first; for elements that
