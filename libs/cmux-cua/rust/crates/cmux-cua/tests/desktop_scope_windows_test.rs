...
//!
//! Desktop-scope is cmux-cua's *foreground*, vision-only, **screen-absolute**
...
//! the one MCP server we spawn here (not across separate `cmux-cua call`
...
//!   cargo test -p cmux-cua --test desktop_scope_windows_test -- --ignored --nocapture --test-threads=1
...
use cmux_cua_testkit::{harness_app, Driver, McpDriver};
...
fn snapshot(driver: &mut McpDriver, pid: u32, wid: u64) -> cmux_cua_testkit::ToolResponse {
    driver.call(
        "get_window_state",
...
fn element_center(state: &cmux_cua_testkit::ToolResponse, id: &str) -> (i64, i64) {
