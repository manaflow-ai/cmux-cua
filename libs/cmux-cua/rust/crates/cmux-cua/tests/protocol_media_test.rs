...
fn recording_session() {
    ...
    let tmp_dir =
        std::env::temp_dir().join(format!("cmux-cua-rec-test-{}", std::process::id()));
...
fn recording_screenshot_capture() {
    ...
    let tmp_dir = std::env::temp_dir().join(format!("cmux-cua-rec-ss-{}", std::process::id()));
    let tmp_str = tmp_dir.to_string_lossy().to_string();
...
fn start_recording_record_video_flag_accepted() {
    ...
    let tmp_dir = std::env::temp_dir().join(format!("cmux-cua-recvid-{}", std::process::id()));
    let tmp_str = tmp_dir.to_string_lossy().to_string();
...
fn replay_trajectory() {
    ...
    let traj_dir =
        std::env::temp_dir().join(format!("cmux-cua-replay-{}", std::process::id()));
...
fn click_debug_image_out() {
    ...
    let dbg_path =
        std::env::temp_dir().join(format!("cmux-cua-dbg-{}.png", std::process::id()));
