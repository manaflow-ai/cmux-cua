...
impl ExplicitProxyDriver {
    fn spawn(socket: &PathBuf, compat: bool) -> Self {
        let mut command = Command::new(env!("CARGO_BIN_EXE_cmux-cua"));
        command
            .arg("mcp")
            .arg("--socket")
            .arg(socket)
...
impl CompatDriver {
    fn spawn(compat: bool) -> Self {
        ...
        let mut child = command.spawn().expect("spawn cmux-cua");
        let stdin = child.stdin.take().unwrap();
        let stdout = BufReader::new(child.stdout.take().unwrap());
        let mut driver = Self {
            child,
        ...
        if compat {
            ...
            assert_eq!(initialized["result"]["serverInfo"]["name"], "cmux-cua");
...
impl DaemonDriver {
    ...
    fn spawn_with_environment(
        ...
        command
            ...
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .env_remove("CMUX_CUA_CODEX_ALLOW_UNVERIFIED_CLIENT")
        ...
        if allow_unverified_client {
            command.env("CMUX_CUA_CODEX_ALLOW_UNVERIFIED_CLIENT", "1");
        }
        command.envs(environment.iter().copied());
        let mut child = command.spawn().expect("spawn cmux-cua daemon");
...
fn default_profile_socket(home: &std::path::Path, compat: bool) -> PathBuf {
    home.join("Library/Caches/cmux-cua").join(if compat {
        "cmux-cua-codex-computer-use.sock"
    } else {
        "cmux-cua.sock"
    })
}
...
fn spawn_default_profile_daemon(home: &std::path::Path, compat: bool) -> DaemonDriver {
    ...
    command
        ...
        .arg("--no-overlay")
        .env("HOME", home)
        .env_remove("CMUX_CUA_MCP_HTTP_PORT")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::piped());
    if compat {
...
fn hosted_codex_daemon_keeps_cursor_control_private_and_host_authenticated() {
    ...
    let daemon = DaemonDriver::spawn_with_environment(
        ...
        true,
        &[
            ("CMUX_CUA_SOCKET_AUTH_TOKEN", "agent-secret-A1B2C3"),
            (
...
fn raw_client_cannot_reuse_a_live_compat_session_without_its_broker_token() {
    ...
    let mut proxy = Command::new(env!("CARGO_BIN_EXE_cmux-cua"))
        .arg("mcp")
        .arg("--socket")
        .arg(&socket)
        ...
        .arg("--no-overlay")
        .env("CMUX_CUA_MCP_FORCE_PROXY", "1")
        .env("CUA_LOG", "cmux_cua::proxy=debug")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
