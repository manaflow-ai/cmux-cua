...
//!
//! ```text
//! cargo test -p cmux-cua --test cross_platform_behavior_test -- --ignored \
...
use cmux_cua_testkit::ax::{element_index_by_id, element_index_containing};
...
use cmux_cua_testkit::sentinel::ForegroundSentinel;
use cmux_cua_testkit::{
    ...
    F: FnOnce(&mut Fixture) -> Observation,
{
    ...
    match outcome {
        Ok(observation) => {
            ...
            if result.test_status == cmux_cua_testkit::e2e::TestStatus::Fail {
...
fn launch_host_with_evidence(spec: &HostSpec, scenario: &str, evidence: &mut Evidence) -> Fixture {
    ...
    let mut driver = spawn_driver(&recording_label).unwrap_or_else(|| {
        panic!(
            "cmux-cua could not be started for the required {} fixture",
            spec.name
        )
    });
...
fn shared_case(spec: &HostSpec, action: &str, addressing: &str, delivery: &str) -> CaseSpec {
    ...
    if delivery_kind == Delivery::Background {
        ...
        if cmux_cua_testkit::e2e::DisplayServer::current()
            != cmux_cua_testkit::e2e::DisplayServer::Wayland
        {
    ...
    let mut route = shared_web_route(
        cmux_cua_testkit::e2e::Platform::current(),
        cmux_cua_testkit::e2e::DisplayServer::current(),
        action,
        targeting,
        delivery_kind,
    )
    ...
    if cfg!(target_os = "windows")
        ...
        && delivery_kind == Delivery::Background
    {
        route = cmux_cua_testkit::e2e::DriverRoute::PostMessage;
    } else if cfg!(target_os = "macos")
        && targeting == Targeting::Px
        && delivery_kind == Delivery::Background
        && matches!(action, "left_click" | "child_window")
    {
        route = cmux_cua_testkit::e2e::DriverRoute::MacosAxAction;
    }
