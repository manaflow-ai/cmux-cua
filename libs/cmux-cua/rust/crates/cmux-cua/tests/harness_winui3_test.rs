...
use cmux_cua_testkit::ax::element_index_by_id;
...
use cmux_cua_testkit::{harness_app, spawn_in_job, Driver, McpDriver};
...
/// cmux-cua `click` now tries Invoke → Toggle → SelectionItem →
...
/// Documents cmux-cua gap: WinUI3 Slider implements
/// `RangeValuePattern`, not `ValuePattern`. cmux-cua's `set_value` tool
/// queries `ValuePatternId` specifically (impl_.rs:2640), so it silently
/// fails on RangeValuePattern-only elements. Real fix: cmux-cua should
...
/// set_value. cmux-cua's UIA cache now pre-fetches RangeValuePattern,
