...
#[cfg(target_os = "linux")]
use cmux_cua_testkit::e2e::DisplayServer;
use cmux_cua_testkit::e2e::{write_environment_from_env, EnvironmentRecord};
use cmux_cua_testkit::{driver_binary, harness_app, spawn_in_job, Driver, McpDriver};
...
fn spawn_driver() -> McpDriver {
    #[cfg(target_os = "macos")]
    {
        McpDriver::spawn_macos_daemon_proxy_named("environment-preflight")
            .expect("installed CmuxCua daemon is not available")
    }
    #[cfg(not(target_os = "macos"))]
    {
        McpDriver::spawn_named("environment-preflight")
            .expect("source-built cmux-cua could not be started")
    }
}

fn has_image(response: &cmux_cua_testkit::ToolResponse) -> bool {
