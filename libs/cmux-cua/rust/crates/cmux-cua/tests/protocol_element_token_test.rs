...
//! by spawning the real `cmux-cua` binary and walking `tools/list`.
//! Unit tests for the resolution logic live in
//! `cmux-cua-core::element_token` and the per-platform tool modules —
...
use cmux_cua_testkit::driver_binary;
...
fn token_accepting_tools_advertise_element_token_in_schema_and_capabilities() {
    ...
    // `default_capabilities_for` map in cmux-cua-core.
