...
use cmux_cua_testkit::RawDriver;

#[test]
#[cfg(any(target_os = "macos", target_os = "windows"))]
fn concurrent_clients() {
    //! Verify two concurrent cmux-cua processes both respond correctly.
    //! This tests the "multiple cmux-cua processes" scenario relevant to
...
fn concurrent_clients_with_cursor_moves() {
    //! Two concurrent cmux-cua processes, each driving their own cursor, should not crash.
    ...
    for (i, d) in drivers.iter_mut().enumerate() {
        ...
        let resp = d.recv();
        assert_eq!(resp["id"], 99, "cmux-cua process {i} crashed during concurrent cursor test");
    }
}
...
fn concurrent_multi_driver_isolation() {
    //! Two cmux-cua processes running simultaneously with different cursor IDs.
...
fn multi_cursor_instance_state() {
    ...
    let resp = d.recv();
    assert_eq!(resp["id"], 99, "cmux-cua crashed during multi-cursor test");
}
...
fn overlay_move_cursor_stays_alive() {
    ...
    let resp = d.recv();
    assert_eq!(resp["id"], 99, "cmux-cua crashed during overlay move_cursor test");
}
