...
//! loudly if cmux-cua later closes the underlying gap, prompting
...
//!   cargo test -p cmux-cua --test harness_libreoffice_test -- --ignored --nocapture
//!
...
use cmux_cua_testkit::{Driver, McpDriver};
...
fn setup() -> Option<LoSession> {
    ...
    // the real writer is a child. Poll list_apps via cmux-cua to
    // find it.
...
/// cmux-cua see VCL toolbar SplitButtons' dropdown halves.
///
...
/// the BUTTONDROPDOWN role, and cmux-cua maps it to
...
///   - (2) cmux-cua click tool's MSAA dispatch broke (check
