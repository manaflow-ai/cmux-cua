# cmux-cua Rust integration tests
...
```bash
cargo test -p cmux-cua --test protocol_handshake_test
cargo test -p cmux-cua --test harness_appkit_test -- --ignored --nocapture
cargo test -p cmux-cua --test desktop_scope_macos_test -- --ignored --nocapture
```
