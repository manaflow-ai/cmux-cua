...
use cmux_cua_testkit::{harness_app, spawn_in_job, Driver, FixtureJournal, McpDriver, ToolResponse};
...
/// Launch the harness exe + a cmux-cua child with `CMUX_CUA_CDP_PORT`
    ...
    F: FnOnce(i64, u64, &mut McpDriver, &FixtureJournal),
{
    ...
    execute_case(case, |evidence| {
        ...
        let mut driver = McpDriver::spawn_named_with_env(
            &cell_id,
            &[("CMUX_CUA_CDP_PORT", cdp_port_string.as_str())],
        )
...
fn harness_electron_page_tool() {
    ...
    // cmux-cua's page tool now reaches Electron's CDP successfully.
    run_web_case(
        "electron",
        "page_execute",
        electron_exe(),
