...
//! SAME cmux-cua interface as the Windows `desktop_scope_windows_test`:
...
//! harness). Run:
//!   cargo test -p cmux-cua --test desktop_scope_macos_test -- --ignored --nocapture --test-threads=1
...
fn launch(driver: &mut McpDriver) -> Option<(u32, u64)> {
    ...
    let child = match cmux_cua_testkit::spawn_in_job(
        Command::new(&exe)
            .stdout(Stdio::null())
            .stderr(Stdio::null()),
    ) {
