...
//!
//!   - **CLI** (`CliDriver`) is stateless — each `cmux-cua call` is its own
...
//!   cargo test -p cmux-cua --test transport_config_persistence_test -- --ignored --nocapture

use cmux_cua_testkit::{CliDriver, Driver, McpDriver};
...
/// are independent `cmux-cua call` invocations.
#[test]
#[ignore]
