//! cursor-overlay — shared types and math for the cmux-cua cursor overlay.
//!
...
pub use render_state::{
    draw_default_arrow, paint_cursor, render_frame, FocusRect, MotionTickResult, RenderStateCore,
};
...
pub struct CursorConfig {
    ...
    /// Launch-time gradient default from `CMUX_CUA_CURSOR_GRADIENT`.
    pub gradient_colors: Vec<[u8; 4]>,

    /// Launch-time halo default from `CMUX_CUA_CURSOR_BLOOM`.
    pub bloom_color: Option<[u8; 4]>,

    /// Launch-time per-instance label default from `CMUX_CUA_CURSOR_LABEL`.
...
impl AgentCursorDefaults {
    ...
    pub fn parse_values(
        ...
        if let Some(value) = gradient {
            ...
            match stops {
                ...
                None => warnings.push(format!(
                    "invalid CMUX_CUA_CURSOR_GRADIENT={value:?}; expected comma-separated #RGB/#RRGGBB stops"
                )),
            }
        }
        ...
        if let Some(value) = bloom {
            match (!value.trim().is_empty()).then(|| parse_hex_color(value.trim())).flatten() {
                ...
                None => warnings.push(format!(
                    "invalid CMUX_CUA_CURSOR_BLOOM={value:?}; expected #RGB or #RRGGBB"
                )),
            }
        }
        ...
        if let Some(value) = label {
            ...
            if value.is_empty() {
                warnings.push(
                    "invalid CMUX_CUA_CURSOR_LABEL: label must not be empty".to_owned(),
                );
            } else {
    ...
    pub fn from_env() -> Self {
        let gradient = std::env::var("CMUX_CUA_CURSOR_GRADIENT").ok();
        let bloom = std::env::var("CMUX_CUA_CURSOR_BLOOM").ok();
        let label = std::env::var("CMUX_CUA_CURSOR_LABEL").ok();
        ...
        for warning in warnings {
            eprintln!("[cmux-cua] warning: {warning}");
        }
        defaults
    }
}
...
/// one-shot `cmux-cua call` path is backward compatible.
