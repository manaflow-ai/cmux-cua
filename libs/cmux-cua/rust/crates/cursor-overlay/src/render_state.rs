...
/// Motion milestones consumed by platform renderers and input dispatchers.
///
/// `close_enough` is intentionally earlier than `arrived`: an input action can
/// be delivered once the visible cursor is near its target while the compositor
/// continues the final glide/spring. This mirrors the native Codex cursor's
/// separate next-interaction and animation-completion callbacks.
...
// Conservative defaults matching the intent of Sky's private
// CloseEnoughConfiguration. The distance guard makes short moves responsive;
// the progress guard prevents a long glide from holding input until its final
// spring/settle phase.
const CLOSE_ENOUGH_PROGRESS: f64 = 0.82;
const CLOSE_ENOUGH_DISTANCE: f64 = 28.0;
...
pub struct RenderStateCore {
    ...
    pub pressed: bool,
    /// Whether the current path has already emitted its close-enough milestone.
    close_enough_fired: bool,
...
impl RenderStateCore {
    ...
    pub fn tick_motion(&mut self, dt: f64) -> bool {
        self.tick_motion_events(dt).arrived
    }

    /// Advance the configurable renderer and return both interaction milestones.
    pub fn tick_motion_events(&mut self, dt: f64) -> MotionTickResult {
        let spring_k = self.motion.spring * 400.0;
        let spring_c = self.motion.spring * 20.0;

        let mut result = MotionTickResult::default();

        if let Some(ref p) = self.path {
            let path_len = p.length.max(1.0);
            let path_frac = (self.dist / path_len).clamp(0.0, 1.0);
            if !self.close_enough_fired
                && (path_frac >= CLOSE_ENOUGH_PROGRESS
                    || (path_len - self.dist).max(0.0) <= CLOSE_ENOUGH_DISTANCE)
            {
                self.close_enough_fired = true;
                result.close_enough = true;
            }
    ...
    /// Advance the animation by `dt` seconds using the Swift reference motion
    /// profile.  The default values remain `peakSpeed=900`, `minStart=300`,
    /// and `minEnd=200`, while launch-time speed overrides are honored on
    /// macOS just as they are on the other platform renderers.
    ///
    ...
    pub fn tick_swift_constants(&mut self, dt: f64) -> bool {
        self.tick_swift_constants_events(dt).arrived
    }

    /// Advance the macOS reference renderer and return both interaction milestones.
    pub fn tick_swift_constants_events(&mut self, dt: f64) -> MotionTickResult {
        const DEFAULT_MIN_END_SPEED: f64 = 200.0;
        const SPRING_K: f64 = 400.0;
        const SPRING_C: f64 = 17.0;
        const SPRING_OVERSHOOT: f64 = 0.8;

        let peak_speed = self.motion.peak_speed;
        let min_start_speed = self.motion.min_start_speed;
        let min_end_speed = self.motion.min_end_speed;
        ...
        if let Some(ref p) = self.path {
            ...
            if self.dist >= path_len {
                ...
                // proportionally to a short duration.
                let impulse = if self.motion.glide_duration_ms > 0.0 {
                    // Fixed-duration glides intentionally keep the stock
                    // landing impulse so a speed multiplier changes travel
                    // pacing without making the visual landing explosive.
                    DEFAULT_MIN_END_SPEED
                } else {
                    current_speed
                };
                self.spring = Some(Spring {
...
mod glide_duration_tests {
    ...
    fn arrival_secs_with_speed(multiplier: f64, dist_pts: f64) -> f64 {
        let mut config = CursorConfig::default();
        config.motion = config.motion.scaled_by(multiplier);
        config.motion.idle_hide_ms = 0.0;
    ...
    #[test]
    fn macos_speed_multiplier_changes_speed_based_arrival() {
        let stock = arrival_secs_with_speed(1.0, 1_000.0);
        let fast = arrival_secs_with_speed(1.75, 1_000.0);
        assert!(fast < stock * 0.8, "stock={stock} fast={fast}");
    }

    #[test]
    fn close_enough_milestone_precedes_full_arrival() {
        let mut core = RenderStateCore::new(CursorConfig::default());
        core.motion.idle_hide_ms = 0.0;
        core.place_at(0.0, 0.0);
        core.path = Some(PathPlanner::plan(
            0.0, 0.0, 0.0, 1_000.0, 0.0, 0.0, 0.0, 80.0,
        ));
        let dt = 1.0 / 240.0;
        let mut saw_close = false;
        let mut saw_arrival_after_close = false;
        for _ in 0..200_000 {
            let tick = core.tick_swift_constants_events(dt);
            saw_close |= tick.close_enough;
            if tick.arrived {
                saw_arrival_after_close = saw_close;
                break;
            }
        }
        assert!(saw_close);
        assert!(saw_arrival_after_close);
    }
}
