...
//! diagnostic call and never have to know cmux-cua internals: specific
//! MCP tool names, TCC field names, bundle IDs, per-platform check
//! matrix. cmux-cua owns the health model entirely; consumers stay
...
pub fn text_summary(report: &Report) -> String {
    ...
    let mut lines = vec![format!(
        "{overall_icon} cmux-cua {} on {} — {overall_word}",
...
fn def() -> &'static ToolDef {
    DEF.get_or_init(|| ToolDef {
        ...
        description: r#"Single-call end-to-end driver diagnostics. Designed to let downstream consumers ship one stable call instead of stitching together check_permissions, doctor, version, bundle attribution, and a screenshot probe. cmux-cua owns the health model; consumers stay thin.
