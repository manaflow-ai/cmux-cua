...
//! `$CMUX_CUA_STATE_DIR/<driver_pid>.cursor.json` and is refreshed on every
...
/// host should paint.
const GRADIENT_ENV: &str = "CMUX_CUA_CURSOR_GRADIENT";
const BLOOM_ENV: &str = "CMUX_CUA_CURSOR_BLOOM";
const LABEL_ENV: &str = "CMUX_CUA_CURSOR_LABEL";

const SCHEMA_VERSION: u8 = 1;

/// Host cursor branding read once from `CMUX_CUA_CURSOR_*`. Colours are
...
impl CursorFeed {
    /// Build the feed whenever `CMUX_CUA_STATE_DIR` is set, regardless of
