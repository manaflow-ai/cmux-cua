// cmux-cua-uia: Windows uiAccess-elevated tool worker.
//
// Listens on \\.\pipe\cmux-cua-uia for line-delimited JSON requests with the
// same shape as cmux-cua's daemon pipe (\\.\pipe\cmux-cua), so cmux-cua's
...
//
// The protocol is intentionally byte-identical to cmux-cua/serve.rs so that
// the existing client code in cli.rs::run_call can talk to either pipe.

#[cfg(not(target_os = "windows"))]
fn main() {
    eprintln!("cmux-cua-uia: Windows-only. (this binary is a no-op on non-Windows hosts.)");
    std::process::exit(0);
}
...
#[cfg(target_os = "windows")]
const PIPE_PATH: &str = r"\\.\pipe\cmux-cua-uia";
...
async fn async_main() -> anyhow::Result<()> {
    ...
    eprintln!("cmux-cua-uia: {tool_count} tools registered; listening on {PIPE_PATH}");
...
async fn handle_request(
    reg: &cmux_cua_core::tool::ToolRegistry,
    req: PipeRequest,
) -> PipeResponse {
        ...
        "call" => {
            ...
            if is_err {
                let msg = result.content.iter()
                    .filter_map(|c| if let cmux_cua_core::protocol::Content::Text { text, .. } = c {
                        Some(text.as_str())
                    } else { None })
                    .collect::<Vec<_>>().join("\n");
