...
//! shows what the cmux-cua agent just did: a post-action screenshot
...
//! crate) and the registration pattern used by `cmux_cua_core::video`
...
/// Canonical `~/.cmux-cua/config.json` path matching what the per-platform
...
pub fn default_config_path() -> Option<std::path::PathBuf> {
    std::env::var_os("HOME")
        ...
        .map(|h| std::path::PathBuf::from(h).join(".cmux-cua").join("config.json"))
}

/// Read a single key from `~/.cmux-cua/config.json` as a raw JSON value,
...
/// across stateless `cmux-cua call` invocations.
...
/// Merge a single `key`/`value` into `~/.cmux-cua/config.json`,
...
pub struct PipConfig {
    ...
    /// one place. Defaults to "cmux-cua — agent view (experimental)".
    pub title: String,
}
...
impl Default for PipConfig {
    fn default() -> Self {
        Self {
            ...
            title: "cmux-cua — agent view (experimental)".to_owned(),
        }
    }
}
