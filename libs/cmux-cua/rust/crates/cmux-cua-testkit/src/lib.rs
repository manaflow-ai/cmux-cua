//! Shared test harness for cmux-cua integration tests.
//!
...
//! cmux-cua is driven two ways, and a test should be able to target either:
//!   - **MCP** ([`McpDriver`]) — one long-lived `cmux-cua` server over stdio
//!     JSON-RPC. State (e.g. `set_config`) persists for the connection.
//!     Returns the `{"result":{"content",…,"structuredContent"}}` envelope.
//!   - **CLI** ([`CliDriver`]) — a stateless `cmux-cua call <tool> <json>`
