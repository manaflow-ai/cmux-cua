//! MCP transport: one long-lived `cmux-cua` server over stdio JSON-RPC.
...
/// A spawned cmux-cua MCP server. State (e.g. `set_config`) persists for the
...
impl McpDriver {
    ...
    ///
    /// A raw `target/debug/cmux-cua` MCP process initializes AppKit for the
    /// cursor overlay and can lose Screen Recording-attributed window titles even
    /// when the shell's one-shot `cmux-cua call` path can see them. The installed
    ...
    fn spawn_macos_daemon_proxy_internal(recording_label: Option<&str>) -> Option<Self> {
        ...
        let socket = format!("{home}/Library/Caches/cmux-cua/cmux-cua.sock");
        if std::os::unix::net::UnixStream::connect(&socket).is_err() {
            eprintln!(
                "[testkit] CmuxCua daemon not listening at {socket} — \
                 run `./scripts/install-local.sh` and `open -n -g -a 'cmux Computer Use' --args serve`; skipping"
            );
            return None;
        }
        Self::spawn_internal(&[("CMUX_CUA_MCP_FORCE_PROXY", "1")], recording_label)
    }
