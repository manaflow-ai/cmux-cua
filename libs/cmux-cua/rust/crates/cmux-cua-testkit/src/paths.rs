...
//! under test (`crates/cmux-cua`), so `workspace_root()` resolves the same
//! whether called from the test or from here.

use std::path::PathBuf;

/// The Rust workspace root (`libs/cmux-cua/rust`).
...
/// The built `cmux-cua` binary (`.exe` on Windows), preferring a release
...
mod tests {
    ...
    fn relocated_runner_overrides_are_respected() {
        ...
        with_env("CUA_TEST_DRIVER_BIN", "/tmp/cmux-cua", || {
            assert_eq!(driver_binary(), PathBuf::from("/tmp/cmux-cua"));
        });
