//! Parity check for `cmux-cua status` + `cmux-cua stop` CLI subcommands.
...
fn main() {
    ...
    assert_eq!(lines[0], "cmux CUA daemon is running",
    ...
    assert!(stderr3.contains("cmux CUA daemon is not running"),
    ...
    assert!(stderr4.contains("cmux CUA daemon is not running"),
