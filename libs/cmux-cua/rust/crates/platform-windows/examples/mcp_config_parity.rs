...
fn main() {
    let exe = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent().unwrap()
        .parent().unwrap()
        .join("target/debug/cmux-cua.exe");

    let cases = [
        // Each (args, expected substrings)
        (vec![], vec!["\"mcpServers\"", "\"cmux-cua\"", "\"command\":", "\"args\": [\"mcp\"]"]),
        (vec!["--client", "claude"], vec!["claude mcp add --transport stdio cmux-cua"]),
        (vec!["--client", "codex"],  vec!["codex mcp add cmux-cua"]),
        (vec!["--client", "cursor"], vec!["\"type\": \"stdio\"", "\"mcpServers\""]),
        (vec!["--client", "openclaw"], vec!["openclaw mcp set cmux-cua"]),
        (vec!["--client", "opencode"], vec!["opencode.ai/config.json", "\"type\": \"local\""]),
        (vec!["--client", "hermes"], vec!["mcp_servers:", "cmux-cua:"]),
