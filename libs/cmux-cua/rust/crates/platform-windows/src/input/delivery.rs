//! Shared delivery-mode logic for cmux-cua Windows input tools.
//!
...
//!   the default because cmux-cua's value proposition is that input never
...
pub fn delivery_mode_schema() -> Value {
    ...
    cmux_cua_core::tool_schema::delivery_mode_schema_with(
...
pub fn background_unavailable_error(
    hwnd: u64,
    kind: EventKind,
) -> cmux_cua_core::protocol::ToolResult {
    ...
    cmux_cua_core::protocol::ToolResult::error(text).with_structured(serde_json::json!({
...
mod tests {
    ...
    fn delivery_mode_defaults_to_background() {
        ...
        // all resolve to Background. This is the cmux-cua no-foreground-by-
    ...
    fn background_unavailable_with_cause_preserves_actuator_failure() {
        ...
        let text = match &result.content[0] {
            cmux_cua_core::protocol::Content::Text { text, .. } => text,
