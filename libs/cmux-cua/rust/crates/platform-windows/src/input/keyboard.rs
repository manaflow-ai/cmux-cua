...
/// processes when not driven by user input. The `cmux-cua-uia` worker runs
        ...
        if actual_fg != target {
            ...
            bail!(
                "Foreground swap to target HWND {:?} was rejected by Windows \
                 ...
                 / spawn the cmux-cua-uia worker (UIAccess-manifested PE) \
        ...
        if sent as usize != events.len() {
            ...
            bail!(
                "SendInput inserted only {sent} of {} events. Likely cause: \
                 ...
                 hotkey through the cmux-cua-uia worker.",
                events.len()
            );
        }
        ...
        if actual_fg != target {
            bail!(
                "Foreground swap to target HWND {:?} was rejected by Windows \
                 ...
                 the cmux-cua-uia worker (UIAccess-manifested PE) and route \
