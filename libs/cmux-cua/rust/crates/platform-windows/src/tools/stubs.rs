...
use cmux_cua_core::{protocol::ToolResult, tool::{Tool, ToolDef, ToolRegistry}};
...
stub_tool!(get_window_state_m, GetWindowStateTool, "get_window_state",
    ...
    serde_json::json!({"type":"object","required":["pid","window_id"],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"pid":{"type":"integer"},"window_id":{"type":"integer"},"query":{"type":"string"},"capture_mode": cmux_cua_core::capture_mode::capture_mode_schema(),"include_screenshot":{"type":"boolean","description":"Default true — returns a grounding screenshot alongside the tree. Set false to skip the grab and return tree only (the cheap path for re-indexing before an element ax action)."}},"additionalProperties":false}));
...
stub_tool!(click_m, ClickTool, "click",
    ...
    serde_json::json!({"type":"object","required":[],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"pid":{"type":"integer"},"window_id":{"type":"integer"},"element_index": cmux_cua_core::tool_schema::element_index_schema(),"element_token": cmux_cua_core::tool_schema::element_token_schema(),"x":{"type":"number"},"y":{"type":"number"},"button": cmux_cua_core::tool_schema::button_schema(),"modifier": cmux_cua_core::tool_schema::modifier_schema(),"from_zoom":{"type":"boolean"}},"additionalProperties":false}));
...
stub_tool!(type_text_m, TypeTextTool, "type_text",
    ...
    serde_json::json!({"type":"object","required":["pid","text"],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"pid":{"type":"integer"},"text":{"type":"string"},"element_index": cmux_cua_core::tool_schema::element_index_schema(),"window_id":{"type":"integer"},"x":{"type":"number"},"y":{"type":"number"}},"additionalProperties":false}));
...
stub_tool!(press_key_m, PressKeyTool, "press_key",
    ...
    serde_json::json!({"type":"object","required":["pid","key"],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"pid":{"type":"integer"},"key":{"type":"string"},"modifiers":{"type":"array","items":{"type":"string"}},"window_id":{"type":"integer"},"element_index": cmux_cua_core::tool_schema::element_index_schema(),"x":{"type":"number"},"y":{"type":"number"}},"additionalProperties":false}));

stub_tool!(hotkey_m, HotkeyTool, "hotkey",
    "Press a combination of keys simultaneously, e.g. [\"cmd\",\"c\"] for Copy.",
    serde_json::json!({"type":"object","required":["pid","keys"],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"pid":{"type":"integer"},"keys":{"type":"array","items":{"type":"string"},"minItems":2},"x":{"type":"number"},"y":{"type":"number"}},"additionalProperties":false}));

stub_tool!(set_value_m, SetValueTool, "set_value",
    "Set the value of an AX/UIA element (text field, dropdown, checkbox).",
    serde_json::json!({"type":"object","required":["pid","window_id","element_index","value"],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"pid":{"type":"integer"},"window_id":{"type":"integer"},"element_index": cmux_cua_core::tool_schema::element_index_schema(),"value":{"type":"string"}},"additionalProperties":false}));
...
stub_tool!(scroll_m, ScrollTool, "scroll",
    ...
    serde_json::json!({"type":"object","required":["direction"],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"pid":{"type":"integer"},"direction":{"type":"string","enum":["up","down","left","right"]},"by":{"type":"string","enum":["line","page"]},"amount":{"type":"integer","minimum":1,"maximum":50},"window_id":{"type":"integer"},"element_index": cmux_cua_core::tool_schema::element_index_schema()},"additionalProperties":false}));
    ...
    impl MoveCursorTool {
        pub fn def_static() -> &'static ToolDef {
            DEF.get_or_init(|| ToolDef {
                ...
                input_schema: serde_json::json!({"type":"object","required":["x","y"],"properties":{"session": cmux_cua_core::tool_schema::session_schema(),"x":{"type":"number"},"y":{"type":"number"},"cursor_id":{"type":"string"}},"additionalProperties":false}),
