...
impl Tool for ListAppsTool {
    ...
    async fn invoke(&self, _args: Value) -> ToolResult {
        ...
        let apps = tokio::task::spawn_blocking(|| -> Vec<serde_json::Value> {
            ...
            // log level — enable with `RUST_LOG=cmux_cua=debug` when debugging
...
impl Tool for ClickTool {
    fn def(&self) -> &ToolDef {
        CLICK_DEF.get_or_init(|| ToolDef {
            ...
            description: "Left-click against a target pid. **Prefer `element_index` over \
                ...
                default), cmux-cua FIRST does a UIA hit-test at the resolved \
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if let Some(idx) = elem_idx {
            ...
            let result = tokio::task::spawn_blocking(move || -> anyhow::Result<String> {
                ...
                if use_uia_invoke {
                    ...
                    if let Some(element_guard) = state_clone.element_cache.get_element_retained(pid, hwnd, idx) {
                        ...
                        // fallthroughs, cmux-cua couldn't drive WinUI3
            ...
            // (`src/CmuxCua.Win/Tools/ClickTool.cs::InvokePixelClickAsync`):
            //
    ...
    fn def(&self) -> &ToolDef {
        HOTKEY_DEF.get_or_init(|| ToolDef {
            ...
            description: "Press a combination of keys simultaneously — e.g. `[\"ctrl\", \"c\"]` \
                ...
                `cmux-cua-uia.exe` worker pipe when both daemons are running.\n\n\
...
impl Tool for SetValueTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let (hwnd, idx) = match resolved {
            ...
            cmux_cua_core::element_token::ResolvedElement::None => {
                return ToolResult::error(
                    "Missing required integer fields pid, window_id, and element_index.",
                )
            }
...
///
/// With permonitorv2 DPI awareness (set in cmux-cua.manifest),
...
impl Tool for CheckPermissionsTool {
    fn def(&self) -> &ToolDef {
        PERMS_DEF.get_or_init(|| ToolDef {
            ...
            description: "Check required permissions for cmux-cua on Windows.".into(),
            input_schema: json!({"type":"object","properties":{},"additionalProperties":false}),
            read_only: true,
            destructive: false,
            idempotent: true,
    ...
    async fn invoke(&self, _args: Value) -> ToolResult {
        ...
        // important cases that matter to cmux-cua:
        //
        ...
        //      common case for cmux-cua's autostart task on non-RID-500
                ...
                let msg = if focused {
                    ...
                    format!(
                        "✅ bring_to_front: pid {pid} hwnd 0x{hwnd:x} raised to the visible front \
                         ...
                         land; for keyboard focus run the cmux-cua-uia worker."
                    )
                } else {
                    format!(
...
// `cmux-cua autostart kick`) sees about a given pid's top-level windows.
...
impl Tool for DebugWindowInfoTool {
    fn def(&self) -> &ToolDef {
        DEBUG_WI_DEF.get_or_init(|| ToolDef {
            ...
            description: "Diagnostic: dump everything cmux-cua sees about a pid's \
...
pub fn build_registry(compat: bool) -> ToolRegistry {
    ...
    r.register(Box::new(cmux_cua_core::page::PageTool::new(
