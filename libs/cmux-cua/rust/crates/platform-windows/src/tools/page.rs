...
//!        pre-existing `cmux-cua-eval` bookmark (creation flow is
//!        described in the docs; not yet automated end-to-end).  Zero
//!        config, no launch flag.
//!     2. **CDP fallback** — uses the shared `cmux_cua_core::cdp` helper.
//!        Requires `--remote-debugging-port=N` and `CMUX_CUA_CDP_PORT=N`.
//!        Returns an actionable error if neither path works.

use async_trait::async_trait;
use cmux_cua_core::page::{ClickElementResult, PageBackend};
...
impl PageBackend for WindowsPageBackend {
    ...
    async fn execute_javascript(
        ...
        //    Requires that a `cmux-cua-eval` bookmark exists in the
        ...
        let port: u16 = match std::env::var("CMUX_CUA_CDP_PORT")
            .ok()
            .and_then(|s| s.parse::<u16>().ok())
        {
            ...
            None => anyhow::bail!(
                "Chromium's `execute_javascript` on Windows tried the bookmark-URL UIA \
                 ...
                 and export CMUX_CUA_CDP_PORT=N before starting cmux-cua. \
                 Alternatively, create a bookmark named `cmux-cua-eval` (any URL — the \
        ...
        let result = cmux_cua_core::cdp::evaluate(port, javascript, true).await?;
    ...
    async fn execute_javascript_targeted(
        ...
        let port = cdp_port.or_else(|| {
            std::env::var("CMUX_CUA_CDP_PORT")
                .ok()
                .and_then(|value| value.parse::<u16>().ok())
        }).ok_or_else(|| anyhow::anyhow!(
            "targeted execute_javascript on Windows requires cdp_port or CMUX_CUA_CDP_PORT"
        ))?;
        let result = cmux_cua_core::cdp::evaluate_targeted(
            port,
            javascript,
            true,
            target_url_contains,
