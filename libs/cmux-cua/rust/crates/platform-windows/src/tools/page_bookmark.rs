...
//! single `cmux-cua-eval` bookmark URL (one caller's write would be
...
//! That violates the cmux-cua no-foreground contract. The current code
...
const BOOKMARK_NAME: &str = "cmux-cua-eval";
...
unsafe fn try_bookmark_exec_blocking(
        ...
        find_favorites_bar(&automation, &edge_window).ok_or_else(|| {
            anyhow!(
                "Favorites bar not visible after Ctrl+Shift+B PostMessage \
                 — cmux-cua's bookmark-URL JS exec needs the bar so \
                 we can invoke the `cmux-cua-eval` bookmark. The toggle \
    ...
    // 3) Find or create the cmux-cua-eval bookmark.
...
/// Drive the right-click → Edit dialog so the `cmux-cua-eval` bookmark
