...
unsafe fn target_is_obscured(target: HWND) -> bool {
    ...
    // overlay (e.g. the cmux-cua agent cursor) that's not actually opaque
        ...
        Err(primary_error) => {
            ...
            match crate::wgc::screenshot_window_via_wgc(hwnd) {
                Ok((pixels, width, height)) => Ok((
                    cmux_cua_core::image_utils::encode_bgra_to_png(
                        &pixels, width, height,
                    )?,
                    false,
                )),
...
unsafe fn screenshot_window_bytes_with_occlusion_unsafe(hwnd: u64) -> Result<(Vec<u8>, bool)> {
    ...
    if crate::input::is_xaml_host_hwnd(hwnd_raw) {
        match crate::wgc::screenshot_window_via_wgc(hwnd_raw) {
            Ok((pixels, w, h)) => {
                return Ok((
                    cmux_cua_core::image_utils::encode_bgra_to_png(&pixels, w, h)?,
        ...
        match screenshot_via_screen_region(hwnd) {
            Ok((pixels, w, h)) => {
                return Ok((
                    cmux_cua_core::image_utils::encode_bgra_to_png(&pixels, w as u32, h as u32)?,
                    occluded,
                ));
            }
            Err(e) => {
    ...
    if is_mostly_black_bgra(&pixels) {
        match crate::wgc::screenshot_window_via_wgc(hwnd_raw) {
            Ok((alt_pixels, w, h)) => {
                return Ok((
                    cmux_cua_core::image_utils::encode_bgra_to_png(&alt_pixels, w, h)?,
                    false,
                ));
            }
            Err(e) => {
        ...
        match screenshot_via_screen_region(hwnd) {
            Ok((alt_pixels, alt_w, alt_h)) => {
                return Ok((
                    cmux_cua_core::image_utils::encode_bgra_to_png(&alt_pixels, alt_w as u32, alt_h as u32)?,
                    occluded,
                ));
            }
            Err(e) => {
    ...
    Ok((cmux_cua_core::image_utils::encode_bgra_to_png(&pixels, w as u32, h as u32)?, false))
}
...
// `cmux_cua_core::image_utils::encode_bgra_to_png` which goes through
...
// `cmux_cua_core::image_utils::*` so all three platform crates call the
// same code. See `CMUX_CUA_DEDUP_AUDIT.md` for the full audit.
...
pub fn screenshot_display() -> Result<(String, u32, u32)> {
    ...
    let (w, h) = cmux_cua_core::image_utils::png_dimensions(&png_bytes)?;
    Ok((BASE64.encode(&png_bytes), w, h))
}
