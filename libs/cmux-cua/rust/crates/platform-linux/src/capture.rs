...
pub fn screenshot_window(xid: u64) -> Result<(String, u32, u32)> {
    // Try `import -window <xid> png:-` (ImageMagick).
    if let Ok(bytes) = capture_via_import(xid) {
        let (w, h) = cmux_cua_core::image_utils::png_dimensions(&bytes)?;
...
fn capture_via_xgetimage(xid: u64) -> Result<(String, u32, u32)> {
    ...
    let png = cmux_cua_core::image_utils::encode_rgba_to_png(&rgba, w, h)?;
...
// were extracted to `cmux_cua_core::image_utils` in the 2026-05 dedup
// audit so all three platforms call the same code. See
// `CMUX_CUA_DEDUP_AUDIT.md`. RGBA-encoding callers below now go
// through `cmux_cua_core::image_utils::encode_rgba_to_png`.

/// Capture the primary display (root window) as raw PNG bytes.
///
/// Dispatch:
/// - Native Wayland (`CMUX_CUA_ENABLE_WAYLAND=1` + Wayland session):
...
pub(crate) fn screenshot_display_bytes_x11() -> Result<Vec<u8>> {
    ...
    cmux_cua_core::image_utils::encode_rgba_to_png(&rgba, w, h)
}
