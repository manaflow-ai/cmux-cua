//! Linux platform backend for cmux-cua.
//!
...
pub fn no_display_hint() -> String {
    ...
    if is_wsl() {
        " — no usable display detected (WSL). WSLg provides $DISPLAY/$WAYLAND_DISPLAY \
         ...
         (e.g. VcXsrv) and export DISPLAY. cmux-cua needs a real display to capture and \
         report screen size."
            .to_owned()
    } else if neither_display_set {
        " — no usable display detected: neither $DISPLAY (X11) nor $WAYLAND_DISPLAY is set. \
         Start an X server / Wayland compositor (or Xvfb for headless) before running cmux-cua."
            .to_owned()
    } else {
        String::new()
    }
