...
/// backend.
pub const ENABLE_WAYLAND_ENV: &str = "CMUX_CUA_ENABLE_WAYLAND";
...
/// `CMUX_CUA_ENABLE_WAYLAND=1`. Any value other than empty / `0` /
/// `false` enables it.
...
/// cmux-cua automate apps in its **own** Wayland session on ANY host,
...
//
// When cmux-cua's nested compositor is `cua-compositor` (our patched wlroots,
// see nix/cmux-cua/compositor/), it exposes a line-protocol control socket at
