...
//! `cmux_cua_core::cdp` helper — Chromium/Electron must be launched with
//! `--remote-debugging-port=N` for that to succeed.

use async_trait::async_trait;
use cmux_cua_core::page::PageBackend;
...
impl PageBackend for LinuxPageBackend {
            ...
            .and_then(|s| s.parse::<u16>().ok())
        {
            ...
            None => anyhow::bail!(
                "Chromium's `execute_javascript` on Linux requires the browser launched \
                 ...
                 export CMUX_CUA_CDP_PORT=N before starting cmux-cua, or use \
    ...
    async fn execute_javascript_targeted(
        ...
        let port = cdp_port.or_else(|| {
            ...
            "targeted execute_javascript on Linux requires cdp_port or CMUX_CUA_CDP_PORT"
        ))?;
