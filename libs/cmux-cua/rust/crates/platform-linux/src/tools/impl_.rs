...
/// construction (i.e. on every fresh `cmux-cua call` process) so that a
    ...
    fn def(&self) -> &ToolDef {
        GWS_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({"type":"object","required":["pid","window_id"],"properties":{
                "session": cmux_cua_core::tool_schema::session_schema(),
                "pid":{"type":"integer"},
        ...
        match result {
            Ok(Ok((tree_opt, shot_opt, bounds))) => {
                ...
                if let Some(tr) = tree_opt {
                    ...
                    content.push(cmux_cua_core::protocol::Content::text(
                    ...
                    let elements: Vec<serde_json::Value> = tr
                        ...
                        .filter_map(|n| {
                            ...
                            let mut entry = json!({
                                ...
                                // integer index. See cmux-cua-core's
                                // `element_token` module for the format
                                // and validity contract.
                                "element_token": cmux_cua_core::element_token::token_for(snapshot_id, idx),
                                "role": n.role,
                                "depth": n.depth,
                            });
                    ...
                    structured["snapshot_id"] =
                        json!(cmux_cua_core::element_token::token_for(snapshot_id, 0)
                            .trim_end_matches(":0")
                            .to_string());
                    ...
                    if count == 0 {
                        ...
                        structured["degraded_reason"] = json!(
                            "atspi_tree_empty: the AT-SPI walk returned no actionable \
                             ...
                             run `cmux-cua doctor`), or the window is a non-AX surface \
                ...
                if let Some((b64_opt, file_path, w, h, orig_w)) = shot_opt {
                    ...
                    if let Some(b64) = b64_opt {
                        content.push(cmux_cua_core::protocol::Content::image_png(b64));
                    }
...
impl Tool for ClickTool {
        ...
        CLICK_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({
                ...
                "type":"object","required":[],"properties":{
                    ...
                    "x":{"type":"number"},
                    "y":{"type":"number"},
                    "element_index": cmux_cua_core::tool_schema::element_index_schema(),
                    "element_token": cmux_cua_core::tool_schema::element_token_schema(),
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let resolved = match cmux_cua_core::element_token::resolve_element_args(
            pid as i32,
            element_index_arg,
        ...
        let elem_idx_resolved: Option<usize> = match &resolved {
            cmux_cua_core::element_token::ResolvedElement::Element { element_index, .. } => {
                Some(*element_index)
            }
            cmux_cua_core::element_token::ResolvedElement::None => None,
        };
        let window_id_resolved: Option<u64> = match &resolved {
            ...
            cmux_cua_core::element_token::ResolvedElement::None => window_id_arg,
        };
...
impl Tool for TypeTextTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        // cmux_cua_core::text_sanitize docs for rationale.
        let text = cmux_cua_core::text_sanitize::strip_trailing_agent_protocol_tags(&text_raw)
            .into_owned();
        ...
        let (resolved_elem_idx, resolved_window_id) = match &resolved {
            cmux_cua_core::element_token::ResolvedElement::Element {
                element_index,
                window_id,
                ..
            } => (Some(*element_index), window_id.map(|v| v as u64)),
            cmux_cua_core::element_token::ResolvedElement::None => (None, None),
        };
...
impl Tool for SetValueTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let idx = match resolved {
            ...
            cmux_cua_core::element_token::ResolvedElement::None => return ToolResult::error(
...
impl Tool for ScrollTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let xid_opt: Option<u64> = match &resolved {
            cmux_cua_core::element_token::ResolvedElement::Element { window_id, .. } => window_id
                .map(|v| v as u64)
                .or_else(|| args.opt_u64("window_id")),
            cmux_cua_core::element_token::ResolvedElement::None => args.opt_u64("window_id"),
        };
        ...
        if let cmux_cua_core::element_token::ResolvedElement::Element { element_index, .. } =
            &resolved
        {
        ...
        if crate::wayland::wayland_input_enabled() {
            ...
            let local_point = match (pixel_target, &resolved) {
                ...
                (None, cmux_cua_core::element_token::ResolvedElement::None) => None,
            };
        ...
        let element_point = match &resolved {
            ...
            cmux_cua_core::element_token::ResolvedElement::None => pixel_target.map(|local| {
    ...
    fn def(&self) -> &ToolDef {
        RCLICK_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({"type":"object","required":["pid"],"properties":{
                ...
                "element_index": cmux_cua_core::tool_schema::element_index_schema(),
                "element_token": cmux_cua_core::tool_schema::element_token_schema(),
                "modifier": cmux_cua_core::tool_schema::modifier_schema(),
                "from_zoom":{"type":"boolean","description":"Set true after a zoom call to auto-translate zoom-image pixel coordinates back to full-window space."},
                "delivery_mode": crate::input::delivery::delivery_mode_schema()
            },"additionalProperties":false}),
            read_only: false, destructive: true, idempotent: false, open_world: true,
    ...
    fn def(&self) -> &ToolDef {
        DRAG_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({"type":"object","required":["pid","from_x","from_y","to_x","to_y"],"properties":{
                ...
                "modifier": cmux_cua_core::tool_schema::modifier_schema(),
                "button": cmux_cua_core::tool_schema::button_schema(),
        ...
        match result {
            Ok(Ok((b64_opt, shot_w, shot_h, screen_w, screen_h, written))) => {
                ...
                if let Some(path) = written {
                    ...
                    content.push(cmux_cua_core::protocol::Content::text(format!(
...
impl Tool for MoveCursorTool {
    fn def(&self) -> &ToolDef {
        MCURSOR_DEF.get_or_init(|| ToolDef {
            ...
            input_schema: json!({"type":"object","required":["x","y"],"properties":{
                "x":{"type":"number"},"y":{"type":"number"},"session": cmux_cua_core::tool_schema::session_schema(),"cursor_id":{"type":"string"}
            },"additionalProperties":false}),
...
impl Tool for CheckPermissionsTool {
    fn def(&self) -> &ToolDef {
        PERMS_DEF.get_or_init(|| ToolDef {
            ...
            description: "Check required permissions for cmux-cua on Linux.".into(),
            input_schema: json!({"type":"object","properties":{},"additionalProperties":false}),
            read_only: true,
            destructive: false,
            idempotent: true,
...
impl Tool for GetConfigTool {
    fn def(&self) -> &ToolDef {
        GCFG_DEF.get_or_init(|| ToolDef {
            ...
            description: "Return current cmux-cua configuration.".into(),
            input_schema: json!({"type":"object","properties":{},"additionalProperties":false}),
            read_only: true,
            destructive: false,
            idempotent: true,
    ...
    async fn invoke(&self, _args: Value) -> ToolResult {
        ...
        ToolResult::text("cmux-cua configuration").with_structured(json!({
...
impl Tool for SetConfigTool {
    fn def(&self) -> &ToolDef {
        SCFG_DEF.get_or_init(|| ToolDef {
            ...
            description: "Update cmux-cua configuration. capture_mode / \
                ...
                The experimental_pip keys persist to ~/.cmux-cua/config.json and apply on next \
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        match result {
            Ok(Ok(crop)) => {
                ...
                use cmux_cua_core::protocol::Content;
                ToolResult {
                    content: vec![
...
impl Tool for TypeTextCharsTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        // cmux_cua_core::text_sanitize for rationale.
        let text = cmux_cua_core::text_sanitize::strip_trailing_agent_protocol_tags(&text_raw)
            .into_owned();
...
pub fn build_registry(compat: bool) -> ToolRegistry {
    ...
    r.register(Box::new(
        cmux_cua_core::health_report::HealthReportTool::new(std::sync::Arc::new(
    ...
    r.register(Box::new(cmux_cua_core::page::PageTool::new(Arc::new(
