...
pub fn ensure_chromium_accessibility_enabled() {
    ...
    ADVERTISED.call_once(|| {
        let mode = advertise_mode_from(
            std::env::var_os("CMUX_CUA_DISABLE_A11Y_ADVERTISE").is_some(),
            std::env::var("CMUX_CUA_A11Y_ADVERTISE_MODE")
                .ok()
                .as_deref(),
...
fn advertise_mode_from(
        ...
        .as_deref()
    {
        ...
        Some(other) => {
            tracing::warn!(
                mode = other,
                "unknown CMUX_CUA_A11Y_ADVERTISE_MODE; using desktop default"
            );
