---
name: cmux-cua-windows
description: Drive a native Windows app (Win32, UWP, WebView2/Edge) via the cmux-cua CLI or MCP server — snapshot its UIA tree, click/type/scroll by element_index or window-client pixels, verify via re-snapshot, all without bringing the target to the foreground. Use when the user asks you to operate, drive, automate, or perform a GUI task in a real Windows application on the host.
---

# cmux-cua — Windows

Orchestrates Windows app automation via the `cmux-cua` Rust binary
(`cmux-cua` repo, ships as `cmux-cua.exe`). Whenever a user
...
PostMessage** (UWP Calculator, Win11 Notepad, WinUI3 apps, etc.). cmux-cua
...
spawn the `cmux-cua-uia` worker (UIAccess-manifested PE), or — for tasks
...
- **Tab-switching shortcuts in browsers (`Ctrl+1..9`, `Ctrl+Tab`,
  ...
  **Prefer the windows-over-tabs pattern**: for each URL you need to
  ...
  anything. Tabs are a UX grouping for humans; cmux-cua
...
via `cmux-cua get_window_state` — none of these change focus.
...
`cmux-cua launch_app` is the one correct path for process startup —
...
## Defaults — always prefer cmux-cua over shell shims

**Default transport is the `cmux-cua` CLI** — `Bash` shelling out
to `cmux-cua <tool-name>` with JSON piped via stdin (avoids
PowerShell 5.1's argv quoting quirks for strings containing both
quotes and spaces). MCP tools (prefix `mcp__cmux-cua__*`) only when
...
doc means `cmux-cua <name>` with JSON piped via stdin — translate
to MCP form only when MCP is requested.

### CLI argument plumbing on Windows

Three equivalent shapes for passing JSON to `cmux-cua <tool>`:

1. **Stdin pipe (recommended)** — avoids PS quoting bugs entirely:
   ```powershell
   '{"pid":1234,"text":"hello world"}' | & cmux-cua call type_text
   ```
2. **Positional with escaped quotes** — works for JSON without spaces
   in string values:
   ```powershell
   & cmux-cua call list_windows '{\"app_name\":\"Calculator\"}'
   ```
...
3. **`--%` stop-parser directive** (PowerShell 5.1 specific):
   ```powershell
   & cmux-cua call type_text --% {"pid":1234,"text":"hello world"}
   ```
...
When a cmux-cua call surprises you, diagnose cmux-cua first:
...
1. **Does this command foreground the target?** If yes — stop and
   translate to the cmux-cua equivalent from the mapping table.
2. **Does this command move the user's real cursor?** (`SendInput`,
   ...
   warps the cursor.
3. **Does this command bypass cmux-cua entirely?** (PowerShell
   GUI scripts, AutoHotkey, `SendKeys`, AppleScript-equivalent
   automation tools.) If yes — stop; find the cmux-cua tool that
   does the intent.
...
1. **`cmux-cua` is on `$PATH`** — `Get-Command cmux-cua` or
   `where.exe cmux-cua`. Install location:
   `%LOCALAPPDATA%\Programs\trycua\cmux-cua\bin\cmux-cua.exe`,
   added to the user PATH by the install script.
   If missing, point the user at:
   ```powershell
   irm https://github.com/manaflow-ai/cmux-cua/releases/latest/download/install.ps1 | iex
   ```
   and stop.
2. **The daemon must run in an interactive session (Session 1+),
   ...
   ```powershell
   Get-Process cmux-cua | Select Id,SessionId
   ```
   ...
   ```powershell
   schtasks /Run /TN cmux-cua-serve
   ```
3. **Run `cmux-cua doctor`** — reports session ID, COM apartment
   status, UIA reachability, install paths, version. If anything reads
   `false` / `error`, fix that before tool-calling.
4. **Permissions** — Windows has no TCC equivalent. cmux-cua
   needs:
...
## Using cmux-cua from the shell

Tool names are `snake_case`, management subcommands are
`kebab-case` — no ambiguity. Tools invoked as `cmux-cua call
<tool-name>` with JSON via stdin or positional arg. Management
subcommands:

- **`cmux-cua serve`** — start persistent daemon (**required** for
  ...
  re-run with `schtasks /Run /TN cmux-cua-serve`, not by spawning
  `cmux-cua serve` from SSH (Session 0 problem).
- **`cmux-cua stop`** / **`status`** — daemon lifecycle.
- **`cmux-cua doctor`** — full diagnostics.
- **`cmux-cua list-tools`** / **`describe <tool>`** — tool surface
  discovery.
- **`cmux-cua autostart {enable|disable|status|kick}`** — manage the
...
- **`cmux-cua recording start|stop|status`** — see `RECORDING.md`.
...
'{"aumid":"Microsoft.WindowsCalculator_8wekyb3d8bbwe!App"}' | & cmux-cua call launch_app
# → {pid: 6004, windows: [{window_id: 459672, ...}]}

# Snapshot the UIA tree.
'{"pid":6004,"window_id":459672}' | & cmux-cua call get_window_state
# Returns: tree_markdown with [N] element indices, screenshot, dimensions.

# Click element [22] (the "Equals" button per the tree).
'{"pid":6004,"window_id":459672,"element_index":22}' | & cmux-cua call click
...
(see forbidden shortcuts above); window switching via `cmux-cua`
...
1. **Bookmark-URL UIA bypass (default)** — `cmux-cua` looks for a
   bookmark named `cmux-cua-eval` on the Favorites bar, edits its
   ...
   **Requirements:**
   - The `cmux-cua-eval` bookmark **must exist** on the Favorites
   ...
   - **The Favorites bar must already be visible.** This is a
     ...
     hidden, cmux-cua now synthesizes `Ctrl+Shift+B` via
...
2. **CDP fallback** — `Runtime.evaluate` via raw WebSocket against
   ...
   that flag and `CMUX_CUA_CDP_PORT=N` exported before the daemon
   starts.
...
`cmux-cua-eval` URL field and one caller would invoke another's
...
- **`Session 0` daemon** — `cmux-cua doctor` reports
...
- **JPEG screenshot has more compression than expected** — default
  ...
  `cmux-cua call screenshot`, defaults to PNG (no compression).
...
- PATH state (is `cmux-cua` actually on PATH?)
...
`cmux-cua autostart status` reports whether the daemon is
...
- `not-registered` — install didn't set up autostart, or user
  removed the task. Re-register via `cmux-cua autostart enable`.
- `registered (not running)` — autostart task exists but no daemon
  process. Kick it with `cmux-cua autostart kick`.
- `registered (running)` — happy path.

## Recording

Screen recording is **not yet supported on Windows** in
cmux-cua. The `recording start|stop|status` subcommands are
registered but return "Recording is currently macOS-only" on
Windows. Tracking: see the cmux-cua roadmap in the main repo.
