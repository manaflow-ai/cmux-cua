---
name: cmux-cua
description: Drive a native GUI app (macOS, Windows, Linux) via the cmux-cua CLI (default) or MCP server — snapshot its accessibility tree, click/type/scroll by element_index or pixel coords, verify via re-snapshot, all without bringing the target to the foreground. Use when the user asks you to operate, drive, automate, or perform a GUI task in a real application on the host.
---

# cmux-cua

Orchestrates cross-platform app automation via `cmux-cua`. Whenever
...
`cmux-cua doctor` — it reports the platform and the right entry
point.
...
"raise", or "make key", stop and translate to the cmux-cua tool
...
to `cmux-cua <tool-name> '<JSON-args>'`. MCP tools (prefix
`mcp__cmux-cua__*`) only when the user explicitly asks for them.
...
skill means `cmux-cua click '{...}'` — translate to MCP form only
when MCP is requested.

### Claude Code computer-use compatibility mode

For normal Claude Code use, keep the default CLI or `cmux-cua` MCP
server path above. If the user explicitly wants Claude Code's
vision/computer-use-style flow, they can register:

```bash
cmux-cua mcp-config --client claude   # then paste + run the printed line
```
...
the normal CmuxCua tools and changes only `screenshot`. The
...
shell out to `cmux-cua screenshot` as a substitute: CLI screenshots
still work as CmuxCua calls, but they do not expose the
...
`cmux-cua approvals list|revoke|clear`.
...
`kebab-case` — no ambiguity. Tools invoked as `cmux-cua
...
- `cmux-cua serve` — start persistent daemon (**required** for
  ...
  form.
- `cmux-cua stop` / `status`
- `cmux-cua list-tools`, `describe <tool>`
- `cmux-cua recording start|stop|status` — see `RECORDING.md`
- `cmux-cua check-update [--json] [--no-cache]` — read-only "is a newer release available?" probe. Same payload as the `check_for_update` MCP tool; pair with `cmux-cua update --apply` to install.
...
```bash
cmux-cua serve
cmux-cua launch_app '{"bundle_id":"..."}'
# → {pid: 844, windows: [{window_id: 10725, ...}]}
cmux-cua get_window_state '{"pid":844,"window_id":10725}'
cmux-cua click '{"pid":844,"window_id":10725,"element_index":14}'
cmux-cua stop
```
...
`cmux-cua set_agent_cursor_enabled '{"enabled":true|false}'` only to
...
**Per-session cursors.** In **embedded** mode (`CMUX_CUA_EMBEDDED=1`)
anonymous calls automatically own a default cursor keyed by
`CMUX_CUA_DEFAULT_SESSION` (else `embedded-<pid>`), so you pass nothing
...
Requires the daemon process's UI runloop, which `cmux-cua serve` /
...
distinct *connections*. Subagents that share one `cmux-cua mcp` (stdio)
...
parallel agents, give each its **own connection**: separate `cmux-cua mcp`
processes, or point each agent's MCP client at the daemon's HTTP endpoint
(`CMUX_CUA_MCP_HTTP_PORT` → `POST http://127.0.0.1:<port>/mcp`). The daemon
...
cmux-cua get_window_state '{"pid":N,"window_id":W,"screenshot_out_file":"/tmp/shot.jpg"}'

# CLI --screenshot-out-file flag is equivalent
cmux-cua get_window_state '{"pid":N,"window_id":W}' --screenshot-out-file /tmp/shot.jpg
```
...
authoritative. cmux-cua does not pre-flight check against it —
...
`cmux-cua-core::tool_schema` (+ `capture_mode`), and a CI gate
...
`cmux-cua recording start|stop|status`; raw tools:
