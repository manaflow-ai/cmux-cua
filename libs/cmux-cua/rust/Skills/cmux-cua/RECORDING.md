...
inherits cmux-cua's own Screen Recording grant — no separate
...
tools, or the friendlier `cmux-cua recording` subcommand group
(wraps both with human-readable output).

```
cmux-cua recording start ~/cua-trajectories/run-1
# … run the workflow …
cmux-cua recording status    # -> enabled / disabled, next_turn, output_dir
cmux-cua recording stop      # -> "Recording stopped. (video → recording.mp4)"
```

Raw-tool equivalent:

```
cmux-cua start_recording '{"output_dir":"~/cua-trajectories/run-1"}'
cmux-cua get_recording_state
cmux-cua stop_recording '{}'
```

The `recording` subcommands require a running daemon (`cmux-cua
...
```
cmux-cua recording start ~/cua-trajectories/demo1
# … run the workflow …
cmux-cua recording stop
# Later: replay against a new build.
cmux-cua replay_trajectory '{"dir":"~/cua-trajectories/demo1","delay_ms":500}'
```
