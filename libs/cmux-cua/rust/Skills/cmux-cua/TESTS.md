# Natural-language tests for `cmux-cua`
...
**Prompt:** `Create a new note in Notes titled 'cmux-cua test' with the body 'native text entry verification'.`

**Exercises:** `type_text` via `kAXSelectedText` against native AppKit text fields.

**Success:**
- Re-snapshot of Notes shows a note row in the sidebar whose title contains `cmux-cua test`.
...
These are where CmuxCua's AX-activation trio matters:
