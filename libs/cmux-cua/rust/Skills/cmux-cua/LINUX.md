---
name: cmux-cua-linux
description: Drive a native Linux app via the cmux-cua CLI — snapshot the AT-SPI tree, click by element_index or pixel, type, screenshot, record, verify via re-snapshot. Background by default (no foreground steal, no real-pointer move) on X11. Wayland input/capture is opt-in and still preview.
---
...
- **`background`** (default) — inject **without activating or raising** the
  ...
  XInput2 MPX pointer). This is cmux-cua's differentiator and the right
  default.
...
session's D-Bus**. cmux-cua reaches it via `DBUS_SESSION_BUS_ADDRESS`. When
...
cmux-cua now **auto-discovers the session bus at startup** (mirroring the
...
1. **An accessibility bus must be running** in that session, and
   **`toolkit-accessibility` must be on** — cmux-cua advertises a screen
   reader at startup to flip it, but a session with no a11y bus at all
   (`/usr/libexec/at-spi-bus-launcher`) can't expose a tree. `cmux-cua
...
Set `CMUX_CUA_ENABLE_WAYLAND=1` to enable native Wayland support. The
...
1. `cmux-cua doctor` — reports the display server (X11 / Wayland),
...
2. Check `XDG_SESSION_TYPE` — `x11` is fully supported; `wayland`
   needs `CMUX_CUA_ENABLE_WAYLAND=1` for native input (preview),
   else XWayland.
...
Prefer cmux-cua tools with an explicit `window_id`. When in doubt,
ask the user.
