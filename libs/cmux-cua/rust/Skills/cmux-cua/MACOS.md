...
reason cmux-cua exists. Users pay for the right to keep typing in
...
- **Tab-switching shortcuts in browsers (`⌘1..⌘9`, `⌘]`, `⌘[`,
  ...
  **Prefer the windows-over-tabs pattern**: for each URL you need to
  ...
  are a UX grouping for humans; cmux-cua workflows should default to
...
`cmux-cua launch_app` is the one correct path for process
...
3. **Does this command bypass cmux-cua entirely?** (`osascript`
   mutating GUI state, AppleScript files, external helpers.) If
   yes — stop; find the cmux-cua tool that does the intent.
...
1. `cmux-cua` is on `$PATH` (`which cmux-cua`). If not, point the
   user at `scripts/install-local.sh` and stop.
2. Run `cmux-cua check_permissions` (with the daemon up — see step 3).
   ...
   Accessibility and Screen Recording to `cmux Computer Use.app`, then stop.
   Pass `'{"prompt":false}'` for a purely read-only status check that
   won't steal focus.
3. Start the daemon with `open -n -g -a 'cmux Computer Use' --args serve` (the
   recommended form — goes through LaunchServices so TCC attributes
   the process to cmux Computer Use.app). `cmux-cua serve &` also works;
   the CLI auto-relaunches through `open -n -g -a "cmux Computer Use"` when it
   detects a wrong-TCC context (any IDE-spawned shell: Claude Code,
   Cursor, VS Code, Conductor). Verify with `cmux-cua status`.
...
entire point of cmux-cua: agents drive apps in the background while
...
`Sources/CmuxCuaCore/Input/MouseInput.swift` (`clickViaAuthSignedPost`)
...
2. `CGEvent.post(tap: .cghidEventTap)` with a leading `mouseMoved`
   event (~30 ms before the click). `cmux-cua click` when the
...
is the single most-misused cmux-cua capability. If the target is
...
`--remote-debugging-port=9222` — cmux-cua will detect and prefer the
