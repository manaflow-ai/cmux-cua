...
Canonical HTML test fixtures used by the cmux-cua test harness. One source
...
```bash
open libs/cmux-cua-fixtures/test_page.html
```
...
```
libs/cmux-cua/rust/tests/integration/
├── fixtures/
│   └── interactive.html      → ../../../../cmux-cua-fixtures/interactive.html
└── v2/assets/
    └── test_page.html        → ../../../../../cmux-cua-fixtures/test_page.html
```
...
but doesn't probe four gestures that the Windows cmux-cua stress
...
- **GPU-composited content blanks PrintWindow.** `cmux-cua screenshot`
...
- **`hotkey` SendInput injection requires the daemon at UIAccess
  ...
  of 4 events`. Workaround: run `cmux-cua` via the
  `cmux-cua-uia.exe` worker (PR #1593) which carries the manifest.
